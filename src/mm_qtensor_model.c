...
/*   if(pd->e[0][R_MESH1]) EH(-1, "assemble_qtensor is not deformable mesh friendly."); */
	  ...
	  for( j=0; j<ei->dof[var]; j++)
	      ...
	      for(a = 0; a < DIM; a++)
		{
		  ...
		  for ( i=0; i<ei->dof[eqn]; i++)
		    {
		      wt_func = bf[eqn]->phi[i];
		      R_new[a][i] = (-vort_dir_pert[a]*pd->etm[0][eqn][(LOG2_ADVECTION)]+
				     fv->vd[a]*pd->etm[0][eqn][(LOG2_SOURCE)])*wt_func * dV;
		...
		for(a = 0; a < DIM; a++)
		  {
			...
			wt_func = bf[eqn]->phi[i];
			R_new[a][i] = (-vort_dir_pert[a]*pd->etm[0][eqn][(LOG2_ADVECTION)]+
				       fv->vd[a]*pd->etm[0][eqn][(LOG2_SOURCE)])*wt_func 
			  * wt * hh3 * detJ1;
			lec->J[peqn][pvar][i][j] += (R_old[a][i]-R_new[a][i])/eps;
