...
{ 
  ...
  for (ip = 0; ip < ip_total; ip++) {
    ...
    if (TimeIntegration != STEADY && pd->e[0][MESH_DISPLACEMENT1]) {
	...
	for (i = 0; i < num_dofs; i++)  {
	  ...
	  for (p = 0; p < BC_Types[bc_input_id].desc->vector; p++) {
		...
		(((BC_Types[bc_input_id].BC_Name == LAGRANGE_NO_SLIP_BC) ||
		 ...
		 ((ac_lm == 1) || (pd->i[0][R_LAGR_MULT1] == I_P0))) ) {
	      /*
		...
		weight *= pd->etm[0][eqn][(LOG2_BOUNDARY)];
	      }
...
/* make sure this element has velocity defined */
  if ( pd_glob[ei->mn]->i[0][VELOCITY1] <= 0 ) {
...
/***************************** EXECUTION BEGINS ******************************/
  ...
  DeformingMesh = pd->e[0][R_MESH1]; /* Use to catch bad references to moving */
				  /* mesh which isn't. */
...
#define DEBUG_LS_INTEGRATION 0
	  ...
	  if (oAC != 0)
	      ...
	      if ( !ls->Ignore_F_deps )
		{
		  ...
		  if( pd->e[0][R_ENERGY] && !ls->AdaptIntegration )
		    {
		      assemble_energy_path_dependence(
				 time_value, theta, dt,
					        pg_data);
		    }
		  if( pd->e[0][R_MASS] && !ls->AdaptIntegration )
		    {
                      ...
                      int lvdof )
{
  ...
  if ( pd->i[0][var_type] != I_P0 && pd->i[0][var_type] != I_P1 &&
       pd->i[0][var_type] != I_P0_GP && pd->i[0][var_type] != I_P1_GP &&
       pd->i[0][var_type] != I_P0_GN && pd->i[0][var_type] != I_P1_GN &&
       pd->i[0][var_type] != I_P0_XV && pd->i[0][var_type] != I_P1_XV &&
       pd->i[0][var_type] != I_P0_G && pd->i[0][var_type] != I_P1_G )
    {
      ...
      */
{
   ...
   */
  if (ei->deforming_mesh && (pd->e[0][R_MESH1] || pd->v[0][R_MESH1]))
    {
      if (!pd->e[0][R_MESH1]) {
     // printf(" We are here\n");
      }
    ...
    //if (ei->deforming_mesh && pd->e[0][R_MESH1])
    // {
    // err = load_bf_mesh_derivs(); 
    // EH( err, "load_bf_mesh_derivs");
    //}
