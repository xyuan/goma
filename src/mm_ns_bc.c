...
static double slip_coefficient ( const double,
                                 const double,
                                 const double,
                                 const double,
                                 double *,
...
void
apply_SES(double func[MAX_PDIM],
	   ...
	   int id_side )
{
  ...
  if (nf == 1 && upd->vp[0][SHELL_TENSION])
    {
      ie = Index_Solution(inode, SHELL_TENSION, 0, 0, -1);
  ...
  if ( lsi->near)
    {
      ...
      for(i=0; i< dim; i++) n_i[i] = fv->grad_F[i];
...
/****************************************************************************/
void 
fvelo_normal_bc(double func[DIM],
		double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
  ...
  if (af->Assemble_Jacobian) {
	...
	if (pd->v[0][var]) {
	  for ( j=0; j<ei->dof[var]; j++) {
	    ...
	    if (TimeIntegration != 0 && p == kdir) {
	      d_func[0][var][j] += penalty*( -(1.+2.*tt)* phi_j/dt) * fv->snormal[kdir] * delta(p, kdir);
	    }
	  }
	}
      }
	...
	for ( j=0; j<ei->dof[var]; j++) {
      ...
      if((type == VELO_NORMAL_LS_BC || type == VELO_NORMAL_LS_PETROV_BC
           || VELO_NORMAL_LS_COLLOC_BC)  && pd->v[0][var] )
	{
	  for( j=0; j<ei->dof[var]; j++)
	    {
  ...
  if (af->Assemble_Jacobian) {
	...
	if (pd->v[0][var]) {
	  for ( j=0; j<ei->dof[var]; j++) {
	    ...
	    if (TimeIntegration != 0 && p == kdir) {
	      d_func[0][var][j] += penalty*( -(1.+2.*tt)* phi_j/dt) * fv->stangent[0][kdir] * delta(p, kdir);
	    }
	  }
	}
      }
...
double
sdc_stefan_volume_flow(JACOBIAN_VAR_DESC_STRUCT *func_jac,
		       BOUNDARY_CONDITION_STRUCT *bc,
		       int ip, ELEM_SIDE_BC_STRUCT *elem_side_bc,
		       const double x_dot[MAX_PDIM], const dbl tt, 
		       const dbl dt)
...
double
mass_flux_surface(JACOBIAN_VAR_DESC_STRUCT *func_jac,
                  const double x_dot[MAX_PDIM], const dbl tt, const dbl dt)
      ...
      ************************************************************************/
{
  ...
  if (af->Assemble_Jacobian) {
	 ...
	 *  Jacobian_Var_Desc structure
	 */
        jacobianVD_addNewEntry(func_jac, lvdesc,
                               n_dot_diffv * pj->JacVector[j]);
      }
    }
  } /* end of if Assemble_Jacobian */
  return func_value;
...
double
vol_flux_surface(JACOBIAN_VAR_DESC_STRUCT *func_jac,
		 const double x_dot[MAX_PDIM], const dbl tt, const dbl dt)
...
void 
fvelo_normal_edge_bc(double func[DIM],
		     double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		     const double vnormal, /* normal velocity */
	...
	if (pd->v[0][var]) {
	  for ( j=0; j<ei->dof[var]; j++) {
	    ...
	    d_func[0][var][j] += (fv->v[kdir] - x_dot[kdir]) *
	      fv->dstangent_dx[0][kdir][p][j];
	    if (TimeIntegration != 0 && p == kdir) {
	      d_func[0][var][j] += ( -(1.+2.*tt)* phi_j/dt) *
		fv->stangent[0][kdir] * delta(p, kdir);
	    }
...
void 
fvelo_normal_disc_bc(double func[DIM],
		     double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		     const double vnormal,         /* normal velocity */
...
void
fvelo_tangent_edge_bc(double func[DIM],
		      double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
      ...
      *  Here t is the tangent to the edge in the plane of the primary (first) surface used in identifying
      *  the edge and vweb is the velocity of the a moving substrate.
      *
...
void 
fvelo_tangential_bc(double func[],
		    double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
     ...
     
{
  ...
  if (alpha != 0.) {
    ...
    for (icount = 0; icount < pd->Num_Dim; icount ++)  {
      dist += (xsurf[icount] - xdcl[icount]) *
	(xsurf[icount] - xdcl[icount]);
    }
    dist = sqrt(dist);
  }
	      ...
	      if (pd->v[0][var])
		{
		  for ( j=0; j<ei->dof[var]; j++)
			...
			* fv->dstangent_dx[0][kdir][p][j];
		      ...
		      if(TimeIntegration != 0 && p == kdir)
			{
			  d_func[0][var][j] += ( -(1.+2.*tt)* phi_j/dt * beta * ead ) 
			    * fv->stangent[0][kdir];
			}
		    }
		  /* 
	...
	vtang_user = fv->stangent[0][0]*vel_user[0] + fv->stangent[0][1]*vel_user[1];
  	*func -= vtang_user;
  	if (af->Assemble_Jacobian) 
	    ...
	    if( pd->e[0][R_MESH1] )
	      {
		vel_user_dx[0][0] = dvelo_vary_fnc_d1(UVARY_BC, fv->x[0], fv->x[1]
						      , fv->x[2], u_par, time_value);
		vel_user_dx[0][1] = dvelo_vary_fnc_d2(UVARY_BC, fv->x[0], fv->x[1]
						      , fv->x[2], u_par, time_value);
		vel_user_dx[1][0] = dvelo_vary_fnc_d1(VVARY_BC, fv->x[0], fv->x[1]
						      , fv->x[2], u_par, time_value);
			...
			for ( j=0; j<ei->dof[var]; j++)
			  {
			    ...
			    d_func[0][var][j] += (fv->v[kdir] - x_dot[kdir] * beta * ead ) 
			      * fv->dstangent_dx[0][kdir][p][j];
			    d_func[0][var][j] -= 
			      (vel_user[kdir]*fv->dstangent_dx[0][kdir][p][j] 
...
void 
fvelo_slip_electrokinetic_bc(double func[MAX_PDIM],
	      double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...
/***************************** EXECUTION BEGINS ******************************/
  ...
  t_gen[0] = 0.; t_gen[1] = 0.; t_gen[2]= 0.;
  v_tangent[0] = 0.; v_tangent[1] = 0.; v_tangent[2]= 0.;
  memset( d_t_gen_dx, 0, MAX_PDIM*MAX_PDIM*MDE*sizeof(double) );
		  ...
		  if (pd->v[0][var])
		      ...
		      for ( j=0; j<ei->dof[var]; j++)
			{
			  ...
			  d_func[0][var][j] += (fv->v[kdir]) 
			    * fv->dstangent_dx[0][kdir][p][j]*betainv;
			}
		    }
		}
       ...
       if( type == VELO_TANGENT_SOLID_BC )
 	{
		...
		if (pd->v[0][var])
		  {
			...
			for(w=0; w<pd->Num_Species_Eqn; w++) 
			  {
			    /*     d( )/dC        */
			    d_func[0][MAX_VARIABLE_TYPES + w][j_id] += d_vconv->C[kdir][w][j_id]*fv->stangent[0][kdir]*betainv;
			  }
		      }
		  }
	      }
		...
		for ( j_id=0; j_id<ei->dof[var]; j_id++)
		  {
			...
			for(kdir=0; kdir<dim; kdir++) 
			  {
			    /*     d( )/dx        */
 			    d_func[0][var][j_id] += ((vconv[kdir]+v_solid_mesh[kdir])*fv->dstangent_dx[0][kdir][jvar][j_id]
 			      + d_vconv->X[kdir][jvar][j_id]*fv->stangent[0][kdir])*betainv;
			  }
		      }
		  }
	      }
 		...
 		for ( j_id=0; j_id<ei->dof[var]; j_id++)
 		  {
 			...
 			for(kdir=0; kdir<dim; kdir++) 
 			  {
 			    /*     d( )/dx        */
 			    d_func[0][var][j_id] += d_vconv->rs[kdir][jvar][j_id]
 						*fv->stangent[0][kdir]*betainv;	
 			  }
 		      }
 		  }
 	      }
		...
		if (pd->v[0][var]) 
		  { 
			...
			if(TimeIntegration != 0) 
			  { 
			    d_func[0][var][j] += ( (1.+2.*tt)* phi_j/dt)*fv->stangent[0][jvar]*betainv;  
			  } 
		      } 
		  }
	      } 
		...
		for ( j_id=0; j_id<ei->dof[var]; j_id++)
		  {
			...
			phi_j = bf[var]->phi[j_id];
 			for(kdir=0; kdir<dim; kdir++) 
 			    {
 			d_func[0][var][j_id] += d_vconv->v[kdir][jvar][j_id] * fv->stangent[0][kdir]*betainv;
 			    }
		      }
		  }
	      }
		...
		for ( j_id=0; j_id<ei->dof[var]; j_id++)
		  {
			...
			for(kdir=0; kdir<dim; kdir++) 
			  {
			    /*     d( )/dx        */
			    d_func[0][var][j_id] += (vconv[kdir]+v_solid_mesh[kdir])
			                            *fv->dsnormal_dx[kdir][jvar][j_id]
...
#define TANGENT_SLIP_ONLY 0
  ...
  if( TimeIntegration == TRANSIENT && pd->e[0][R_MESH1] )
    {
   ...
   * STRONG IC for no slip
   */
  if ( alpha*dist > log(beta*LITTLE_PENALTY) )
    {
      betainv		   = LITTLE_PENALTY;		
      d_betainv_dvslip_mag = 0.;
      d_betainv_dP	   = 0.;
  ...
  if (af->Assemble_Jacobian) {
     ...
     */
    if  ( (type == VELO_SLIP_FILL_BC || type == VELO_SLIP_ROT_FILL_BC ) 
	  && pd->v[0][MESH_DISPLACEMENT1] && ls !=NULL)
      {
	for ( jvar=0; jvar < ei->ielem_dim; jvar++ )
	  {
	    var = MESH_DISPLACEMENT1 + jvar;
	    if ( pd->v[0][var] && lsi->near )
	      {
		for ( j=0; j < ei->dof[var]; j++ )
		  {
		    for ( p=0; p < pd->Num_Dim; p++ )
		...
		for (j=0; j<ei->dof[var]; j++)
		  {
		    ...
		    for (p=0; p<pd->Num_Dim; p++)
		      {
			d_func[p][var][j] += (-betainv) * phi_j *
			  (-fv->snormal[jvar] * fv->snormal[p]);
		      }
		  }
	      }
	  }
                 ...
                 const double dt)
{
  ...
  beta = slip_coefficient( beta_inside, beta_outside, fv->F, width, d_beta_dF, gas_phase_factor, contact_fraction );
...
#if 1
      if (type == VELO_SLIP_LEVEL_SIC_BC && pd->v[0][var] )
	{
	  for (p=0; p<pd->Num_Dim; p++)
	    {
	      for (q=0; q<pd->Num_Dim; q++)
  ...
  for( i=0; i< Proc_SS_Node_Count[k]; i++)
    {
      ...
      ie = Index_Solution(I,R_FILL, 0, 0, -2);
      if(  F*x[ie] <= 0.0 )
	{
	  double distance = 0.0;
...

{
  ...
  double dil, fact;
      ...
      else if(mp->SurfaceTensionModel == DILATION)
	{
	  ...
	  mp->surface_tension = mp->u_surface_tension[0] + mp->u_surface_tension[1]
	    * (1 - 1 / fabs(dil));
 ...
 ******************************************************************************/
void
elec_surf_stress(double cfunc[MDE][DIM],
...
void
sheet_tension ( double cfunc[MDE][DIM],
		double d_cfunc[MDE][DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...

{
  ...
  if (var_sh_tens) memset( dT_dS_dmesh, 0, DIM*MDE*sizeof(double));
  memset( ddetJ_dmesh, 0, DIM*MDE*sizeof(double));
...
/*  if( fv->snormal[1] < 0.0 ) sheet_tension = -1.0 * fabs( sheet_tension);
	...
	if( fv->snormal[1] < 0.0) surf_sign = -1.0;
	else surf_sign = 1.0;
                        /* this is a total kludge to change the sign of 
			 * the curvature convention so that a positive pressure
			 * will always cause the surface to bulge away from the
  ...
  if (var_sh_tens)
    {
      ...
      /* Grab nodal sh_tens values if needed */
          T_dof[i] = Index_Solution(I, SHELL_TENSION, 0, 0, -1);
					 ...
					 if (var_sh_tens)
                            {
							 ...
							 /* d_cfunc[ldof][0][var][j] += bf[eqn]->phi[ldof]*(dT_dS * dY_dS/detJ/detJ )*ddetJ_dmesh[j][b];*/
              ...
              if ( var_sh_tens)
                {
                  ...
                  for (k=0; k< (int) elem_side_bc->num_nodes_on_side ; k++)
                    ...
                    /*  d_cfunc[ldof][0][var][j] = -bf[var]->phi[j] * dY_dS* sign * bf[eqn]->dphidxi[ldof][i_basis] / detJ;
					  d_cfunc[ldof][0][var][j]  -=  bf[eqn]->phi[ldof] *  sign*bf[var]->dphidxi[j][0] * dY_dS /detJ ;   */
					       
					  if ( T_dof[k] != -1 ) {
						d_cfunc[ldof][0][var][j] = -phi_shell[k] * sign*bf[eqn]->dphidxi[ldof][i_basis] * dY_dS/detJ;
						d_cfunc[ldof][0][var][j]  -= bf[eqn]->phi[ldof] *  sign*dphidxi_shell[k] * dY_dS /detJ ;   
					  }
			  ...
			  for (p=0; p<VIM; p++)
			    {
 			      ...
 			      d_cfunc[ldof][a][var][j] -= sigma * mp->surface_tension * 
			      ...
			      for (q=0; q<VIM; q++)
				{
				  d_cfunc[ldof][a][var][j] += sigma *  mp->surface_tension * 
				    (
				     bf[VELOCITY1+a]->d_grad_phi_e_dmesh[ldof][a] [q][p][jvar][j] * fv->snormal[p] * fv->snormal[q] +
				     bf[VELOCITY1+a]->grad_phi_e[ldof][a] [q][p] * fv->dsnormal_dx[p][jvar][j] * fv->snormal[q] +
				     bf[VELOCITY1+a]->grad_phi_e[ldof][a] [q][p] * fv->snormal[p] * fv->dsnormal_dx[q][jvar][j]);
				}
			    }
		...
		{	  
		  for (w=0; w<pd->Num_Species_Eqn; w++)
			...
			{  
			  for (p=0; p<VIM; p++)
			    { 
			      d_cfunc[ldof][a][var][j] -= sigma * mp->d_surface_tension[MAX_VARIABLE_TYPES + w] 
				* bf[VELOCITY1+a]->grad_phi_e[ldof][a] [p][p] * bf[var]->phi[j];  
			      for (q=0; q<VIM; q++)
				{
				  d_cfunc[ldof][a][var][j] += sigma * mp->d_surface_tension[MAX_VARIABLE_TYPES + w] 
		...
		{  
		  ...
		  for (p=0; p<VIM; p++)
		    { 
		      ...
		      for (q=0; q<VIM; q++)
			{
			  cfunc[ldof][a] += sigma * mp->surface_tension * bf[VELOCITY1+a]->grad_phi_e[ldof][a] [q][p] 
			    * fv->snormal[p] * fv->snormal[q];
			}
		    }
		}
...
void 
apply_repulsion (double cfunc[MDE][DIM],
		 double d_cfunc[MDE][DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...
/*****************************************************************************/
void 
apply_vapor_recoil (double cfunc[MDE][DIM],
		    double d_cfunc[MDE][DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...
/***************************** EXECUTION BEGINS *******************************/
 ...
 /* Calculate ablation pressure */
   ...
   if( theta < 0.)
     ...
     {
       pr = P_scale*(pabl_c0 + pabl_c1 * theta +
...
void
flow_n_dot_T_hydro(double func[DIM],
		   double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		   const double a, 
		   const double b,
		   const double c,
...
*****************************************************************************/
{
  ...
  if (!af->Assemble_Jacobian) d_Pi = NULL; /* save cost if we aren't assembling the Jacobian */
  ...
  if (af->Assemble_Jacobian)
      ...
      if ( pd->v[0][POLYMER_STRESS11] )
	{
      ...
      if ( pd->v[0][POLYMER_STRESS11] && (vn->evssModel == EVSS_F) )
	{
	  for (p=0; p<pd->Num_Dim; p++)
	    {
	      for (q=0; q<pd->Num_Dim; q++)
...
void
flow_n_dot_T_gradv(double func[DIM],
		   double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		   const double pdatum, /* pressure datum from input card */
		   const int iflag)  /* -1 to use pdatum, otherwise use P  */
...
/*  h_elem = 0.;
  ...
  for ( a=0; a<dim; a++)
    {
      meqn = R_MOMENTUM1 + a;
      momentum_residual[a] = rho*v_dot[a]  * pd->etm[0][meqn][(LOG2_ADVECTION)]
	+ fv->grad_P[a] * pd->etm[0][meqn][(LOG2_DIFFUSION)]
	  - div_s[a] * pd->etm[0][meqn][(LOG2_DIFFUSION)]
	    - div_tau_p[a]  * pd->etm[0][meqn][(LOG2_DIFFUSION)]
	     - mu * div_G[a]  * pd->etm[0][meqn][(LOG2_DIFFUSION)]
	       - f[a] * pd->etm[0][meqn][(LOG2_SOURCE)];
      for ( b=0; b<dim; b++)
	{
	  momentum_residual[a] += rho* (fv->v[b]- x_dot[b]) * fv->grad_v[b][a] 
	    * pd->etm[0][meqn][(LOG2_ADVECTION)];
	}
    }
  ...
  if(af->Assemble_LSA_Mass_Matrix)
    {
	  ...
	  meqn = R_MOMENTUM1 + a;
	  if (pd->v[0][var]) {
	    if (pd->e[0][meqn] & T_MASS) {
	      for (j = 0; j < ei->dof[var]; j++) {
		d_func[0][var][j] += tau_pspg * bf[var]->phi[j] * rho *
		    pd->etm[0][meqn][(LOG2_MASS)] * fv->snormal[a];
	      }
	    }
	  }
	  ...
	  if(pd->v[0][var])
	    if(pd->e[0][meqn] & T_ADVECTION)
	      for(j = 0; j < ei->dof[var]; j++)
		for(b = 0; b < pd->Num_Dim; b++)
		  {
		    meqn = R_MOMENTUM1 + b;
		    d_func[0][var][j] -= rho * bf[var]->phi[j] * fv->grad_v[a][b] 
		      * pd->etm[0][meqn][(LOG2_ADVECTION)] * fv->snormal[b] * tau_pspg;
		  }
	}
      return;
    }
	      ...
	      for ( j=0; j<ei->dof[var]; j++)
		{
		  ...
		  for (a=0; a<dim; a++)
			  ...
			  if ( pd->e[0][meqn] & T_MASS )
			    {
			      ...
			      mass *= rho *  pd->etm[0][meqn][(LOG2_MASS)]; 
			    }
			}

		      diffusion = 0.;
		      if ( pd->e[0][meqn] & T_DIFFUSION )
			{ 
			  diffusion -= d_mu->v[b][j]*div_G[a];
			  diffusion -= d_div_tau_p_dv[a][b][j];
			  diffusion *= pd->etm[0][meqn][(LOG2_DIFFUSION)] ; 
			}
		      ...
		      if ( pd->e[0][meqn] & T_ADVECTION )
			{
			  ...
			  advection_a *= rho * pd->etm[0][meqn][(LOG2_ADVECTION)] ; 
			}


		      source_a = 0.;
		      if ( pd->e[0][meqn] & T_SOURCE )
			{
			  source_a   -= df->v[a][b][j] * pd->etm[0][meqn][(LOG2_SOURCE)];
			}
		  ...
		  if ( pd->v[0][var] )
		      ...
		      for ( j=0; j<ei->dof[var]; j++)
			{
			  pressure_stabilization =  -fv->snormal[q] * bf[var]->grad_phi[j] [p]
			    * pd->etm[0][R_MOMENTUM1 + q][(LOG2_DIFFUSION)];
			      ...
			      for ( r=0; r<VIM; r++)
				{
				  pressure_stabilization -= 
				    fv->snormal[q] * bf[var]->phi[j]  *  fv->grad_e[p][r][q] 
				      * pd->etm[0][R_MOMENTUM1 + a][(LOG2_DIFFUSION)];
				}
			      for ( a=0; a<dim; a++)
				{
				  pressure_stabilization -= fv->snormal[a] * 
		      ...
		      if( pd->e[0][meqn])
			{
			  ...
			  if ( pd->v[0][var1] )
			    {
			      if ( pd->e[0][meqn] & T_DIFFUSION )
				{
				  pressure = fv->d_grad_P_dmesh[a] [b][j] * pd->etm[0][meqn][(LOG2_DIFFUSION)];
				}
			    }
			  ...
			  if ( pd->v[0][var1])
			      ...
			      if ( pd->e[0][meqn] & T_DIFFUSION )
				{
				  for ( mode=0; mode<vn->modes; mode++)
				    {
				      stress -= fv->d_div_S_dmesh[mode][a] [b][j] * pd->etm[0][meqn][(LOG2_DIFFUSION)];
				    }
				}
			    }
			  ...
			  if ( pd->v[0][var1])
			    {
			      if ( pd->e[0][meqn] & T_DIFFUSION )
				{
				  velocity_gradient -= fv->d_div_G_dmesh[a] [b][j] * pd->etm[0][meqn][(LOG2_DIFFUSION)];
				}
			    }
			  ...
			  if ( pd->e[0][meqn] & T_SOURCE )
			    {
			      source_a -= df->X[a][b][j] * pd->etm[0][meqn][(LOG2_SOURCE)];
			    }
...
void
fapply_CA (
     double *func,  /* Value of the Residual Function */
     double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
      ...
      for (p=0; p<pd->Num_Dim; p++)
	{
	  ...
	  if (pd->v[0][var])
		...
		if(pd->Num_Dim == 2 && contact_angle > 0.75*M_PIE)	
		  {
		    d_func[0][var][j] += dfsnormal_dx[0][p][j]*ssnormal[1]
		      -  dfsnormal_dx[1][p][j]*ssnormal[0] ;
		  } else {
		    for (q=0; q<pd->Num_Dim; q++)
		      {
      ...
      for (p=0; p<pd->Num_Dim; p++)
	      ...
	      for (j=0; j<ei->dof[var]; j++)
		{
		  if(pd->Num_Dim == 2 && contact_angle > 0.75*M_PIE)	
		    {
		      d_func_ss[0][var][j] += fsnormal[0]*dssnormal_dx[1][p][j]
			- fsnormal[1]*dssnormal_dx[0][p][j]; 
		    } else {
		      for (q=0; q<pd->Num_Dim; q++)
		        {
	  ...
	  for (j=0; j<ei->dof[var]; j++)
	    {
	      ...
	      if(  TimeIntegration == TRANSIENT)
		{
		  d_cos_CA_dx[q][j] += cT*mu*clnormal[p]*( (1.+2.*tt)* bf[var]->phi[j]/dt)
		                         *delta(p,q)/sigma;
		}
	    }
	}
  ...
  for (p=0; p<pd->Num_Dim; p++)
      ...
      for (q=0; q<pd->Num_Dim; q++)
	{
	  ...
	  for (j=0; j<ei->dof[var]; j++)
		...
		{
		  d_cos_CA_dx[q][j] -= d_cos_CA_Ca_local* 
		                       mp->viscosity*clnormal[p]*( (1.+2.*tt)* bf[var]->phi[j]/dt)*
		                       delta(p,q)/mp->surface_tension;
		}
	    }
	}
    }
...
if(((sign_of(pos[0]) == sign_of(sign_origx) && 
   fabs(pos[0]) > 1.e-6)) ||
   ((sign_of(pos[1]) == sign_of(sign_origy)) && 
   fabs(pos[1]) > 1.e-6))
  {
    *ipin = 0;
    return(1);
	  ...
	  if (pd->v[0][var])
		  ...
		  for (j=0; j<ei->dof[var]; j++)
		    {
			    ...
			    * ( advancing_ca - stat_ca) * scaling 
			    / cosh(scaling * v_rel) / cosh(scaling * v_rel)
			    * dfsnormal_dx[q][p][j] * (x_dot[q] - vwall[q]) 
			    * M_PIE / 180.)
			 );
		    }
		}
	      if(TimeIntegration != 0)
		{
		  for (j=0; j<ei->dof[var]; j++)
			 ...
			 * ( advancing_ca - stat_ca) * scaling 
			 / cosh(scaling * v_rel) / cosh(scaling * v_rel)
			 * fsnormal[p] * (1.+2.*tt) * bf[var]->phi[j] / dt 
			 ) * M_PIE / 180.;
		    }
		}
	    }
...
/* use 10 point Gaussian quadrature for now	*/
  ...
  const double gpt[10] = { -0.973906528517172, -0.865063366688985, -0.679409568299024,
			...
			0.973906528517172 };
  const double wt[10] = { 0.066671344308688, 0.149451349150581, 0.219086362515982,
...
/* disabling this sign change for now - doesn't seem necessary*/
   ...
   */
  costheta = ( fsnormal[0]*ssnormal[0] +
	       fsnormal[1]*ssnormal[1] +
	       fsnormal[2]*ssnormal[2] );
  ...
  switch (bc_type)
	...
	case VELO_THETA_COX_BC:
		...
		for(j=0 ; j< num_gauss_pts ; j++)
			...
			f_den = lambda*(SQUARE(th)-SQUARE(sinth))*
				(M_PIE-th+sinth*costh)
				+ (SQUARE(M_PIE-th)-SQUARE(sinth))*
					(th-sinth*costh);
	...
	case VELO_THETA_SHIK_BC:
		...
		veloc0max = (sinthetamax - theta_max*costhetamax)/
				(sinthetamax*costhetamax-theta_max);
...
fprintf(stderr,"velocity  %g %g %g\n",v,v_mesh,v_mesh_dt);
  ...
  if ( ! af->Assemble_Jacobian )  return;
	  ...
	  for (j=0; j<ei->dof[var]; j++)
	    {
  		switch (bc_type)
			...
			case VELO_THETA_COX_BC:
				...
				break;
			case VELO_THETA_SHIK_BC:
	        		dv_ddpj = factor *0.5*v_new/g*modified_surface_tension*mp->d_surface_tension[var]*bf[var]->phi[j];
				break;
			}
	...
	if (pd->v[0][var])
	  {
	    for (j=0; j<ei->dof[var]; j++)
  		...
  		switch (bc_type)
			...
			case VELO_THETA_TPL_BC:
	        		dv_ddpj = dewet*v0*cosh((costhetaeq - costheta)*g)*
			(costhetaeq - costheta) * modified_surface_tension*
			mp->d_surface_tension[var]*bf[var]->phi[j]*factor;
				break;
			case VELO_THETA_HOFFMAN_BC:
				dv_ddpj = factor*dewet*(g_dca-g_sca)*
					(liq_visc*modified_surface_tension*
				mp->d_surface_tension[var]*bf[var]->phi[j]
		 			- g*d_mu->T[j])/SQUARE(liq_visc);
				break;
			case VELO_THETA_COX_BC:
				f_den = log(reciprocal_slip) + 
					(q_inner/f_sca - q_outer/f_dca);
	 ...
	 if (pd->v[0][var])
	   {
		...
		{  
 		...
 		mp->d_surface_tension[MAX_VARIABLE_TYPES+w]*bf[var]->phi[j]
				...
				break;
			case VELO_THETA_SHIK_BC:
	        		dv_ddpj = factor * 0.5*v_new/g*modified_surface_tension*mp->d_surface_tension[MAX_VARIABLE_TYPES+w]*bf[var]->phi[j];
				break;
			}
...
void
apply_ST_scalar(double func[MAX_PDIM],
		double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		const int id_side, /* ID of the side of the element        */
...
/*****************************************************************************/
void
apply_ST_3D(double func[MAX_PDIM],
	    double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
	    const double tx,
	    const double ty,
	    const double tz,
...
void
apply_ST_scalar_3D(double func[MAX_PDIM],
		   double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
 ...
 if ( dim == 2 )
   {
     ...
     sign = 1.0;
     if(  ( lsi->normal[0]*fv->snormal[1] - lsi->normal[1]*fv->snormal[0] ) < 0.0 ) sign = -1.0;
     
     t[0] = sign*fv->snormal[1]; t[1] = -sign*fv->snormal[0];
     
     dot_prod = dot_product( dim, t, lsi->normal);
     
     for (p=0; p<dim; p++)
       {
	 func[p] = lsi->delta * mp->surface_tension * dot_prod *
		 ...
		 for ( j=0 ; j < ei->dof[var]; j++ )
		   {
			 ...
			 d_func[p][var][j] += lsi->delta
			   * mp->surface_tension
			   * (dt_dmesh[a][b][j]*lsi->normal[a] +
			      t[a] * lsi->d_normal_dmesh[a][b][j])
			   * (sin_ca*fv->snormal[p] + cos_ca*t[p]);
		       }
		     d_func[p][var][j] += lsi->delta
		       * mp->surface_tension
 ...
 else
   {
     ...
     if ( ! pd->v[0][MESH_DISPLACEMENT1] ) return;
     for ( b = 0; b < dim; b++ )
       {
	 var = MESH_DISPLACEMENT1 + b;
	 for ( j=0; j < ei->dof[var]; j++ )
	   {
	     ...
	     for ( a=0; a < dim; a++ )
	       {
		 ddp_dmeshbj = lsi->d_normal_dmesh[a][b][j] * nw[a]
		   + nf[a] * fv->dsnormal_dx[a][b][j];
	       }
	     ...
	     for ( a=0; a < dim; a++ )
	       {
		 dt_dmeshbj[a] = (lsi->d_normal_dmesh[a][b][j] - ddp_dmeshbj*nw[a]
				  - dp * fv->dsnormal_dx[a][b][j]) / tmag
...
void
apply_sharp_ca(  double func[MAX_PDIM],
		 double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		 const double ca)               /* contact angle                             */
     /*
      *
  ...
  if ( dim == 2 )
    {
      ...
      for (p=0; p<dim; p++)
	{
	  func[p] = mp->surface_tension * dot_prod *
	    ( sin_ca*fv->snormal[p] + cos_ca*t[p]);
	}
      if ( af->Assemble_Jacobian )
	{
...
void
apply_wetting_velocity(	double func[MAX_PDIM],
			double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE], 
			int id_side,
			int element_type,
			double depth,
			...
			double cT )
{
  ...
  if ( lsi->near )
      ...
      for( a=0; a<dim; a++)
	{
      ...
      if( wetting_length < 1.e-12 ) wetting_length = 1.;
      ...
      for( j=0; j<ei->dof[var]; j++)
	{
	  ...
	  for( a=0, d_wet_vector_dF[a][j] = 0.0; a<dim; a++)
	    d_wet_vector_dF[a][j] = d_t_dFj[a]*wet_speed*delta + t[a]*d_dp_dFj*delta/cT + t[a]*wet_speed*d_delta_dFj;
	}
#endif
      ...
      if( pd->v[0][var] )
	{
	  for(b=0 ; b<dim; b++)
	      ...
	      for( j=0 ; j<ei->dof[var]; j++)
		{
		  ...
		  for( a=0; a<dim; a++)
		    {
		      d_wet_vector_dX[a][b][j] = d_t_dXj[a][b]*wet_speed*delta + t[a]*d_dp_dXj[b]*delta/cT
...
void
apply_linear_wetting_sic( double func[MAX_PDIM],
			  double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE], 
			  const double dt,
			  const double tt,
			  int id_side,
...
void
apply_sharp_wetting_velocity(double func[MAX_PDIM],
			     double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
			     const int bc_type,
			     const double depth,
			     ...
			     const double v_old )
{
  ...
  const int num_gauss_pts = 10;
  const double gpt[10] = { -0.973906528517172, -0.865063366688985, -0.679409568299024,
			   -0.433395394129247, -0.148874338981631, 0.148874338981631,                        0.433395394129247, 0.679409568299024, 0.865063366688985,
			   0.973906528517172 };
  const double wt[10] = { 0.066671344308688, 0.149451349150581, 0.219086362515982,                        0.269266719309996, 0.295524224714753, 0.295524224714753,
			  0.269266719309996, 0.219086362515982, 0.149451349150581,
			  0.066671344308688 };
  ...
  if(bc_type == SHARP_HOFFMAN_VELOCITY_BC || 
     bc_type == SHARP_COX_VELOCITY_BC)
    {
      for ( a=0; a<VIM; a++) {
	for ( b=0; b<VIM; b++) {
	  gamma[a][b] = fv->grad_v[a][b] + fv->grad_v[b][a];
  ...
  for( j=0; j<ei->dof[var]; j++)
	...
	case SHARP_WETLIN_VELOCITY_BC:
	  ...
	  break;
	case SHARP_BLAKE_VELOCITY_BC:
	  d_wet_speed_dFj = v0*cosh(  g*
				      ( cos_ca - cos_ca_static ) )*g*d_dp_dFj;
	  break;
	case SHARP_HOFFMAN_VELOCITY_BC:
	  d_wet_speed_dFj = g/liq_visc * 
	    (1.+1.31*pow(g_dca,0.99))/
	    (1.-1.31*0.99*A_dca/pow(g_dca,0.01))*
	    pow(A_dca,0.294)*(-4.)*(-d_dp_dFj)/ (0.706*2*5.16
						 *(3.-costheta)*(1.+costheta));
	  break;
	case SHARP_COX_VELOCITY_BC:
	  /* sensitivity wrt to q_outer has not been included	*/
	  ...
	  d_wet_speed_dFj = g/liq_visc * 
	    (1./(f_dca*(-sintheta))*(-d_dp_dFj))/f_den;
	  break;
	case SHARP_SHIK_VELOCITY_BC:
	  dveloc0_ddpj = (-d_dp_dFj)*(SQUARE(theta) + 
				      theta*sintheta*costheta-2*SQUARE(sintheta))/
	    (SQUARE(sintheta*costheta)-
	     2*theta*sintheta*costheta+SQUARE(theta));
	  ...
	  drhs_ddpj = (rhs_den*drhs_num_ddpj 
		       - rhs_num*drhs_den_ddpj)/SQUARE(rhs_den);
	  d_wet_speed_dFj = 0.5*sqrt(g*v0)*(1-0.5*rhs)
	    *drhs_ddpj/pow(1+rhs,1.5);
	  break;
	}	/* end of switch	*/
      d_wet_speed_dFj *= factor;
      for( a=0, d_wet_vector_dF[a][j] = 0.0; a<dim; a++)
	d_wet_vector_dF[a][j] = d_t_dFj[a]*wet_speed 
	  + t[a]*d_wet_speed_dFj;
    }
#endif
	      ...
	      for( a=0, d_wet_vector_dX[a][b][j]=0.0 ; a<dim; a++)
		{
		  switch (bc_type)
		    ...
		    case SHARP_BLAKE_VELOCITY_BC:
		      ...
		      break;
		    case SHARP_HOFFMAN_VELOCITY_BC:
		      d_wet_speed_dXj = g/liq_visc * 
			(1.+1.31*pow(g_dca,0.99))/
			(1.-1.31*0.99*A_dca/pow(g_dca,0.01))*
			pow(A_dca,0.294)*(4.)/ (0.706*2*5.16
						*(3.-costheta)*(1.+costheta));
			...
			(q_inner/f_sca - q_outer/f_dca);
		      d_wet_speed_dXj = g/liq_visc * 
			(-1./(f_dca*(-sintheta)))/f_den;
		      d_wet_speed_dXj *= -d_dp_dXj[b];
		      d_wet_speed_dXj += wet_speed*(-1./liq_visc)*d_mu->X[b][j];
		      break;
		    case SHARP_SHIK_VELOCITY_BC:
		      dveloc0_ddpj = (-d_dp_dXj[b])*(SQUARE(theta) + 
						     theta*sintheta*costheta-2*SQUARE(sintheta))/
			(SQUARE(sintheta*costheta)-
			 2*theta*sintheta*costheta+SQUARE(theta));
		      ...
		      drhs_ddpj = (rhs_den*drhs_num_ddpj 
				   - rhs_num*drhs_den_ddpj)/SQUARE(rhs_den);
		      d_wet_speed_dXj = 0.5*sqrt(g*v0)*(1-0.5*rhs)
			*drhs_ddpj/pow(1+rhs,1.5);
		      break;
		    }	/* end of switch	*/
		  d_wet_speed_dXj *= factor;
		  d_wet_vector_dX[a][b][j] = 
		    d_t_dXj[a][b]*wet_speed + 
		    t[a]*d_wet_speed_dXj;
		}
	    }
	}
...
void
apply_blake_wetting_velocity(	double func[MAX_PDIM],
				double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE], 
				const int bc_type,
				int id_side,
				int element_type,
  ...
  if ( lsi->near )
    {
      ...
      if ( af->Assemble_Jacobian ) load_lsi_derivs();
#endif /* COUPLED_FILL */
      if(bc_type == WETTING_SPEED_HOFFMAN_BC || 
	 bc_type == WETTING_SPEED_COX_BC)
	{
	  for ( a=0; a<VIM; a++) {
	    for ( b=0; b<VIM; b++) {
	      gamma[a][b] = fv->grad_v[a][b] + fv->grad_v[b][a];
      ...
      for( j=0; j<ei->dof[var]; j++)
	{
	  ...
	  for( a=0, d_wet_vector_dF[a][j] = 0.0; a<dim; a++)
	    d_wet_vector_dF[a][j] = d_t_dFj[a]*wet_speed*delta + t[a]*d_wet_speed_dFj*delta + t[a]*wet_speed*d_delta_dFj;
	}
#endif
		  ...
		  for(a=0; a<dim; a++)
		      ...
		      for( j=0; j < ei->dof[var] ; j++)
			{
			  switch (bc_type)
				...
				(liq_visc*surf_tens*
				 mp->d_surface_tension[var]*bf[var]->phi[j]
				 -g*d_mu->v[b][j]*visc_factor)/SQUARE(liq_visc);
			      break;
			    case WETTING_SPEED_COX_BC:
			      f_den = log(reciprocal_slip) + 
				(q_inner/f_sca - q_outer/f_dca);
		  ...
		  for( j=0; j < ei->dof[var] ; j++)
			...
			case WETTING_SPEED_HOFFMAN_BC:
			  d_wet_vector_dpj = (g_dca-g_sca)*
			    (liq_visc*surf_tens*
			     mp->d_surface_tension[var]*bf[var]->phi[j]
			     - g*d_mu->T[j]*visc_factor)/SQUARE(liq_visc);
			  break;
			case WETTING_SPEED_COX_BC:
			  f_den = log(reciprocal_slip) + 
			    (q_inner/f_sca - q_outer/f_dca);
		      ...
		      for (j=0; j<ei->dof[var]; j++)
			{
			  switch (bc_type)
				...
				*bf[var]->phi[j];
				...
				(liq_visc*surf_tens*
				 mp->d_surface_tension[MAX_VARIABLE_TYPES+w]*bf[var]->phi[j]
				 -g*d_mu->C[w][j]*visc_factor)/SQUARE(liq_visc);
			      break;
			    case WETTING_SPEED_COX_BC:
			      f_den = log(reciprocal_slip) + 
				(q_inner/f_sca - q_outer/f_dca);
...
void
apply_blake_wetting_velocity_sic( double func[MAX_PDIM],
                                  double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE], 
                                  const double dt,
                                  const double tt,
				  int bc_type,
                                  ...
                                  const double contact_fraction)
{
  ...
  beta = slip_coefficient( beta_inside, beta_outside, fv->F, wetting_length, d_beta_dF, gas_phase_factor, contact_fraction );
  ...
  if ( lsi->near )
      ...
      for( a=0; a<pd->Num_Dim; a++)
	{
      ...
      if(bc_type == HOFFMAN_DIRICHLET_BC || 
	 bc_type == COX_DIRICHLET_BC)
	{
	  for ( a=0; a<VIM; a++) {
	    for ( b=0; b<VIM; b++) 
 ...
 if ( near_ls )
   {
	 ...
	 sign = 1.0;
	 if(  ( normal_ls[0]*fv->snormal[1] - normal_ls[1]*fv->snormal[0] ) < 0.0 ) sign = -1.0;
...
void 
continuous_tangent_velocity(double func[DIM],
			    double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...
/*  initialize variables */
	...
	*func = vnormal;
        if(mode == EVAPORATION)
	  {
	    ...
	    for(w=0; w<pd->Num_Species_Eqn; w++) diffusion_flux[w] = 0.;
			   ...
			   if(mode == EVAPORATION)
			     {
			       for(w=0; w<pd->Num_Species_Eqn; w++)
				 {
				   d_diffusion_flux_dx[w][p][j] += fv->d_grad_c_dmesh[kdir][w][p][j]*fv->snormal[kdir]
			       ...
			       for(w=0; w<pd->Num_Species_Eqn; w++)
				 {
				   d_diffusion_flux_dx[w][p][j] -= fv->d_grad_c_dmesh[kdir][w][p][j]*fv->snormal[kdir]
                        ...
                        for(w=0; w<pd->Num_Species_Eqn; w++)
                           {
                          d_func[0][var][j] += mp->density*mp->diffusivity[w]*d_diffusion_flux_dx[w][p][j];
                           }
                         }
		     }
	         }
...
void 
fnormal_stress_bc(double func[DIM],
		  double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
	  ...
	  for( a=0; a<pd->Num_Dim; a++)
	    {
	      ...
	      for( j=0;  pd->v[0][var] &&  j<ei->dof[var]; j++)
		{
...
void 
qside_directional(double func[DIM],
		double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...
void 
q_velo_slip_bc(double func[MAX_PDIM],
	       double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
               int ibc,
	       double x[MAX_PDIM],
	       const double xsurf[MAX_PDIM], /* coordinates of surface Gauss  *
...
#define THREADED_SLIP FALSE
      ...
      radius = sqrt( ( fv->x[0] - X_0[0] )*( fv->x[0] - X_0[0] ) +
                     ( fv->x[1] - X_0[1] )*( fv->x[1] - X_0[1] ) );
      ...
      if (pd->v[0][MESH_DISPLACEMENT1] )
	{
	  for ( b=0; b<VIM; b++)
	      ...
	      for (j=0; j<ei->dof[var]; j++)
		{
		  for (p=0; p<VIM; p++)
		    {
		      d_func[0][var][j] += fv->snormal[p] * d_q->X[p][b][j]
		 			+ q[p] * fv->dsnormal_dx[p][b][j];
		    }
		}
	    }
	}
...
void 
potential_nobc_surf(double func[DIM],
 	  double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
 	  double time) 
...
void 
acoustic_plane_transmission(double func[DIM],
	  double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
	  double time,
	  const int bc_type, 
...
/***************************** EXECUTION BEGINS *******************************/
  ...
  if(bc_type == APR_PLANE_TRANS_BC)
  	{ var = ACOUS_PIMAG; conj_var = ACOUS_PREAL; }
  else
  	{ var = ACOUS_PREAL; conj_var = ACOUS_PIMAG; }
...
/*****************************************************************************/
void 
acoustic_nobc_surf(double func[DIM],
	  double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
	  double time,
	  const int bc_type) 
...
/***************************** EXECUTION BEGINS ******************************/
  ...
  if(bc_type == APR_NOBC_BC)
	{ ac_eqn = R_ACOUS_PREAL;	ac_var = ACOUS_PREAL;}
  else
	{ ac_eqn = R_ACOUS_PIMAG;	ac_var = ACOUS_PIMAG;}
...
{
  double T_scale,q_scale,T_boil,theta, vapor_c0 = 0.0,vapor_c1 = 0.0,vapor_c2 = 0.0,vapor_c3 = 0.0;
  double evap_loss;
...
double
calculate_laser_flux ( const double p[], 
                       double time, 
                       const double x[], 
                       double normal[], 
                       int sw_ale, 
...
/***************************** EXECUTION BEGINS *******************************/
  ...
  if(sw_trv_spt <0.1)
      ...
      while(ispot<=ispot_total-1)
	{
	  if(time >= (t_0+ispot*t_spot) && time < (t_deltpk+ispot*t_spot))
	    {
	      q_laserpow = base_qlaser+(qlaser0-base_qlaser)*(time-(t_0+ispot*t_spot))/(t_deltpk-t_0);
	    }
  ...
  else if(sw_trv_spt>0.9 && sw_trv_spt<1.1)
    { /* linear traveling weld */
      t_0=0.0;
	...
	{ q_laserpow = q_laserpow*(1.0+sin(2.0*3.14159*time/t_spot)); }
      ...
      if(q_laserpow < base_qlaser) { q_laserpow= base_qlaser; }
    }
  ...
  if(use_pth == 1 && sw_ale==1)
	...
	if(laser_z < 0)
	  { /* danger - assumes that simulation is x-y and laser moves in z for 2D */
	    center_x = sqrt(pow(laser_x_dir,2))*(laser_x);
	    center_y = sqrt(pow(laser_x_dir,2))*(laser_y);
	    center_z = sqrt(pow(laser_x_dir,2))*(laser_z+beam_w*(time-t_travrad0)+spot_space*rispot);
	    radial_pos = sqrt(0.0*pow(x0c-center_x,2) + pow(x1c-center_y,2) + pow(x2c-center_z,2))/radius_r;
	  }
	else
	  { /* standard x-y */
	    center_x = sqrt(pow(laser_y_dir,2))*(laser_x+beam_w*(time-t_travrad0)+spot_space*rispot);
	    center_y = sqrt(pow(laser_x_dir,2))*(laser_y+beam_w*(time-t_travrad0)+spot_space*rispot);
            center_z = 0.;
	...
	if(sw_th_exp > 0.5)
	  { /* DANGER Assuming Beam Variation in only x-y plane */
	    radial_pos = sqrt(pow(x0c-center_x,2) + pow(x1c-center_y,2) + 0.0*pow(x2c-center_z,2))/radius_r;
	  }
	else
	  {
	    radial_pos = sqrt(pow(x0c-center_x,2) + pow(x1c-center_y,2) + pow(x2c-center_z,2))/radius_r;
	  }
      }
	...
	if(sw_th_exp > 0.5)
	  { /* DANGER Assuming Beam Variation in-plane only */
	    if(sqrt(pow(laser_x_dir,2))>0.5)
	      { /* DANGER assume x normal with this switch */
		if(sw_vn > 0.5)
		  {
	    ...
	    if(sqrt(pow(laser_y_dir,2))>0.5)
	      { /* DANGER assume y normal with this switch */
		if(sw_vn > 0.5)
		  {
	    ...
	    if(sqrt(pow(laser_z_dir,2))>0.5)
	      { /* DANGER assume z normal with this switch */
		if(sw_vn > 0.5)
		  {
    ...
    dldz = 0.0;
    if(sqrt(pow(laser_x_dir,2))>0.5)
      { /* have x normal component of beam so varies in y and z */
	if(x1c-center_y > 0.0) { dldy=-1.0; }
	if(x1c-center_y < 0.0) { dldy= 1.0; }
	if(x2c-center_z > 0.0) { dldz=-1.0; }
	if(x2c-center_z < 0.0) { dldz= 1.0; }
      }
    if(sqrt(pow(laser_y_dir,2))>0.5)
      { /* have y normal component of beam so varies in x and z */
	if(x0c-center_x > 0.0) { dldx=-1.0; }
	if(x0c-center_x < 0.0) { dldx= 1.0; }
	if(x2c-center_z > 0.0) { dldz=-1.0; }
	if(x2c-center_z < 0.0) { dldz= 1.0; }
      }
    if(sqrt(pow(laser_z_dir,2))>0.5)
      { /* have z normal component of beam so varies in x and y */
	if(x0c-center_x > 0.0) { dldx=-1.0; }
	if(x0c-center_x < 0.0) { dldx= 1.0; }
	if(x1c-center_y > 0.0) { dldy=-1.0; }
	if(x1c-center_y < 0.0) { dldy= 1.0; }
...
/*****************************************************************************/
void 
qside_contact_resis(double func[DIM],
		    double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...
/****************************************************************************/
void 
qrad_surf_repulse(double func[DIM],
	  double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...
void
shear_to_shell ( double cfunc[MDE][DIM],
		double d_cfunc[MDE][DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		const int id_side,	/* ID of the side of the element */
		double scale,
		struct elem_side_bc_struct *elem_side_bc,
...
void
apply_hysteresis_wetting_sic (	double *func, 			
								double d_func[MAX_PDIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE], 
								double delta_t,
								double theta,
								double *float_data )
...
void 
fgamma1_deriv_bc(double func[DIM],
		 double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
	      ...
	      for (b = 0; b < dim; b++)
		{
		  ...
		  for (j = 0; j < ei->dof[var]; j++)
		      ...
		      for (i = 0; i < dofs; i++)
			{
			  for (p = 0; p < VIM; p++)
			    {
			      d_func[0][var][j] += fv->snormal[p] * (*esp->div_s_v[i]) * bf_ptr->d_grad_phi_dmesh[i][p][b][j];
...
void 
fgamma2_deriv_bc(double func[DIM],
		 double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		 const double normalDerivative)
			  ...
			  for (p = 0; p < dim; p++)
			    {
			      ...
			      d_func[0][var][j] += fv->snormal[p] * (*esp->curv[i]) * bf_ptr->d_grad_phi_dmesh[i][p][b][j];
...
void 
dvzdr_zero_deriv_bc(double func[DIM],
		    double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
	      ...
	      for (b = 0; b < dim; b++) 
		{
		  ...
		  for (j = 0; j < ei->dof[var]; j++)
		      ...
		      for (i = 0; i < dofs; i++)
			{
			  for (p = 0; p < VIM; p++)
			    {
			      d_func[0][var][j] += *esp->v[0][i] * bf_ptr->d_grad_phi_dmesh[i][p][b][j] * nwall[p];
			    }
			}
		    }
		}
...
void 
light_transmission(double func[DIM],
	  double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
	  double time,
	  const int bc_type, 
