...

{
   ...
   *      problem dimension.
   */
  if (! pd->e[0][eqn] || (ei->ielem_dim < pd->Num_Dim))
    {
      return(status);
    }
  ...
  if ( cr->MeshMotion == LAGRANGIAN ||
	...
	memset(d_vconv->X, 0, DIM*DIM*MDE*sizeof(dbl));
      ...
      if (pd->v[0][VELOCITY1] || pd->v[0][POR_LIQ_PRES])
	memset(d_vconv->v, 0, DIM*DIM*MDE*sizeof(dbl));

      if (pd->v[0][MASS_FRACTION] || pd->v[0][POR_LIQ_PRES])
  ...
  if ( af->Assemble_Residual )
      ...
      for ( a=0; a<dim; a++)
	{
	  ...
	  mass_on = pd->e[0][eqn] & T_MASS;
	  advection_on =  pd->e[0][eqn] & T_ADVECTION ;
	  diffusion_on = pd->e[0][eqn] & T_DIFFUSION;
	  source_on = pd->e[0][eqn] & T_SOURCE;
	  
	  mass_etm = pd->etm[0][eqn][(LOG2_MASS)];
	  advection_etm = pd->etm[0][eqn][(LOG2_ADVECTION)];
	  diffusion_etm = pd->etm[0][eqn][(LOG2_DIFFUSION)];
	  source_etm = pd->etm[0][eqn][(LOG2_SOURCE)];
		      ...
		      for ( w=0; w<pd->Num_Species_Eqn; w++)
			{
			  ...
			  if ((cr->MeshMotion == LAGRANGIAN ||
			      ...
			      for ( p=0; p<dim; p++)
				{
			      ...
			      advect_a *= pd->etm[0][eqn][LOG2_ADVECTION];
			      ...
			      for ( p=0; p<dim; p++)
				{
			      ...
			      advect_b *= pd->etm[0][eqn][LOG2_ADVECTION];
 ...
 */
 	      if ((cr->MeshMotion == LAGRANGIAN ||
		   cr->MeshMotion == DYNAMIC_LAGRANGIAN) &&  pd->e[0][eqn]  ) 
		{
		  var = TEMPERATURE;
		  if (pd->v[0][var])
		    {
	      ...
	      if ( pd->v[0][var] )
		{
		  ...
		  for ( j=0; j<ei->dof[var]; j++)
		      ...
		      if ( pd->e[0][eqn] & T_ADVECTION )
			{
			  ...
			  advection_a *= h3;
			  advection_a *= pd->etm[0][eqn][(LOG2_ADVECTION)];
			  if(supg!=0.)
			    {
			      h_elem_deriv = 0.;
			      if(hsquared[b] != 0.)
			  ...
			  if(supg!=0.)
			      ...
			      for(p=0;p<dim;p++)
				{
			      ...
			      advection_b *= h3;
			      advection_b *= pd->etm[0][eqn][(LOG2_ADVECTION)];
			    }
		  ...
		  const Exo_DB *exo)
{
  ...
  int mesh_disp_on = pd->v[0][MESH_DISPLACEMENT1];
  ...
  if (af->Assemble_Residual)
      ...
      for ( a=0; a<wim; a++)
	{
	  ...
	  advection_etm = pd->etm[0][eqn][(LOG2_ADVECTION)];
	  diffusion_etm = pd->etm[0][eqn][(LOG2_DIFFUSION)];
	  porous_brinkman_etm = pd->etm[0][eqn][(LOG2_POROUS_BRINK)];
	  source_etm = pd->etm[0][eqn][(LOG2_SOURCE)];
      ...
      for (a = 0; a < wim; a++)
	{
	  ...
	  for (i = 0; i < ei->dof[eqn]; i++) {		  
		...
		if ( pdv[var] )
	       ...
	       */
	      if (upd->vp[0][SOLID_DISPLACEMENT1]) {
		    for ( b=0; b<dim; b++)
		      {
			var = SOLID_DISPLACEMENT1+b;
			if ( pdv[var] )
		      ...
		      for ( w=0; w<pd->Num_Species_Eqn; w++)
			{
			  ...
			  if ( transient_run )
			      ...
			      if ( pd->e[0][eqn] & T_MASS )
				{
			      ...
			      if (pd->e[0][eqn] & T_POROUS_BRINK) /* if porous flow is considered. KSC on 5/10/95 */
				{
				  mass /= por;
				}
			    }
			  ...
			  if ( pd->e[0][eqn] & T_POROUS_BRINK )
			    {
			      ...
			      porous    *= pd->etm[0][eqn][(LOG2_POROUS_BRINK)];
		    ...
		    const PG_DATA *pg_data )
{
  ...
  if ((lagrangian_mesh_motion ||(total_ale_on && !pd->v[0][VELOCITY1]))
      ...
      for ( i=0; i<ei->dof[eqn]; i++)
	{
	  ...
	  if ( (hydromassflux_on) && ( suspensionsource_on))
	      ...
	      for( p=0; p<dim; p++)
		{
	      ...
	      h_flux *= h3 * det_J * wt;
	      h_flux *= pd->etm[0][eqn][(LOG2_ADVECTION)];
	      /*  h_flux = 0.0; */
	    } 
  ...
  if (af->Assemble_Jacobian)
	...
	{ 
		      ...
		      if (advection_on)
			{
			  if (pdv[VELOCITY1])
			      ...
			      if((hydromassflux_on) && ( suspensionsource_on ) )
				{
				  ...
				  h_flux *= ( rhos - rhof )/rhof * wt * pd->etm[0][eqn][(LOG2_ADVECTION)];	
			  ...
			  else if (lagrangian_mesh_motion || ( total_ale_on && !pd->v[0][VELOCITY1]) )
			    {
			      advection += fv->volume_change 
				* ( d_h3detJ_dmesh_bj ); 
		  ...
		  for ( j=0; j<ei->dof[var]; j++)
		    {
		      ...
		      if ( pd->e[0][eqn] & T_ADVECTION )
			{
			  if (cr->MeshMotion == TOTAL_ALE && !pd->v[0][VELOCITY1])
			    {
		  ...
		  for (w=0; w<pd->Num_Species_Eqn; w++) 
		      ...
		      if (pd->e[0][eqn] & T_SOURCE)
			{
			  ...
			  if (  mp->DensityModel == REACTIVE_FOAM || mp->DensityModel == DENSITY_FOAM )
			    {
			      source += dFVS_dC[w][j];
			      source *= phi_i * h3 * det_J * wt* pd->etm[0][eqn][LOG2_SOURCE];
			    }
	  ...
	  if ( cr->MassFluxModel == HYDRODYNAMIC &&  pd->v[0][var] )
	    {
		      ...
		      for( j=0; j<ei->dof[var]; j++)
			{
			  ...
			  if( pd->e[0][eqn] & T_ADVECTION )
			      ...
			      for( p=0; p<dim; p++ )
				{
			      ...
			      advection1 *= phi_i*wt;
			      advection1 *= pd->etm[0][eqn][(LOG2_ADVECTION)];
      ...
      ***************************************************************************/
{
  ...
  */
  ...
  for(p = 0; pdv[VORT_DIR1] && p < DIM; p++)
    {
      v = VORT_DIR1 + p;
      /*if (pd->v[0][v] || upd->vp[0][v] == -1) fv->vd[p] = 0.0; */
  ...
  /*  if (pd->v[0][v] || upd->vp[0][v] == -1) fv->vlambda = 0.0; */
  ...
  if (efv->ev)
      ...
      for ( w=0; w<efv->Num_external_field; w++)
	{
	   ...
	   * swallowed into the mesh
           */
          if (strcmp(efv->name[w], "DMX") == 0 && !pd->v[0][MESH_DISPLACEMENT1])
	    {
          ...
          if (strcmp(efv->name[w], "DMY") == 0 && !pd->v[0][MESH_DISPLACEMENT2])
	    {
	      fv->d[1] = fv->external_field[w];
	      fv_old->d[1] = fv_old->external_field[w];
	      fv_dot->d[1] = fv_dot->external_field[w];
	    }
          if (strcmp(efv->name[w], "DMZ") == 0 && !pd->v[0][MESH_DISPLACEMENT3])
	    {
      ...
      ***************************************************************************/
{
  ...
  */  
  ...
  else if (zero_unused_grads && upd->vp[0][TEMPERATURE] == -1 )
    {
      for (p=0; p<VIM; p++) fv->grad_T[p] = 0.0;
    }
  ...
  if ( pd->v[0][PRESSURE] )
    ...
    } else if ( zero_unused_grads &&  upd->vp[0][PRESSURE] == -1 ) {
  ...
  } else if ( zero_unused_grads &&  upd->vp[0][FILL] == -1 ) {
      for (p=0; p<VIM; p++)
	  {
		  fv->grad_F[p] = 0.0; fv_old->grad_F[p] = 0.0; fv_dot->grad_F[p] = 0.0;
	  }
  ...
  else if ( zero_unused_grads &&  upd->vp[0][CURVATURE] == -1 )
    {
      for (p=0; p<VIM; p++) 
	{
	  fv->grad_H[p] = 0.0;
  ...
  else  if ( zero_unused_grads &&  upd->vp[0][VOLTAGE] == -1 ) {
  ...
  else if (zero_unused_grads &&  upd->vp[0][SURF_CHARGE] == -1)
    {
      for (p = 0; p < VIM-1; p++)
	{
	  fv->grad_qs[p] = 0.0;
  ...
  } else  if (zero_unused_grads &&  upd->vp[0][SHEAR_RATE] == -1 )   {
  ...
  if (pd->v[0][MESH_DISPLACEMENT1] && !ShellElementParentElementCoverageForVariable[MESH_DISPLACEMENT1])
    {
    ...
    } else if (upd->vp[0][MESH_DISPLACEMENT1] == -1) 
      {
	for (p = 0; p < VIM; p++)
	  {
	    for (q = 0; q < VIM; q++)
  ...
  if( pd->v[0][SOLID_DISPLACEMENT1] )
    ...
    } else if ( zero_unused_grads &&  upd->vp[0][SOLID_DISPLACEMENT1] == -1 )
      {
  ...
  if (pd->v[0][MESH_DISPLACEMENT1] && !ShellElementParentElementCoverageForVariable[MESH_DISPLACEMENT1])
    ...
    } else if (zero_unused_grads && upd->vp[0][MESH_DISPLACEMENT1] == -1) {
      fv->div_d_dot = 0.0;
      fv->div_d     = 0.0;
    }
  ...
  else if (zero_unused_grads && upd->vp[0][VELOCITY1] == -1) 
    {
      for (p = 0; p < VIM; p++)
	{
	  for (q = 0; q < VIM; q++)
  ...
  if (pd->v[0][PVELOCITY1])
    ...
    } else if ( zero_unused_grads && upd->vp[0][PVELOCITY1] == -1 ) {
      for ( p=0; p<VIM; p++)
	{
	  for ( q=0; q<VIM; q++)
	    {
  ...
  } else if (zero_unused_grads &&  upd->vp[0][EXT_VELOCITY] == -1 ) {
      for ( p=0; p<VIM; p++)
	  {
		  fv->grad_ext_v[p] = 0.0;
	  }
   ...
   */
  if (pd->v[0][NORMAL1] || pd->v[0][SHELL_NORMAL1])
    {
      if (pd->v[0][NORMAL1]) v = NORMAL1;
      if (pd->v[0][SHELL_NORMAL1]) v = SHELL_NORMAL1;
      dofs = ei->dof[v];
      for (p = 0; p < VIM; p++)
	{
	  for (q = 0; q < VIM; q++)
  ...
  else if ( (zero_unused_grads && upd->vp[0][NORMAL1] == -1) || upd->vp[0][SHELL_NORMAL1] == -1)
    {
      for(p = 0; p < VIM; p++ )
	{
	  for (q = 0; q < VIM; q++ )
  ...
  if (pd->v[0][EFIELD1])
    ...
    } else if (zero_unused_grads && upd->vp[0][EFIELD1] != -1 )  {
      for ( p=0; p<VIM; p++)
	{
	  for ( q=0; q<VIM; q++)
	    {
  ...
  } else if (zero_unused_grads && upd->vp[0][MASS_FRACTION] == -1) {
	  	  for ( w=0; w<pd->Num_Species_Eqn; w++)
		  {			  
			  for ( p=0; p<VIM; p++ )
			  {
  ...
  } else if ( zero_unused_grads && upd->vp[0][POR_LIQ_PRES] == -1 ) {
      for ( p=0; p<VIM; p++ )
	  {
		  fv->grad_p_liq[p] = 0.0;
		  fv_old->grad_p_liq[p] = 0.0;
  ...
  } else if (zero_unused_grads &&  upd->vp[0][POR_GAS_PRES] == -1 ) {
	  for ( p=0; p<VIM; p++ )
	  {
		  fv->grad_p_gas[p] = 0.0;
		  fv_old->grad_p_gas[p] = 0.0;
  ...
  else if (zero_unused_grads &&  upd->vp[0][POR_POROSITY] == -1) 
    {
      for (p = 0; p < VIM; p++ )
	{
	  fv->grad_porosity[p] = 0.0;
  ...
  } else if ( zero_unused_grads && upd->vp[0][POLYMER_STRESS11] == -1 )  {  
      for ( mode=0; mode<vn->modes; mode++)
	  {
		  for ( p=0; p<VIM; p++)
		  {
  ...
  } else if ( zero_unused_grads &&  upd->vp[0][VELOCITY_GRADIENT11] == -1 ) {  
      for ( p=0; p<VIM; p++)
	  {
          fv->div_G[p] = 0.0;
          fv->div_Gt[p] = 0.0;
  ...
  if (pd->v[0][VORT_DIR1])
    ...
    } else if (zero_unused_grads &&  upd->vp[0][VORT_DIR1] == -1 ) {
      for (p=0; p<DIM; p++)
	{
          for (q=0; q<DIM; q++)
	    {
  ...
  if (pd->v[0][N_DOT_CURL_V])
    ...
    } else if (zero_unused_grads &&  upd->vp[0][N_DOT_CURL_V] == -1 ) {
      for (p = 0; p < VIM; p++) {
	fv->grad_n_dot_curl_s_v[p] = 0.0;
      }
    }
  ...
  if (pd->v[0][SHELL_SURF_DIV_V])
    ...
    } else if (zero_unused_grads &&  upd->vp[0][SHELL_SURF_DIV_V] == -1 ) {
      for (p = 0; p < VIM; p++) {
	fv->grad_div_s_v[p] = 0.0;
      }
    }
 ...
 /*
  ...
  if (pd->v[0][SHELL_SURF_CURV])
    ...
    } else if (zero_unused_grads &&  upd->vp[0][SHELL_SURF_CURV] == -1) {
      for (p = 0; p < VIM; p++) {
	fv->grad_curv[p] = 0.0;
      }
    }
  ...
  else  if (zero_unused_grads &&  upd->vp[0][GRAD_S_V_DOT_N1] == -1 ) 
    {
      for (r = 0; r < dim; r++) 
	{
	  for (p = 0; p < VIM; p++) {
  ...
  else if ( zero_unused_grads &&  upd->vp[0][SHELL_DIFF_FLUX] == -1 ){
  ...
  if ( pd->v[0][ACOUS_PREAL] )
    ...
    } else if ( zero_unused_grads && upd->vp[0][ACOUS_PREAL] == -1 ) {
  ...
  if ( pd->v[0][ACOUS_PIMAG] )
    ...
    } else if ( zero_unused_grads && upd->vp[0][ACOUS_PIMAG] == -1 ){
  ...
  if ( pd->v[0][ACOUS_REYN_STRESS] )
    ...
    } else if ( zero_unused_grads && upd->vp[0][ACOUS_REYN_STRESS] == -1 ) {
    for (p=0; p<VIM; p++) fv->grad_ars[p] = 0.0;
  }
  
  if ( pd->v[0][SHELL_BDYVELO] )
    {
      v = SHELL_BDYVELO;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][SHELL_BDYVELO] == -1 ) {
    for (p=0; p<VIM; p++) fv->grad_sh_bv[p] = 0.0;
  }
  
  if ( pd->v[0][SHELL_LUBP] )
    {
      v = SHELL_LUBP;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][SHELL_LUBP] == -1 ) {
  ...
  else if ( zero_unused_grads && upd->vp[0][LUBP] == -1 ) 
    {
  ...
  else if ( zero_unused_grads && upd->vp[0][LUBP_2] == -1 ) 
    {
      for (p=0; p<VIM; p++) fv->grad_lubp_2[p] = 0.0;
    }
  
  if ( pd->v[0][SHELL_LUB_CURV] )
    {
      v = SHELL_LUB_CURV;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][SHELL_LUB_CURV] == -1 ) {
    for (p=0; p<VIM; p++) fv->grad_sh_l_curv[p] = 0.0;
  }

   if ( pd->v[0][SHELL_LUB_CURV_2] )
    {
      v = SHELL_LUB_CURV_2;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][SHELL_LUB_CURV_2] == -1 ) {
  ...
  else if ( zero_unused_grads && upd->vp[0][SHELL_PRESS_OPEN] == -1 ) 
    {
  ...
  else if ( zero_unused_grads && upd->vp[0][SHELL_PRESS_OPEN_2] == -1 ) 
    {
      for (p=0; p<VIM; p++) fv->grad_sh_p_open_2[p] = 0.0;
    }
  
  
  if ( pd->v[0][SHELL_FILMP] )
    {
      v = SHELL_FILMP;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][SHELL_FILMP] == -1 ) {
  ...
  if ( pd->v[0][SHELL_FILMH] )
    ...
    } else if ( zero_unused_grads && upd->vp[0][SHELL_FILMH] == -1 ) {
  ...
  if ( pd->v[0][SHELL_PARTC] )
    ...
    } else if ( zero_unused_grads && upd->vp[0][SHELL_PARTC] == -1 ) {
    for (p=0; p<VIM; p++) fv->grad_sh_pc[p] = 0.0;
  } 

  if ( pd->v[0][SHELL_TEMPERATURE] )
    {
      v = SHELL_TEMPERATURE;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][SHELL_TEMPERATURE] == -1 ) {
    for (p=0; p<VIM; p++) fv->grad_sh_t[p] = 0.0;
  } 

  if ( pd->v[0][LIGHT_INTP] )
    {
      v = LIGHT_INTP;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][LIGHT_INTP] == -1 ) {
    for (p=0; p<VIM; p++) fv->grad_poynt[0][p] = 0.0;
  } 

  if ( pd->v[0][LIGHT_INTM] )
    {
      v = LIGHT_INTM;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][LIGHT_INTM] == -1 ) {
    for (p=0; p<VIM; p++) fv->grad_poynt[1][p] = 0.0;
  } 


  if ( pd->v[0][LIGHT_INTD] )
    {
      v = LIGHT_INTD;
      dofs  = ei->dof[v];
      for ( p=0; p<VIM; p++)
    ...
    } else if ( zero_unused_grads && upd->vp[0][LIGHT_INTD] == -1 ) {
...
#ifdef DEBUG_HKM
  if ((pd->v[0][SHELL_NORMAL1] == 0) && ei->dof[SHELL_NORMAL1] > 0) {
    doExtra = 1;
  }
#endif
...
#endif
    } else   if (upd->vp[0][TEMPERATURE] != -1 && !is_initialized && okToZero) {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_T_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][VOLTAGE] != -1  && !is_initialized  && okToZero) {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_V_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else if ( upd->vp[0][ACOUS_PREAL] != -1   && okToZero)  {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_apr_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][ACOUS_PIMAG] != -1  && okToZero) {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_api_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][ACOUS_REYN_STRESS] != -1   && okToZero){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_ars_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][SHELL_BDYVELO] != -1  && okToZero ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_bv_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][LUBP] != -1  && okToZero ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_lubp_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][LUBP_2] != -1  && okToZero ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_lubp_2_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][SHELL_PRESS_OPEN] != -1  && okToZero ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_p_open_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][SHELL_PRESS_OPEN_2] != -1  && okToZero ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_p_open_2_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][SHELL_FILMP] != -1  ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_fp_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][SHELL_FILMH] != -1  ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_fh_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][SHELL_PARTC] != -1  ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_pc_dmesh[0][0][0]),0, siz);
    } 
...
#endif
    } else   if ( upd->vp[0][SHELL_TEMPERATURE] != -1  ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_t_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if ( upd->vp[0][SHELL_LUB_CURV] != -1  ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_l_curv_dmesh[0][0][0]),0, siz);
    }
...
if ( pd->v[0][SHELL_LUB_CURV_2] )
...
#endif
    } else   if ( upd->vp[0][SHELL_LUB_CURV_2] != -1  ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_l_curv_2_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else  if ( upd->vp[0][SURF_CHARGE] != -1  && okToZero ) {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_qs_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else  if ( upd->vp[0][SHELL_DIFF_FLUX] != -1  && okToZero ) {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(&(fv->d_grad_sh_J_dmesh[0][0][0]),0, siz);
    }
...
#endif
    } else   if (upd->vp[0][SHEAR_RATE] != -1  && okToZero ){
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(fv->d_grad_SH_dmesh,0, siz);
    }
...
#endif	  
    } else   if ( upd->vp[0][PRESSURE] != -1  && okToZero )
      {
	siz = sizeof(double)*DIM*DIM*MDE;
	memset(fv->d_grad_P_dmesh,0, siz);
      }
...
#endif
    } else if (upd->vp[0][MASS_FRACTION] != -1  && okToZero )
      {
	siz = sizeof(double)*DIM*DIM*MAX_CONC*MDE;
	memset(fv->d_grad_c_dmesh,0, siz);
      }  
  ...
  if (pd->v[0][POR_LIQ_PRES])
    ...
    } else if ( upd->vp[0][POR_LIQ_PRES] != -1  && okToZero ) 
      {
	siz = sizeof(double)*DIM*DIM*MDE;
	memset(fv->d_grad_p_liq_dmesh,0, siz);
      }
...
#endif
    } else if (upd->vp[0][POR_GAS_PRES] != -1  && okToZero )
      {
	siz = sizeof(double)*DIM*DIM*MDE;
	memset(fv->d_grad_p_gas_dmesh,0, siz);
      }
...
#endif
    } else if ( upd->vp[0][POR_POROSITY] != -1 && okToZero) {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(fv->d_grad_porosity_dmesh,0, siz);
    }
  ...
  if (pd->v[0][VELOCITY1])
    ...
    } else if (upd->vp[0][VELOCITY1] != -1 && okToZero) 
      {
  ...
  if ( pd->v[0][PVELOCITY1] )
    ...
    } else   if ( upd->vp[0][PVELOCITY1] != -1  && okToZero ) {
      siz = sizeof(double)*DIM*DIM*DIM*MDE;
      memset(fv->d_grad_pv_dmesh, 0, siz);
    }
  ...
  if ( pd->v[0][EXT_VELOCITY] )
    ...
    } else if ( upd->vp[0][EXT_VELOCITY] != -1 && okToZero ) {
      siz = sizeof(double) * DIM * DIM * MDE;
      memset(fv->d_grad_ext_v_dmesh, 0, siz);
    }
  ...
  else if (((upd->vp[0][NORMAL1] != -1) || (upd->vp[0][SHELL_NORMAL1] != -1)) && (okToZero))
    {
      siz = sizeof(double)*DIM*VIM*DIM*MDE;
      memset(fv->d_grad_n_dmesh,0, siz);
      siz = sizeof(double)*DIM*MDE;
  ...
  if (pd->v[0][EFIELD1])
    ...
    } else if (upd->vp[0][EFIELD1] != -1 && okToZero )
      {
	siz = sizeof(double)*DIM*VIM*DIM*MDE;
	memset(fv->d_grad_E_field_dmesh,0, siz);
      }
  ...
  if (pd->v[0][MESH_DISPLACEMENT1] == 1)
    {
      v = MESH_DISPLACEMENT1;
      bfv = bf[v];
      vdofs     = ei->dof[v];
    ...
    } else   if ( upd->vp[0][MESH_DISPLACEMENT1] != -1 && okToZero )
      {
	siz = sizeof(double)*DIM*DIM*DIM*MDE;
	memset(fv->d_grad_d_dmesh,0,siz);
	memset(fv->d_grad_d_dot_dmesh,0,siz);
  ...
  if ( pd->v[0][SOLID_DISPLACEMENT1] )
    ...
    } else   if ( upd->vp[0][SOLID_DISPLACEMENT1] != -1  && okToZero)
      {
	siz = sizeof(double)*DIM*DIM*DIM*MDE;
	memset(fv->d_grad_d_rs_dmesh,0,siz);
	siz = sizeof(double)*DIM*MDE;
  ...
  if ( pd->v[0][POLYMER_STRESS11] )
    ...
    } else if ( upd->vp[0][POLYMER_STRESS11] != -1  && okToZero) {
      siz = sizeof(double)*MAX_MODES*DIM*DIM*DIM*DIM*MDE;
      memset(fv->d_grad_S_dmesh,0,siz);
      siz = sizeof(double)*MAX_MODES*DIM*DIM*MDE;
      memset(fv->d_div_S_dmesh,0,siz);
  ...
  if ( pd->v[0][VELOCITY_GRADIENT11] )
    ...
    } else   if ( upd->vp[0][VELOCITY_GRADIENT11] != -1  && okToZero)  {
      siz = sizeof(double)*DIM*DIM*DIM*DIM*MDE;
      memset(fv->d_grad_G_dmesh,0,siz);
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(fv->d_div_G_dmesh,0,siz);
  ...
  if ( pd->v[0][VORT_DIR1] )
    ...
    } else if ( upd->vp[0][VORT_DIR1] != -1  && okToZero) {
      siz = sizeof(double)*DIM*DIM*DIM*MDE;
      memset(fv->d_grad_vd_dmesh,0, siz);
    }
  ...
  else if (upd->vp[0][N_DOT_CURL_V] != -1 && okToZero) 
    {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(fv->d_grad_n_dot_curl_s_v_dmesh, 0, siz);
    }
  ...
  else if (upd->vp[0][SHELL_SURF_DIV_V] != -1 && okToZero) 
    {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(fv->d_grad_div_s_v_dmesh, 0, siz);
    }
  ...
  else if (upd->vp[0][SHELL_SURF_CURV] != -1 && okToZero) 
    {
      siz = sizeof(double)*DIM*DIM*MDE;
      memset(fv->d_grad_curv_dmesh, 0, siz);
    }
  ...
  else if (upd->vp[0][GRAD_S_V_DOT_N1] != -1 && okToZero) 
    {
      siz = sizeof(double)*DIM*DIM*DIM*MDE;
      memset(fv->d_serialgrad_grad_s_v_dot_n_dmesh, 0, siz);
    }
      ...
      ***************************************************************************/
{
  ...
  if (mp->DensityModel == CONSTANT) {
    ...
    */
    ...
    if (pd->v[0][MASS_FRACTION] && d_rho != NULL)
      {
	var = MASS_FRACTION;
	for ( j=0; j<ei->dof[var]; j++)
	  {
     ...
     */
    if ( d_rho != NULL && pd->v[0][TEMPERATURE] ) {
     ...
     */
    if ( d_rho != NULL && pd->v[0][MASS_FRACTION] ) {
      phi_ptr =  bf[MASS_FRACTION]->phi;
      switch (matrl_species_var_type) {
      case SPECIES_MASS_FRACTION:
  ...
  if( ls != NULL && mp->mp2nd != NULL &&
      ...
      if ( d_rho == NULL )
	{
	  ...
	  if(pfd != NULL && pd->e[0][R_EXT_VELOCITY])
	    {
	      ls_old = ls;
	      ls = pfd->ls[0];
  ...
  if(mp->ConductivityModel == USER )
    {
      ...
      var = TEMPERATURE;
      if( pd->v[0][TEMPERATURE] && d_k != NULL )
	{
      ...
      if (pd->v[0][MESH_DISPLACEMENT1] && d_k != NULL )
	{
	  for ( a=0; a<dim; a++)
	    {
	      var = MESH_DISPLACEMENT1 + a;
      ...
      if (pd->v[0][MASS_FRACTION] && d_k != NULL)
	{
	  for ( w=0; w<pd->Num_Species_Eqn; w++)
	    {
	      var = MASS_FRACTION;
  ...
  if(mp->HeatCapacityModel == USER )
    {
      ...
      if ( d_Cp != NULL && pd->v[0][MESH_DISPLACEMENT1] )
	{
	  for ( a=0; a<dim; a++)
	    {
	      var = MESH_DISPLACEMENT1 + a;
      ...
      if ( d_Cp != NULL && pd->v[0][FILL] )
	{
      ...
      if ( d_Cp != NULL && pd->v[0][MASS_FRACTION] )
	{
	  for ( w=0; w<pd->Num_Species_Eqn; w++)
	    {
	      var = MASS_FRACTION;
			...
			CONDUCTIVITY_DEPENDENCE_STRUCT *d_k )
{
  ...
  if( pd->v[0][var=MESH_DISPLACEMENT1] )
    {
...

{
  ...
  if(mp->Acoustic_ImpedanceModel == USER )
	...
	*/
      ...
      var = TEMPERATURE;
      if( pd->v[0][TEMPERATURE] && d_R != NULL )
	{
      ...
      if (pd->v[0][MESH_DISPLACEMENT1] && d_R != NULL )
	{
	  for ( a=0; a<dim; a++)
	    {
	      var = MESH_DISPLACEMENT1 + a;
      ...
      if (pd->v[0][MASS_FRACTION] && d_R != NULL)
	{
	  for ( w=0; w<pd->Num_Species_Eqn; w++)
	    {
	      var = MASS_FRACTION;
  ...
  if(mp->Acoustic_AbsorptionModel == USER )
    {
      ...
      var = TEMPERATURE;
      if( pd->v[0][TEMPERATURE] && d_alpha != NULL )
	{
      ...
      if (pd->v[0][MESH_DISPLACEMENT1] && d_alpha != NULL )
	{
	  for ( a=0; a<dim; a++)
	    {
	      var = MESH_DISPLACEMENT1 + a;
      ...
      if (pd->v[0][MASS_FRACTION] && d_alpha != NULL)
	{
	  for ( w=0; w<pd->Num_Species_Eqn; w++)
	    {
	      var = MASS_FRACTION;
  ...
  if(mp->Refractive_IndexModel == USER )
    {
      ...
      var = TEMPERATURE;
      if( pd->v[0][TEMPERATURE] && d_n != NULL )
	{
      ...
      if (pd->v[0][MESH_DISPLACEMENT1] && d_n != NULL )
	{
	  for ( a=0; a<dim; a++)
	    {
	      var = MESH_DISPLACEMENT1 + a;
      ...
      if (pd->v[0][MASS_FRACTION] && d_n != NULL)
	{
	  for ( w=0; w<pd->Num_Species_Eqn; w++)
	    {
	      var = MASS_FRACTION;
		     ...
		     dbl time)
{
  ...
  if(mp->MomentumSourceModel == USER)
...
#endif /* COUPLED_FILL */
	       if ( df != NULL && pd->v[0][MESH_DISPLACEMENT1] )
		{
		  for ( b=0; b<dim; b++)
		    {
		      var = MESH_DISPLACEMENT1+b;
	      ...
	      if ( df != NULL && pd->v[0][MASS_FRACTION] )
		{
		  for ( w=0; w<pd->Num_Species_Eqn; w++)
		    {
		      var = MASS_FRACTION;
  ...
  else if (mp->MomentumSourceModel == LEVEL_SET  && pd->v[0][FILL]  )
    {
      DENSITY_DEPENDENCE_STRUCT d_rho_struct;  /* density dependence */
      DENSITY_DEPENDENCE_STRUCT *d_rho = &d_rho_struct;
      double rho = density( d_rho , time);
	...
	else if ( mp->MomentumSourceModel == LEVEL_SET  && pd->v[0][PHASE1]  )
	{
      DENSITY_DEPENDENCE_STRUCT d_rho_struct;  /* density dependence */
      DENSITY_DEPENDENCE_STRUCT *d_rho = &d_rho_struct;
      double rho = density( d_rho, time );
...
/*  Acoustic Reynolds Stress piece	*/
			f[a] += mp->u_momentum_source[0]*fv->grad_ars[a];
 	      	...
 	      	if ( df != NULL && pd->v[0][ACOUS_REYN_STRESS] )
 			{
      ...
      */
{
  ...
  if(pd->v[0][MESH_DISPLACEMENT1] && af->Assemble_Jacobian)
    {
      npass = 2;
                            ...
                            double **cAC )
{
  ...
  if( pd->e[0][R_PHASE1] && !pd->e[0][R_FILL] ) 
	{
	  ls = pfd->ls[0];
	}
      ...
      for ( ipass = 0; ipass < num_elem_passes; ipass++ )
        {
		  ...
		  if( pd->e[0][R_NORMAL1] ) assemble_curvature_with_normals_source () ;
			  ...
			  for(j=0; j<ei->dof[var]; j++)
			    {
			      ...
			      source1 *= pd->etm[0][eqn][(LOG2_SOURCE)];
...
assemble_cont_t_source ( double *xi )
{
   ...
   */
  eqn = R_ENERGY;
  if ( pd->i[0][eqn] == I_Q1_G || pd->i[0][eqn] == I_Q2_G )
...
assemble_p_source ( double pressure, const int bcflag )
{
  ...
  else if( bcflag == 1 && pd->e[0][R_MESH1] && cr->MeshMotion != ARBITRARY)
     {
  /* initialize some arrays */
  memset( TT, 0, sizeof(double)*DIM*DIM);
  if (af->Assemble_Jacobian)
...
/* coefficient variables for the Brinkman Equation
  ...
  if (  transient_run &&  pd->v[0][MESH_DISPLACEMENT1] )
    x_dot = fv_dot->x;
  else
    x_dot = zero;
				     ...
				     const PG_DATA *pg_data)
{
   ...
   */
	...
	if ((cr->MeshMotion == LAGRANGIAN ||
		 cr->MeshMotion == DYNAMIC_LAGRANGIAN) && pd->e[0][R_MESH1])
    {
		err = belly_flop(elc->lame_mu);
		EH(err, "error in belly flop");
		if (err == 2) return(err);
	...
	if ((cr->MeshMotion == TOTAL_ALE && !pd->v[0][VELOCITY1]) && pd->e[0][R_SOLID1])
    {
		err = belly_flop_rs(elc_rs->lame_mu);
		EH(err, "error in belly flop for real solid");
		if (err == 2) return(err);
	...
	if ((cr->MeshMotion == LAGRANGIAN ||
		 cr->MeshMotion == DYNAMIC_LAGRANGIAN ||
		 (cr->MeshMotion == TOTAL_ALE && !pd->v[0][VELOCITY1]))
		&& (mp->PorousMediaType == CONTINUOUS))
    {
		initial_volsolvent = elc->Strss_fr_sol_vol_frac;
		volsolvent = 0.;
	...
	if ( af->Assemble_Jacobian )
	  {
		for ( i=0; i<ei->dof[eqn]; i++)
        {
			...
			*/
			...
			if ( pd->e[0][eqn] & T_ADVECTION )
			{
				if (pd->v[0][VELOCITY1]) /* then must be solving fluid mechanics in this material */
				{
...
/*   if( cr->MassFluxModel == DM_SUSPENSION_BALANCE|| mp->QTensorDiffType[w0] == CONSTANT) */
  ...
  // Mesh Dependence
  if ( d_Pi != NULL && pd->v[0][MESH_DISPLACEMENT1] )
    {
      for ( p=0; p<VIM; p++)
        {
          for ( q=0; q<VIM; q++)
  ...
  if ( d_Pi != NULL && pd->v[0][POLYMER_STRESS11] )
    {
      for ( mode=0; mode<vn->modes; mode++)
        {
          for ( p=0; p<VIM; p++)
  ...
  if ( d_Pi != NULL && pd->v[0][VELOCITY_GRADIENT11] )
    {
      for ( p=0; p<VIM; p++)
        {
          for ( q=0; q<VIM; q++)
           ...
           double time )
{
  ...
  if ( cr->HeatFluxModel == CR_HF_FOURIER_0 )
      ...
      for ( p=0; p<VIM; p++)
	{
      ...
      if (d_q != NULL && pd->v[0][MESH_DISPLACEMENT1])
        {
          for (p=0; p<VIM; p++)
	    {
	      for (b=0; b<wim; b++)
		    ...
		    double dt )	/* current time step size */
{
  ...
  if(mp->HeatSourceModel == USER )
      ...
      if( d_h != NULL && pd->v[0][VELOCITY1])
	{
      ...
      if( d_h != NULL && pd->v[0][MESH_DISPLACEMENT1] )
	{
	  for ( a=0; a<wim; a++)
	    {
	      var = MESH_DISPLACEMENT1 + a;
      ...
      if ( d_h != NULL && pd->v[0][MASS_FRACTION] )
	{
	  for ( w=0; w<pd->Num_Species_Eqn; w++)
	    {
	      var = MASS_FRACTION;
...
#if 1
       intensity = 0;
       if(pd->e[0][R_LIGHT_INTP])
          { intensity += fv->poynt[0];}
       if(pd->e[0][R_LIGHT_INTM])
          { intensity += fv->poynt[1];}
       if(pd->e[0][R_LIGHT_INTD])
		       ...
		       HEAT_SOURCE_DEPENDENCE_STRUCT *df )
{
  ...
  if ( df == NULL )
      ...
      if(  pd->v[0][var=VOLTAGE] )
	{
      ...
      if(  pd->v[0][var=POLYMER_STRESS11] )
	{
	   ...
	   const PG_DATA *pg_data )
{
  ...
  if(pspg_global)
      ...
      if (  pd->TimeIntegration != STEADY)
	{
      ...
      if ( d_pspg != NULL && pd->v[0][VELOCITY1] )
	{
	  for ( b=0; b<dim; b++)
	    {
	      var = VELOCITY1+b;
      ...
      if ( d_pspg != NULL && pd->v[0][MESH_DISPLACEMENT1] )
	{
	  for ( b=0; b<dim; b++)
	    {
	      var = MESH_DISPLACEMENT1+b;
  ...
  for ( a=0; a<wim; a++)
    {
      if (  pd->TimeIntegration != STEADY &&  pd->v[0][MESH_DISPLACEMENT1+a] )
	{
	  x_dot[a] = fv_dot->x[a];
	}
      else
  ...
  if ( pd->v[0][VELOCITY_GRADIENT11] &&  pd->v[0][POLYMER_STRESS11])
    {
      for ( p=0; p<wim; p++)
	{
	  div_G[p] = fv->div_G[p];
  ...
  if (pd->e[0][R_MOMENTUM1] & T_POROUS_BRINK )
    {
      if (mp->PorousMediaType != POROUS_BRINKMAN)
	WH(-1, "Set Porous term multiplier in continuous medium");
  ...
  for ( a=0; a<wim; a++)
  {
      ...
      mass = 0.;
      if ( ( pd->e[0][meqn] & T_MASS ) && ( pd->TimeIntegration != STEADY ) )
	  {
		  mass =  rho_t * v_dot[a] / por;
		  mass *= pd->etm[0][meqn][(LOG2_MASS)];
	  }
      ...
      if ( pd->e[0][meqn] & T_ADVECTION )
	  {
		  ...
		  advection *= pd->etm[0][meqn][(LOG2_ADVECTION)];
	  }
      ...
      source = 0.;
      if ( pd->e[0][meqn] & LOG2_SOURCE )
	  {
		  source = -f[a];
		  source *= pd->etm[0][meqn][(LOG2_SOURCE)];
	  }
	  
      porous = 0.;
      if ( pd->e[0][meqn] & LOG2_POROUS_BRINK )
	  {
		  porous = v[a]*(rho_t*sc*speed/sqrt(per)+vis/per);
		  porous *= pd->etm[0][meqn][(LOG2_POROUS_BRINK)];
	  }
      ...
      if ( d_pspg != NULL && pd->v[0][VELOCITY1] )
	  {
		  for ( b=0; b<wim; b++ )
		  {
			  ...
			  for ( j=0; j<ei->dof[var]; j++ )
			  {
				  ...
				  {
					  source -= df->v[a][b][j] * pd->etm[0][meqn][(LOG2_SOURCE)];
				  }
				  
				  porous = 0.;
				  if ( pd->e[0][meqn] & T_POROUS_BRINK )
				  {
      ...
      if ( d_pspg != NULL && pd->v[0][MESH_DISPLACEMENT1] )
	  {
		  for ( b=0; b<wim; b++ )
		  {
			  ...
			  for ( j=0; j<ei->dof[var]; j++ )
			  {
				  ...
				  advection = 0.;
				  if  ( ( pd->e[0][meqn] & T_ADVECTION ) && ( pd->TimeIntegration != STEADY ) )
				  {
				  ...
				  if ( pd->e[0][meqn] & T_SOURCE )
				  {
					  source -= df->X[a][b][j] * pd->etm[0][meqn][(LOG2_SOURCE)];
				  }
      ...
      if ( d_pspg != NULL && pd->v[0][var] )
	  {
		  ...
		  for ( j=0; j<ei->dof[var]; j++ )
		  {
			  ...
			  if ( pd->e[0][meqn] & T_SOURCE )
			  {
				  source -= df->T[a][j] * pd->etm[0][meqn][(LOG2_SOURCE)];
			  }
      ...
      if ( d_pspg != NULL && pd->v[0][var] )
	  {
		  ...
		  for (w=0; w<pd->Num_Species_Eqn; w++)
		  {
			  for ( j=0; j<ei->dof[var]; j++ )
			  {
				  ...
				  if ( pd->e[0][meqn] & T_SOURCE )
				  {
					  source -= df->C[a][w][j] * pd->etm[0][meqn][(LOG2_SOURCE)];
				  }
...
assemble_max_strain ()   
{
  ...
  // FEM weights
  dbl dA = det_J * wt * h3;
  dbl etm_mass = pd->etm[0][eqn][(LOG2_MASS)];
  dbl etm_source = pd->etm[0][eqn][(LOG2_SOURCE)];
