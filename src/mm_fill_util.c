...
#ifdef USE_RCSID
static char rcsid[] = "$Id: mm_fill_util.c,v 5.24 2010-02-26 21:40:14 prschun Exp $";
#endif
...
beer_belly(void)
{
  ...
  /*
   * Now copy mapping Jacobians from the MapBf basis function to the other Basis functions
   *
   */
  ...
  switch ( dim )
      ...
      if (DeformingMesh)
	{
	  for (j = 0; j < pdim; j++)
	      ...
	      for (k = 0; k < mdof; k++)
		{
		  ...
		  MapBf->d_det_J_dm[j][k] = bf[R_MESH1]->dphidxi[k][0]
		    * MapBf->J[0][j] / MapBf->detJ;
		  if (fabs(MapBf->J[0][j]) > DBL_SMALL)
		    {
		       MapBf->dB[j][0] [j][k] = -bf[R_MESH1]->dphidxi[k][0]
      ...
      if (DeformingMesh)
	{
	  ...
	  for (k = 0; k < pdim; k++)
	      ...
	      for (n = 0; n < mdof; n++)
		{
		  ...
		  MapBf->dB[0][0][k][n] = MapBf->dJ[1][1][k][n] * f 
		    + MapBf-> J[1][1]       * g;

		  MapBf->dB[0][1][k][n] =-MapBf->dJ[0][1][k][n] * f 
		    - MapBf-> J[0][1]       * g;

		  MapBf->dB[1][0][k][n] =-MapBf->dJ[1][0][k][n] * f 
		    - MapBf->J[1][0]        * g;

		  MapBf->dB[1][1][k][n] = MapBf->dJ[0][0][k][n] * f 
		    + MapBf-> J[0][0]       * g;
		} 
	    }
	} 
	...
	+ MapBf->J[0][2] * ( MapBf->J[1][0] * MapBf->J[2][1]
      ...
      MapBf->B[0][0] = ( MapBf->J[1][1] * MapBf->J[2][2]
			 -MapBf->J[2][1] * MapBf->J[1][2] )
	/(MapBf->detJ);

      MapBf->B[0][1] =-( MapBf->J[0][1] * MapBf->J[2][2]
			 -MapBf->J[2][1] * MapBf->J[0][2])
	/(MapBf->detJ);

      MapBf->B[0][2] = ( MapBf->J[0][1] * MapBf->J[1][2]
			 -MapBf->J[1][1] * MapBf->J[0][2])
	/(MapBf->detJ);

      MapBf->B[1][0] =-( MapBf->J[1][0] * MapBf->J[2][2]
			 -MapBf->J[2][0] * MapBf->J[1][2])
	/(MapBf->detJ);

      MapBf->B[1][1] = ( MapBf->J[0][0] * MapBf->J[2][2]
			 -MapBf->J[2][0] * MapBf->J[0][2])
	/(MapBf->detJ);

      MapBf->B[1][2] =-( MapBf->J[0][0] * MapBf->J[1][2]
			 -MapBf->J[1][0] * MapBf->J[0][2])
	/(MapBf->detJ);

      MapBf->B[2][0] = ( MapBf->J[1][0] * MapBf->J[2][1]
			 -MapBf->J[1][1] * MapBf->J[2][0])
	/(MapBf->detJ);

      MapBf->B[2][1] =-( MapBf->J[0][0] * MapBf->J[2][1]
			 -MapBf->J[2][0] * MapBf->J[0][1])
	/(MapBf->detJ);

      MapBf->B[2][2] = ( MapBf->J[0][0] * MapBf->J[1][1]
	  ...
	  for (k = 0; k < pdim; k++)
	      ...
	      for (n = 0; n < mdof; n++)
		{
		  ...
		  MapBf->dB[0][2][k][n] = ( MapBf->dJ[0][1][k][n]* MapBf->J[1][2]
					    ...
					    - MapBf->J[1][1]      *MapBf->dJ[0][2][k][n])*f
		    +
		  ...
		  MapBf->dB[1][0][k][n] =-( MapBf->dJ[1][0][k][n]* MapBf->J[2][2]
					    + MapBf->J[1][0]      *MapBf->dJ[2][2][k][n]
					    -MapBf->dJ[2][0][k][n]* MapBf->J[1][2]
					    - MapBf->J[2][0]      *MapBf->dJ[1][2][k][n])*f
		    -
		  ...
		  MapBf->dB[1][1][k][n] = ( MapBf->dJ[0][0][k][n]* MapBf->J[2][2]
					    ...
					    - MapBf->J[2][0]      *MapBf->dJ[0][2][k][n])*f
		    +
		  ...
		  MapBf->dB[1][2][k][n] =-( MapBf->dJ[0][0][k][n]* MapBf->J[1][2]
					    ...
					    - MapBf->J[1][0]      *MapBf->dJ[0][2][k][n])*f
		    -
		  ...
		  MapBf->dB[2][0][k][n] = ( MapBf->dJ[1][0][k][n]* MapBf->J[2][1]
					    ...
					    - MapBf->J[1][1]      *MapBf->dJ[2][0][k][n])*f
		    +
		  ...
		  MapBf->dB[2][1][k][n] =-( MapBf->dJ[0][0][k][n]* MapBf->J[2][1]
					    ...
					    - MapBf->J[2][0]      *MapBf->dJ[0][1][k][n])*f
		    -
		  ...
		  MapBf->dB[2][2][k][n] = ( MapBf->dJ[0][0][k][n]* MapBf->J[1][1]
					    ...
					    - MapBf->J[1][0]      *MapBf->dJ[0][1][k][n])*f
		    +
      ...
      */
{
  ...
  case 0:
    fv->stangent[0][0] = 1.;
    fv->stangent[1][0] = 0.;
    for (j = 0 ; j < nodes_per_elem; j++) fv->dstangent_dx[0][0][0][j] = 0.;
    break;
  ...
  case 2:
    ...
    if ( SQUARE(alpha) <= 1.e-7 ) {
      fprintf(stderr,
	      "Warning - norm of tangent on surface of element %d Jacobian is dangerously small %e\n",
	      ielem, alpha);
    }
    
    /* First tangent defined by seed vector gives t1 = tseed - n(tseed DOT n) normalized */
    /* not yet implemented */

    /* if in an element adjacent to one edge, make 1st tangent be perpendicular to that edge */
    /* not yet implemented */

    /* Second tangent defined as t2 = n X t1 automatically normalized because t1 and n are
     * unit vectors */
	...
	for (j=0; j<ielem_surf_dim+1; j++) {
	  dalpha_dx[j] = (2.*fv->snormal[2]*fv->dsnormal_dx[2][j][ldof] + (fv->snormal[0]-fv->snormal[1])*
			  (fv->dsnormal_dx[0][j][ldof]-fv->dsnormal_dx[1][j][ldof]))/alpha;
	    
	  fv->dstangent_dx[0][0][j][ldof]= (-alpha*fv->dsnormal_dx[2][j][ldof] + fv->snormal[2]*dalpha_dx[j])
	    /SQUARE(alpha);
	    
	  fv->dstangent_dx[0][1][j][ldof]= (alpha*fv->dsnormal_dx[2][j][ldof] - fv->snormal[2]*dalpha_dx[j])
	    /SQUARE(alpha);
	    
	  fv->dstangent_dx[0][2][j][ldof]= (alpha*(fv->dsnormal_dx[0][j][ldof]-fv->dsnormal_dx[1][j][ldof]) 
					    -(fv->snormal[0]-fv->snormal[1])*dalpha_dx[j])/SQUARE(alpha);
...
void 
calc_tangent_from_seed(struct Rotation_Vectors *tangent,
...

{
  ...
  if (af->Assemble_Jacobian && pd->v[0][MESH_DISPLACEMENT1]) {
...
/*ARGSUSED*/
void 
calc_tangent_along_basis(struct Rotation_Vectors *tangent,
			 struct Rotation_Vectors *normal,
			 const int dim,
			 const int id_side,
...
void 
cross_vectors(struct Rotation_Vectors *tangent2,
...

{
  ...
  if (fabs(sum) > 1e-8) {
    printf("2nd Tangent not perpendicular to 1st tangent (%g) vector %g,%g,%g\n",
	   sum, tangent1->vector[0], tangent1->vector[1], 
	   tangent1->vector[2]);
  }
  for (p = 0, sum = 0.; p < dim; p++) {
...
void 
calc_unseeded_edge_tangents (struct Rotation_Vectors *tangent,
...

{
  int 		j, inode, ldof;
  const int ShapeVar = pd->ShapeVar;
  const int DeformingMesh = pd->e[0][R_MESH1];
  int p;
  int j_id;
  double sign;
...

{
  ...
  for (j = 0; j < num_nodes_on_edge; j++)
	...
	{
	  if(j_id == 1) { param_dir_tet = 0;}
	  if(j_id == 2) { param_dir_tet = 1; sign *= -1;}
	  if ( DeformingMesh ) {
	    if (num_varType_at_node(inode, MESH_DISPLACEMENT1)) {
	...
	{
	  if(j_id == 2) { param_dir_tet = 1;}
	  if(j_id == 3) { param_dir_tet = 2; sign *= -1;}
	  if ( DeformingMesh ) {
	    if (num_varType_at_node(inode, MESH_DISPLACEMENT1)) {
...
void
simple_normalize_vector(struct Rotation_Vectors *vector,
			const int dim)
{
  /* Local variables */
  int p, q, j;
  ...
  for(p=0; p<dim; p++) vector->vector[p] /= sqsum;

  /* check the sum */
  for(p=0, sum = 0.; p<dim; p++) sum += vector->vector[p]*vector->vector[p];
  if (fabs(sum - 1.) > 1e-10) EH(-1, "Bad normalization");
  return;
}
/********************************************************************************/
/********************************************************************************/
...
int
load_basis_functions(const double xi[],             /*  [DIM]               */
...
void 
asdv(double **v,		/* vector to be allocated */
...
void
alloc_extern_ija_buffer(const int n, /* Order full system               (in) */
...
void 
alloc_MSR_sparse_arrays(int **ptr_ija,	/* column pointer array */
...
void
alloc_VBR_sparse_arrays (struct Aztec_Linear_Solver_System *ams , 
			 Exo_DB *exo, Dpi *dpi)       
     /*
      *
      */
  ...
  *ija = alloc_int_1(nz_temp, -1);
  ...
  for (inode = 0; inode < itotal_nodes; inode++)
      ...
      for (iunknown = 0; iunknown < inum_unknowns; iunknown++)
	{
	   ...
	   */
          for (j = exo->node_node_pntr[inode];
	      ...
	      for (inter_unknown = 0; inter_unknown < inter_node_unknowns; inter_unknown++)
		{
		  ...
		  if (icol_index > -1)
		    {
		      if ( in_list(icol_index, (*ija)[irow_index], nz_ptr, *ija) < 0 && icol_index != irow_index)
			{
			  if (nz_ptr + 1 == nz_temp) {
			    ...
			    *ija = (int *) realloc((void *) *ija, nz_temp * sizeof(int));
			    if (*ija == NULL) EH(-1, "No space for ija_temp");
...

{
   ...
   */
  avgNodeToNodeConn = (double) exo->node_node_pntr[itotal_nodes] /
    (double) itotal_nodes;
  /*
  ...
  *ija = ija_ptr = alloc_int_1(nz_temp, -1);
  ...
  for (inode = 0; inode < itotal_nodes; inode++) {
     ...
     * node...
     */
    row_num_unknowns = fill_variable_vector(inode, inode_varType,
					    inode_matID);
    /*
    ...
    for (iunknown = 0; iunknown < row_num_unknowns; iunknown++) {
      ...
      fprintf(ifp,"node = %d, unk = %d, irow_index = %d, nz_ptr = %d\n",
	      inode, iunknown, irow_index, nz_ptr);
#endif
      /*
       * Loop over the nodes which are determined to have an interaction
       * with the current row node
	...
	for (inter_unknown = 0; inter_unknown < col_num_unknowns; 
	  ...
	  if ( exo->centroid_list[ inode ] != -1 && 
		...
		if ((rowVarType == colVarType) &&
          ...
          if (Debug_Flag < 0) add_var = TRUE;  /* add all vars for checking jacobian */
...
static int
find_VBR_problem_graph(int *indx[], int *bindx[], int *bpntr[],
...
int
fill_variable_vector(int inode, int ivec_varType[], int ivec_matID[])
...
int 
find_VBR_index(const int I,	/* Block row index */
...
double 
newshape(const double xi[],	/* local coordinates                         */
      ...
      *************************************************************************/
{
  ...
  switch(eshape)
	  ...
	  switch( Iquant )
	    {
            ...
            case DPSI_S:
              ...
              break;
            default:
	      /*EH(-1, "Not a valid Iquant choice!");*/ /*Clever way out of this, for Iquant=2 */
              break;
	    }
	}
      ...
      if ( is_xfem_interp( interpolation ) )
	{
      ...
      else if (interpolation == I_Q1 ||
	       interpolation == I_Q1_D)
	{
	  /* because we may want to solve with Q1 basis functions on a BIQUAD_QUAD mesh */
	  if(Iquant < 3)
	    {
      ...
      else if (interpolation == I_SP)
	{
	  ...
	  if (Inode >= 8) EH(-1, "This element has at least 8 nodes!");
    ...
    case TETRAHEDRON:
	 ...
	 || interpolation == I_Q1_D)
	{
	  value = shape(s, t, u, TRILINEAR_TET, Iquant, Inode);
	}
      else if ( interpolation == I_P0)
        {
	...
	{
	  /* because we may want to solve with Q1 basis functions on a TRIQUAD_HEX mesh */
					 ...
					 * can be a dof at the centroid node         */
{
  ...
  xfem_dof_state( ledof, interpolation, eshape, &xfem_active, &extended_dof, &base_interp, &base_dof );
		   ...
		   (Iquant == DPSI_S || Iquant == DPSI_T || Iquant == DPSI_U) )
                {
                  ...
                  if ( Iquant == DPSI_S ) value += psi * delta * xfem->dF_xi[0];
                  if ( Iquant == DPSI_T ) value += psi * delta * xfem->dF_xi[1];
                  if ( Iquant == DPSI_U ) value += psi * delta * xfem->dF_xi[2];
                }
            }
          else
            {
        ...
        case I_Q2_HV:
          {
            if ( extended_dof )
              {
                if ( Iquant == PSI ) value = H - xfem->bf_plus;
                else if ( Iquant == DPSI_S ) value = delta * xfem->dF_xi[0] - xfem->grad_bf_plus[0];
                else if ( Iquant == DPSI_T ) value = delta * xfem->dF_xi[1] - xfem->grad_bf_plus[1];
                else if ( Iquant == DPSI_U ) value = delta * xfem->dF_xi[2] - xfem->grad_bf_plus[2];
              }
            else
              {
                value = newshape( xi, Ielem_type, Iquant, Inode, eshape, base_interp, base_dof );
        ...
        case I_Q2_HG:
          {
            if ( extended_dof )
              {
                if ( Iquant == PSI ) value = xfem->F * H - xfem->F_plus;
                else if ( Iquant == DPSI_S ) value = ( H + xfem->F*delta ) * xfem->dF_xi[0] - xfem->grad_F_plus[0];
                else if ( Iquant == DPSI_T ) value = ( H + xfem->F*delta ) * xfem->dF_xi[1] - xfem->grad_F_plus[1];
                else if ( Iquant == DPSI_U ) value = ( H + xfem->F*delta ) * xfem->dF_xi[2] - xfem->grad_F_plus[2];
              }
            else
              {
                value = newshape( xi, Ielem_type, Iquant, Inode, eshape, base_interp, base_dof );
...
void
determine_ProjectionVar(PROBLEM_DESCRIPTION_STRUCT *pd_ptr)
...
double
calc_tensor_invariant( dbl T[DIM][DIM],        // Original tensor
		       ...
		       int Ii)                 // Which invariant to calculate
{
  ...
  // Error checking
  if ( DIM != 3 ) EH(-1, "calc_tensor_invariants() has not been instrumented for 1 or 2 dimensions");
