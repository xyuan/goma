...
/*ARGSUSED*/
int 
assemble_potential(double time,	/* present time value */ 
		   ...
		   double dt)	/* current time step size */
{
  ...
  dbl A_inv[MAX_CONC*DIM][MAX_CONC*DIM]; /* inverse of Stefan_Maxwell coefficient matrix; KSC: 10/28/98 */  
  ...
  else if (mp->CurrentSourceModel == ELECTRODE_KINETICS )  /* KSC: 10/21/98; modified, KSC: 8/30/00 */
    {
      ...
      else if(mp->SolutionTemperatureModel == THERMAL_BATTERY) /* thermal battery temperature model */
            {
...
/* Derivatives of the scalar h with respect to mole fraction are computed here; h is the
  ...
  /* Calculate ii2, current density due to concentration gradients of charged  
     ...
     if( cr->MassFluxModel == STEFAN_MAXWELL_CHARGED ||   /* Stefan-Maxwell diffusion of charged species in concentrated electrolyte */  
      ...
      for ( i=0; i<ei->dof[eqn]; i++)
	{
	  ...
	  if (pd->e[0][eqn] & T_DIFFUSION) 
	  {
		...
		(cr->MassFluxModel == STEFAN_MAXWELL_CHARGED || 
	        ...
	        cr->MassFluxModel == FICKIAN_CHARGED        ||
			  ...
			  for ( p=0; p<dim; p++)
			    {
			      diffusion -= grad_phi_i[p] * dkdC[w][j] * grad_V[p];
	    		      ...
	    		      if ( mp->VoltageFormulation == V_CONDUCTIVITY)
			      {
			      if (cr->MassFluxModel == STEFAN_MAXWELL_CHARGED ||   /* add ii2 contribution for
...
/00 */
			      ...
			      else if (cr->MassFluxModel == FICKIAN_CHARGED)       /* add ii2 contribution for
...
/****************************************************************************/
void 
surface_charge_surf(double *func,
...
/*****************************************************************************/
void 
current_BV_surf(double func[DIM],
	        double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
...
******************************************************************************/
{
  ...
  if (pd->e[0][R_ENERGY]) /* if energy equation is active, re-set electrolyte temperature */
    {
      T = fv->T;   
    }
      ...
      for (w1 = 0; w1 < pd->Num_Species_Eqn; w1++ )
        {
          d_func[0][MAX_VARIABLE_TYPES + w1][j_id] -= 
                                     d_i[MAX_VARIABLE_TYPES + w1]*phi_j; 
        }
      ...
      for (j = 0; j < ei->dof[var]; j++)   
        {
          ...
          for(kdir=0; kdir<dim; kdir++) 
            {
              d_func[0][var][j_id]  += 
                            c*d_vconv->T[kdir][j_id] * fv->snormal[kdir];
	    } 
	 }
     }
      ...
      if (pd->v[0][var])
        {
	      ...
	      for(kdir=0; kdir<dim; kdir++) 
	        {
	          d_func[0][var][j_id] += 
	          c*vconv[kdir]*fv->dsnormal_dx[kdir][jvar][j_id] +
      ...
      if (pd->v[0][var])
        {
          for (j_id = 0; j_id < ei->dof[var]; j_id++) 
	       ...
	       phi_j = bf[var]->phi[j_id];
	       d_func[0][var][j_id] += 
		   c*d_vconv->v[jvar][jvar][j_id]*fv->snormal[jvar];
	     }
	 }
     } 
...
assemble_Enorm()
{
  ...
  if(af->Assemble_Residual)
      ...
      for(i = 0; i < ei->dof[R_ENORM]; i++)
	{
	  ...
	  advection = 0.0;
	  if(pd->e[0][R_ENORM] & T_ADVECTION)
	    {
	      advection = -Enorm;
	      advection *= wt_func * detJ * wt * h3;
	      advection *= pd->etm[0][R_ENORM][(LOG2_ADVECTION)];
	    }
	  
	  /* And of course, the glorious LHS, "ENORM", shall just be the source term... */
	  source = 0.0;
	  if(pd->e[0][ENORM] & T_SOURCE)
	    {
	      source = fv->Enorm;
	      source *= wt_func * detJ * wt * h3;
	      source *= pd->etm[0][R_ENORM][(LOG2_SOURCE)];
	    }

	  lec->R[upd->ep[0][R_ENORM]] [i] += advection + source;
	} /* for i = 0 ... dof[R_ENORM] */
      ...
      for(i = 0; i < ei->dof[R_ENORM]; i++)
	{
	  ...
	  for(k = 0; k < ei->dof[VOLTAGE]; k++)
	      ...
	      if(pd->e[0][R_ENORM] & T_ADVECTION)
		{
	      ...
	      lec->J[upd->ep[0][R_ENORM]][upd->vp[0][VOLTAGE]] [i][k] += advection;
	    } /* for k = 0 ... dof[VOLTAGE] */
	  ...
	  for(k = 0; k < ei->dof[ENORM]; k++)
	      ...
	      if(pd->e[0][R_ENORM] & T_SOURCE)
		{
	      ...
	      lec->J[upd->ep[0][R_ENORM]][upd->vp[0][ENORM]] [i][k] += source;
	    } /* for k = 0 ... dof[ENORM] */
	} /* for i = 0 ... dof[R_ENORM] */
			  ...
			  if ( pd->e[0][eqn] & T_SOURCE )
			    {
			      ...
			      source *= wt_func * wt * pd->etm[0][eqn][(LOG2_SOURCE)];
		  ...
		  for ( j=0; j<ei->dof[var]; j++)
		    {
		      ...
		      if ( pd->e[0][eqn] & T_SOURCE )
			{
			  source = phi_j  * det_J * h3 * wt_func * wt * pd->etm[0][eqn][(LOG2_SOURCE)];
			}
...
/*******************************************************************************/
void
apply_potential_grad_bc( double func[],
            ...
            for ( p=0; p<VIM; p++)
              {
               ...
               d_func[0][var][j] += fv->snormal[p]*( perm * grad_phi_j
                                           - d_p_dV[j] * efield[p]);
              }
            }
     }
                ...
                for (j=0; j<ei->dof[var]; j++)
                    {
                         for (q=0; q<VIM; q++)
                            {
          d_func[0][var][j] += fv->snormal[q]*( perm*fv->d_grad_V_dmesh[q][b][j]
               			          -d_p_dX[b][j]*efield[q])
...
int i, j, a, w;
  ...
  if (mp->Elec_ConductivityModel == USER)
    ...
    {
      if (cr->MassFluxModel == STEFAN_MAXWELL_CHARGED || 
          cr->MassFluxModel == STEFAN_MAXWELL_VOLUME)  
       {
         n = n_species * VIM;
...
/* Derivatives of electrical conductivity with respect to mole fraction
   ...
   -- RSL 3/31/00 */
		     ...
		     for (l = 0; l < n_species; l++)
		       {
		         if (w == 0)
		           ...
		           {
		            sum1 += z[l]*(z[w]*A_inv[VIM*l][VIM*w] - z[n_species-1]*A_inv[VIM*l][n-VIM]);
		           }
		       }
		     sum2 = 0.0;
		     for (l=0; l<n_species; l++)
  ...
  else if (cr->MassFluxModel == FICKIAN_CHARGED_X) /* RSL 9/18/00 */
     {
      ...
      if (pd_glob[mn]->e[0][R_ENERGY])  /* if the energy equation is being solved */
         {
          T = fv->T;
         }
