...
belly_flop(dbl mu)		/* elastic modulus (plane stress case) */
{
  ...
  dim = ei->ielem_dim;
  mdof = ei->dof[MESH_DISPLACEMENT1];
  /*******************************************************************************/
              ...
              for (i=0; i<dim; i++)
                 {
                  ...
                  fprintf(stderr,
                     "P_%d: Deformation Gradient %f %f in elem [%d]\n",
                      ...
                      deform_grad[i][1], ei->ielem );
#else
                  fprintf(stderr,
                     "Deformation Gradient %f %f in elem [%d] %g %g\n",
                      deform_grad[i][0], deform_grad[i][1], ei->ielem,fv->x[0],fv->x[1] );
#endif
                 }
		      ...
		      const int n_par)
{
  ...
  if(blk_id != -1 && blk_id != ei->elem_blk_id)
	return;
			   ...
			   double scale) /* Scale factor, nondimension       */
{
    ...
    id_dofmesh = ei->ln_to_dof[R_MESH1][id];
	...
	if (! xfixed[p]) {
	  ...
	  lec->R[upd->ep[0][ieqn_solid]][id_dofmesh] =
	      scale * lec->R[upd->ep[0][ieqn_mom]][id_dofmom];
	}
      }
    }
	...
	for (p = 0; p < wim; p++) {
	    ...
	    if (! xfixed[p])
	      {
		ieqn_mom = R_MOMENTUM1 + p;
		peqn_mom = upd->ep[0][ieqn_mom];
		ieqn_solid = R_MESH1 + p;
		peqn_solid = upd->ep[0][ieqn_solid];
		id_dofmom = ei->ln_to_dof[ieqn_mom][id];
		id_dofmesh = ei->ln_to_dof[ieqn_solid][id];
			   ...
			   double scale) /* Scale factor, nondimension       */
{
    ...
    id_dofmom = ei->ln_to_dof[R_MOMENTUM1][id];
    id_dofsol = ei->ln_to_dof[R_SOLID1][id];
    /* 
     * if you are in the solid phase, return without doing anything.
     * In the solid phase, there are no fluid momentum equations
     */
...
/* if this nodal contribution has already been added to fluid momentum equation (i.e.
   we are at a corner on the second side) return without doing anything */
	...
	for(p=0; p<dim; p++) {
		...
		lec->R[upd->ep[0][ieqn_solid]][id_dofsol] += 
		    scale*lec->R[upd->ep[0][ieqn_mom]][id_dofmom];
	      }
	}
      }
...
/*   add species expansion/shrinkage	*/
  ...
  if (cr->MeshMotion == LAGRANGIAN ||
      cr->MeshMotion == DYNAMIC_LAGRANGIAN)
   {
      ...
      * mesh motion */
     var = PRESSURE;
     if (pd->v[0][var] && ( mp->PorousMediaType == CONTINUOUS ) ) 
       {
	 if (cr->MeshFluxModel == INCOMP_PSTRAIN ||
	 ...
	 mp->PorousMediaType == POROUS_TWO_PHASE) && pd->e[0][R_POR_POROSITY]) 
       {
	 for ( a=0; a<VIM; a++)
	   {
		 ...
		 if (pd->v[0][POR_GAS_PRES]) TT[a][a] -= (1. - mp->saturation) * fv->p_gas; 
	       }
		 ...
		 if (pd->v[0][POR_GAS_PRES]) TT[a][a] -= mp->porosity * (1. - mp->saturation) * fv->p_gas;  
	       }
	     else if (mp->PorousMediaType == POROUS_SATURATED && pd->e[0][POR_POROSITY])
	       { 
		 TT[a][a] -= fv->p_liq; 
 ...
 /* calculate sensitivity to concentration due to concentration dependent
    elastic and bulk modulus */
	 ...
	 var = POR_LIQ_PRES;
	 if (pd->v[0][var] && pd->e[0][R_POR_POROSITY])
	   {
		 for ( p=0; p<VIM; p++)
		   {
		     for ( j=0; j<ei->dof[var]; j++)
			 ...
			 if (mp->CapStress == COMPRESSIBLE) 
			   ...
			   if (pd->v[0][POR_GAS_PRES] && pd->v[0][POR_POROSITY] )  
			     {
			       dTT_dporosity[p][p][j] -= bf[var]->phi[j]
				 * ( 
			   ...
			   if (pd->v[0][POR_POROSITY] && pd->v[0][POR_GAS_PRES] ) 
			     {
			       dTT_dporosity[p][p][j] += bf[var]->phi[j]*
...
/*  species expansion	*/
   if (pd->e[0][R_MASS] && pd->MeshMotion != ARBITRARY)
   {
	for(w=0 ; w<pd->Num_Species_Eqn ; w++)
	   {
   	    if(mp->SpecVolExpModel[w] == CONSTANT )
