...
/*****************************************************************************/
 ...
 "WARNING: Boundary condition %d, %s, applied on SS %d, may be used as an inlet condition \n",
  ...
  if (num_bc_nodes > 0) {
    BC_dup_nodes = alloc_int_1(num_bc_nodes, -1);
    BC_dup_list = (int ***) alloc_ptr_1(num_bc_nodes);
  }
  ...
  for (inode = 0; inode < num_total_nodes; inode++) {
	...
	if (idup > 1) {
	  ...
	  if (in_list(inode, 0, BC_dup_ptr, BC_dup_nodes) == -1) {
	    /* Put the node in the list */
	    BC_dup_nodes[BC_dup_ptr] = inode;
	    /* Put the pointer to the BC_Unk_List for the node
	     * into another list
	     */
	    BC_dup_list[BC_dup_ptr] = BC_Unk_List[inode];
	    BC_dup_ptr++;
	    if (Debug_Flag > 0) {
  ...
  for (inode = 0; inode < num_total_nodes; inode++) {
	...
	if (ROT_list[inode] == NULL) {
	  ...
	  if (eqn == R_MESH1 || eqn == R_MOMENTUM1) {
	     ...
	     */
	    if (in_list(inode, 0, BC_dup_ptr, BC_dup_nodes) == -1) {
	      BC_dup_nodes[BC_dup_ptr] = inode; /* put node in 
						 * list 
						 */
	      BC_dup_list[BC_dup_ptr]  = BC_Unk_List[inode]; /* ptr to
							  * eqn 
							  * BC_list
							  */
	      BC_dup_ptr++;
