 ...
 */
   ...
   if (cr->MeshMotion == TOTAL_ALE && pd->e[0][R_SOLID1])
   {
  ...
  if(PSPG && 0)
	...
	{
	  meqn = R_PMOMENTUM1 + a;
	  momentum_residual[a] = rho*pv_dot[a]/por * pd->etm[0][meqn][(LOG2_MASS)]
	    + grad_P[a]* pd->etm[0][meqn][(LOG2_DIFFUSION)]
	      - div_s[a]  * pd->etm[0][meqn][(LOG2_DIFFUSION)]
		- mu * div_G[a]  * pd->etm[0][meqn][(LOG2_DIFFUSION)]
		  - f[a] * pd->etm[0][meqn][(LOG2_SOURCE)]
	            + v[a]*(rho*sc*speed/sqrt(per)+vis/per) * pd->etm[0][meqn][(LOG2_POROUS_BRINK)] ;
	  for ( b=0; b<wim; b++)
	    {
	  ...
	  if ( pd->e[0][eqn] & T_ADVECTION )
	    {
	      ...
	      if (pd->v[0][PVELOCITY1]) /* then must be solving fluid mechanics in this material */
		{
			  ...
			  for ( a=0; a<wim; a++)
			      ...
			      if ( pd->TimeIntegration != STEADY )
				{
				  if ( pd->e[0][meqn] & T_MASS )
				    {
				      mass += (1.+2.*tt) * phi_j/dt * (double)delta(a,b);
				      mass *= rho/por *  pd->etm[0][meqn][(LOG2_MASS)]; 
				    }
				}
			      ...
			      if ( pd->e[0][meqn] & T_ADVECTION )
				{
				  ...
				  advection_a *= rho/por2 * pd->etm[0][meqn][(LOG2_ADVECTION)] ; 
				}

			      source_a = 0.;
			      if ( pd->e[0][meqn] & T_SOURCE )
				{
				  source_a   -= dfdv[a][b][j] * pd->etm[0][meqn][(LOG2_SOURCE)];
				}
			      ...
			      if ( pd->e[0][meqn] & T_POROUS_BRINK )
				{
				  for ( p=0; p<wim; p++)
					...
					pd->etm[0][meqn][(LOG2_POROUS_BRINK)];
				    }
				}
...
/* MMH */
		  ...
		  for ( a=0; a<wim; a++)
		      ...
		      if ( pd->e[0][meqn] & T_POROUS_BRINK )
			{
			  porous = v[a]*(d_rho->T[j] *sc*speed/sqrt(per))
			    * grad_phi[i][a] * pd->etm[0][meqn][(LOG2_POROUS_BRINK)];
			}
	  ...
	  if ( pd->v[0][var] )
	      ...
	      for ( j=0; j<ei->dof[var]; j++)
		{
		  ...
		  if ((cr->MeshMotion == LAGRANGIAN ||
		       cr->MeshMotion == DYNAMIC_LAGRANGIAN ||
                       cr->MeshMotion == TOTAL_ALE)  && pd->e[0][eqn] & T_ADVECTION)
		    {
		      ...
		      if ( pd->v[0][var] )
			{
			  for ( j=0; j<ei->dof[var]; j++)
				...
				-grad_phi[i][q] * bf[var]->grad_phi[j] [p]
				  * pd->etm[0][R_PMOMENTUM1 + q][(LOG2_DIFFUSION)];
			      ...
			      if ( pd->CoordinateSystem != CARTESIAN )
				{
				  for ( r=0; r<VIM; r++)
					...
					grad_phi[i][q] * phi_j  *  fv->grad_e[p][r][q] 
					  * pd->etm[0][R_PMOMENTUM1 + a][(LOG2_DIFFUSION)];
				    }
				  for ( a=0; a<wim; a++)
				    {
				      pressure_stabilization -= grad_phi[i][a] * 
