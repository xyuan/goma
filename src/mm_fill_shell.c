...
/*Sa	  var = VOLTAGE;
  if ( n_dof[var] > 0)  Sa*/ /* NOTE: Cannot use pd->v[0] here! */
/*Sa	    {
  pvar = upd->vp[0][var];   Sa*/
			 ...
			 const Exo_DB *exo)   
{
  ...
  if (af->Assemble_Residual )
      ...
      for (i = 0; i < ei->dof[eqn]; i++)
	{
	  ...
	  peqn = upd->ep[0][R_SHELL_CURVATURE];
  ...
  if (af->Assemble_Residual)
    {
      ...
      if (pd->e[0][eqn])
        {
          bfn = bf[eqn];
          peqn = upd->ep[0][R_SHELL_DIFF_CURVATURE];
                          ...
                          const double wt)
{
  ...
  if ( !(pd->e[0][eqn] & T_BOUNDARY) ) return;

  /* Unpack variables from structures for local convenience. */
  det_J = fv->sdet;
  peqn  = upd->ep[0][eqn];
  em    = pd->etm[0][eqn][(LOG2_BOUNDARY)];
 ...
 ******************************************************************************/
{
  ...
  if (af->Assemble_Jacobian)
	...
	if (Dolphin[I][VELOCITY1] > 0 ) {
	  ...
	  /*
	   *  We don't check for pd->v[0]ar[] here because the shell variable will not be active on the
			  ...
			  double scale) /* Scale factor, nondimension       */
{
  ...
  if (af->Assemble_Residual) {
    ...
    if (!curvature_fixed ) {
      ...
      lec->R[upd->ep[0][ieqn_shell]][id_dofshell] +=
	scale * (fv->snormal[0] * lec->R[upd->ep[0][ieqn_mom1]][id_dofmom1] + 
		 fv->snormal[1] * lec->R[upd->ep[0][ieqn_mom2]][id_dofmom2]);
    }
    if (!tension_fixed ) {
      ...
      id_dofshell = id_shell; 
      lec->R[upd->ep[0][ieqn_shell]][id_dofshell] +=
	scale * (fv->stangent[0][0] * lec->R[upd->ep[0][ieqn_mom1]][id_dofmom1] + 
		 fv->stangent[0][1] * lec->R[upd->ep[0][ieqn_mom2]][id_dofmom2]);
    }
  }
  ...
  if (af->Assemble_Jacobian)
	...
	{
	  ieqn_shell = R_SHELL_CURVATURE;
	  peqn_shell = upd->ep[0][ieqn_shell];
	  ieqn_mom1   = R_MOMENTUM1;
	  ieqn_mom2   = R_MOMENTUM2;
	  id_dofmom1 = ei->ln_to_dof[ieqn_mom1][id];
	  peqn_mom1 = upd->ep[0][ieqn_mom1];
	  id_dofmom2 = ei->ln_to_dof[ieqn_mom2][id];
	  peqn_mom2 = upd->ep[0][ieqn_mom2];
	  id_dofshell = id_shell; 
	      ...
	      if ( pd->v[0][var] )
		{
		  ...
		  for ( j_id=0; j_id<ei->dof[var]; j_id++)
			...
			scale*(fv->snormal[0] * lec->J[peqn_mom1][pvar][id_dofmom1][j_id] +
			       ...
			       fv->dsnormal_dx[0][q][j_id] * lec->R[upd->ep[0][ieqn_mom1]][id_dofmom1] +
			       fv->dsnormal_dx[1][q][j_id] * lec->R[upd->ep[0][ieqn_mom2]][id_dofmom2]);
		    }
		}
	    }
	      ...
	      if ( pd->v[0][var] )
		{
		  ...
		  for ( j_id=0; j_id<ei->dof[var]; j_id++)
			...
			scale*(fv->stangent[0][0] * lec->J[peqn_mom1][pvar][id_dofmom1][j_id] +
			       ...
			       fv->dstangent_dx[0][0][q][j_id] * lec->R[upd->ep[0][ieqn_mom1]][id_dofmom1] +
			       fv->dstangent_dx[0][1][q][j_id] * lec->R[upd->ep[0][ieqn_mom2]][id_dofmom2]);
		    }
		}
	    }
			  ...
			  double scale) /* Scale factor, nondimension       */
{
  ...
  if (af->Assemble_Residual) {
    ...
    if (!tension_fixed ) {
      ...
      lec->R[upd->ep[0][ieqn_shell]][id_dofshell] +=	  scale * (fv->stangent[0][0] * lec->R[upd->ep[0][ieqn_mom1]][id_dofmom1] + 
								   fv->stangent[0][1] * lec->R[upd->ep[0][ieqn_mom2]][id_dofmom2]);
	  ...
	  peqn = upd->ep[0][eqn];
          ...
          for(i = 0; i < ei->dof[eqn]; i++)
	    {
	      ...
	      for ( b = 0; b < dim; b++ )
	        {
		  var = MESH_DISPLACEMENT1 + b;
		  if ( n_dof[var] > 0)   /* NOTE: Cannot use pd->v[0] here! */
		    {
	              pvar = upd->vp[0][var];   
			   ...
			   const int sic_flag)
{
  ...
  */
  ...
  if(pd->e[0][R_MESH1] && cr->MeshMotion != ARBITRARY)
    {
  ...
  if(pd->e[0][R_SOLID1] && cr->MeshMotion != ARBITRARY)
    {
	    ...
	    cr->MeshMotion == TOTAL_ALE)
	{
	  ...
	  if ( pd->TimeIntegration != STEADY &&
	       pd->etm[0][R_SOLID1][(LOG2_ADVECTION)] )
	    {
	      for ( a=0; a<dim; a++)
		{
		      ...
		      const double *coord)
{
  ...
  if (pd_glob[bm]->e[0][R_MESH1] && cr->MeshMotion != ARBITRARY)
    {
      err = belly_flop(elc->lame_mu);
      EH(err, "error in belly flop");
	...
	{
	  memset( vconv, 0, sizeof(double)*MAX_PDIM );
	  if (pd_glob[bm]->v[0][MESH_DISPLACEMENT1])
	    memset(d_vconv->X, 0, DIM*DIM*MDE*sizeof(dbl));
	  if (pd_glob[bm]->v[0][VELOCITY1] || pd_glob[bm]->v[0][POR_LIQ_PRES])
	    memset(d_vconv->v, 0, DIM*DIM*MDE*sizeof(dbl));
	  if (pd_glob[bm]->v[0][MASS_FRACTION] || pd_glob[bm]->v[0][POR_LIQ_PRES])
	    memset(d_vconv->C, 0, DIM*MAX_CONC*MDE*sizeof(dbl));
	  if (pd_glob[bm]->v[0][TEMPERATURE])
	    memset(d_vconv->T, 0, DIM*MDE*sizeof(dbl) );
	}
    } /* end of STRESS_TENSOR */
			     ...
			     int n_dofptr[MAX_VARIABLE_TYPES][MDE])
{
  ...
  /* Only BOUNDARY terms are assembled here */
  if ( !(pd->e[0][eqn] & T_ADVECTION) ) return;

  /* Unpack variables from structures for local convenience. */
  det_J = fv->sdet;
  peqn  = upd->ep[0][eqn];
  em    = pd->etm[0][eqn][(LOG2_ADVECTION)];
		     ...
		     const Exo_DB *exo)  
{
  ...
  */
  ...
  if (af->Assemble_Residual) {
	...
	diffusion *= det_J * wt * h3 * pd->etm[0][eqn][(LOG2_DIFFUSION)];
      } 
	...
	source *= phi_i;
      ...
      source *= det_J * wt * h3 * pd->etm[0][eqn][(LOG2_SOURCE)];
  ...
  if (af->Assemble_Jacobian) {
	...
	for ( b = 0; b < dim; b++) {
	  ...
	  for ( j=0; j < ei->dof[var]; j++) {
	    ...
	    diffusion *= wt * h3 * pd->etm[0][eqn][(LOG2_DIFFUSION)];
	    ...
	    source *= wt * h3 * pd->etm[0][eqn][(LOG2_SOURCE)];
 ...
 /* Lubrication wall velocity from model */
  ...
  if (pd->e[0][R_LUBP])
      ...
      for ( i = 0; i < dim; i++) 
	{
	  ...
	  if (pd->v[0][FILL]) q[i] -= pow(H,3)*gradII_Hside[i]*curv*mp->surface_tension/(k_turb*mu);
	}
	...
	if (pd->v[0][FILL]) dqdp_1[i] -= 3.*pow(H,2)*dH_U_dp*gradII_Hside[i]*curv*mp->surface_tension/(k_turb*mu);

	dqdmu[i] = pow(H,3)*grad_P[i]*(d_k_turb_dmu*mu + k_turb)/(k_turb*k_turb*mu*mu); 
	if (pd->v[0][FILL]) dqdmu[i] += pow(H,3)*gradII_Hside[i]*curv*mp->surface_tension*(d_k_turb_dmu*mu + k_turb)/(k_turb*k_turb*mu*mu);
	if (pd->v[0][SHELL_DELTAH] && 
	    (mp->HeightUFunctionModel == CONSTANT_SPEED_DEFORM ||
	     mp->HeightUFunctionModel == CONSTANT_SPEED_MELT ||
	     mp->HeightUFunctionModel == FLAT_GRAD_FLAT_MELT ||
	...
	if (pd->v[0][FILL])
	  {
	    if (pd->v[0][SHELL_DELTAH]) dqdh[i] -= 3.*pow(H,2)*gradII_Hside[i]*curv*mp->surface_tension/(k_turb*mu);
...
/* add Petrov-Galerkin terms as necessary */
	  ...
	  source *= if_liquid * det_J * wt * h3 * pd->etm[0][eqn][(LOG2_SOURCE)];
  ...
  if (af->Assemble_Jacobian) {
	...
	for ( j=0; j<ei->dof[var]; j++) {
	  ...
	  if( pd->TimeIntegration != STEADY)
	      ...
	      if ( pd->e[0][eqn] & T_MASS )
		{
		  ...
		  mass *= - H*phi_i * rho * Cp * det_J * wt;
		  mass *= h3 * pd->etm[0][eqn][(LOG2_MASS)];
		}
	    }
	...
	for ( b = 0; b < dim; b++) {
	  ...
	  for ( j=0; j < ei->dof[var]; j++) {
	    ...
	    diffusion *= k_eff * wt * h3 * pd->etm[0][eqn][(LOG2_DIFFUSION)];
	    ...
	    source *= if_liquid * wt * h3 * pd->etm[0][eqn][(LOG2_SOURCE)];
	...
	for ( j=0; j<ei->dof[var]; j++) {
	  ...
	  source *= if_liquid * wt * det_J* h3 * pd->etm[0][eqn][(LOG2_SOURCE)];
		  ...
		  if (pd->e[0][eqn] & T_SOURCE)
		    {
		      ...
		      source *= det_J * wt;
		      source *= h3;
		      source *= pd->etm[0][eqn][(LOG2_DIFFUSION)];
		    }
...
/* Assemble advection term */
	  ...
	  if (pd->e[0][eqn] & T_ADVECTION) 
	  {
	    ...
	    advection *= det_J * wt * h3 * pd->etm[0][eqn][(LOG2_ADVECTION)];
	  }
	       ...
	       )
{
  ...
  if ( pd->e[0][R_SHELL_SAT_GASN] ) nbar = fv->sh_sat_gasn;
  ...
  if ( pd->v[0][FILL] ) *dPdz_curv = *dPdz_Plub * sigma*lsi->Hn;
  ...
  if ( pd->v[0][FILL] ) load_lsi( oldlsls );  
...
assemble_porous_shell_closed(
  ...
  // Prepare switches
  int mass_on = pd->e[0][eqn] & T_MASS;
  int divergence_on = pd->e[0][eqn] & T_DIVERGENCE;
  int transient_on = pd->TimeIntegration != STEADY;

  // FEM weights
  dbl dA = det_J * wt * h3;
  dbl etm_mass = pd->etm[0][eqn][(LOG2_MASS)];
  dbl etm_divergence = pd->etm[0][eqn][(LOG2_DIVERGENCE)];
  ...
  eqn = R_LUBP;
  if ( af->Assemble_Residual & pd->e[0][eqn] & transient_on ) {
    peqn = upd->ep[0][eqn];
	...
	source -= vz * phi_i;
      ...
      source *= dA * pd->etm[0][eqn][(LOG2_SOURCE)];
  ...
  if (af->Assemble_Jacobian) {
	...
	for ( b = 0; b < DIM; b++) {
	  ...
	  for ( j = 0; j < ei->dof[var]; j++) {
	    ...
	    divergence *= pd->etm[0][eqn][(LOG2_DIVERGENCE)];
  ...
  eqn = R_LUBP;
  if ( af->Assemble_Jacobian & pd->e[0][eqn] & transient_on ) {
    peqn = upd->ep[0][eqn];
...
assemble_porous_shell_gasn(
  ...
  if ( !pd->e[0][eqn] || !pd->e[0][R_SHELL_SAT_CLOSED] )
  ...
  // Prepare switches
  int mass_on = pd->e[0][eqn] & T_MASS;
  int source_on = pd->e[0][eqn] & T_SOURCE;
  ...
  if (af->Assemble_Residual) {
	...
	mass += fv_dot->sh_sat_gasn * phi_i;
      ...
      mass *= wt * h3 * det_J * pd->etm[0][eqn][(LOG2_MASS)];
	...
	source -= flux * phi_i;
      ...
      source *= wt * h3 * det_J * pd->etm[0][eqn][(LOG2_SOURCE)];
  ...
  if (af->Assemble_Jacobian) {
	...
	for ( b = 0; b < DIM; b++) {
	  ...
	  for ( j = 0; j < ei->dof[var]; j++) {
	    ...
	    mass *= wt * h3 * fv->dsurfdet_dx[b][jk] * pd->etm[0][eqn][(LOG2_MASS)];
	    ...
	    source *= wt * h3 * fv->dsurfdet_dx[b][jk] * pd->etm[0][eqn][(LOG2_SOURCE)];
...
assemble_porous_shell_open(
  ...
  dbl dA = det_J * wt * h3;
  dbl etm_mass = pd->etm[0][eqn][(LOG2_MASS)];
  dbl etm_diff = pd->etm[0][eqn][(LOG2_DIFFUSION)];
  dbl etm_sour = pd->etm[0][eqn][(LOG2_SOURCE)];
  ...
  // Assemble residual contribution to the lubrication equation
  eqn = R_LUBP;
  if (af->Assemble_Residual & pd->e[0][eqn]) {
    peqn = upd->ep[0][eqn];
  ...
  eqn = R_LUBP_2;
  if ((af->Assemble_Residual & upd->ep[0][eqn]) >= 0) {
    peqn = upd->ep[0][eqn];
 ...
 // Assemble sensitivities for PHASE1
  ...
  eqn = R_LUBP;
  if (af->Assemble_Jacobian & pd->e[0][eqn]) {
    peqn = upd->ep[0][eqn];
  ...
  eqn = R_LUBP_2;
  if ((af->Assemble_Jacobian & upd->ep[0][eqn]) >= 0) {
    peqn = upd->ep[0][eqn];
   ...
   */
{
  ...
  if (!pd->e[0][R_LUBP_2]) return(status);
...
assemble_lubrication_curvature(
  ...
  /* Bail out fast if there's nothing to do */
  if ( !pd->e[0][eqn]  ) return(status);
  if ( !pd->e[0][FILL] ) EH(-1, "Must activate level set equation to calculate curvature.");
  ...
  if (af->Assemble_Residual) {
	...
	mass += *esp->sh_l_curv[i] * phi_i;
      ...
      mass *= det_J * wt * h3 * pd->etm[0][eqn][(LOG2_MASS)];
	...
	for ( a = 0; a < VIM; a++ ) {
      ...
      diff *= det_J * wt * h3 * pd->etm[0][eqn][(LOG2_DIFFUSION)];
	...
	for ( a = 0; a < VIM; a++ ) {
      ...
      div *= det_J * wt * h3 * pd->etm[0][eqn][(LOG2_DIVERGENCE)];
  ...
  if (af->Assemble_Jacobian) {
	...
	for ( b = 0; b < DIM; b++) {
	  ...
	  for ( j = 0; j < ei->dof[var]; j++ ) {
	    ...
	    mass *= fv->dsurfdet_dx[b][jj] * wt * h3 * pd->etm[0][eqn][(LOG2_MASS)];
	    ...
	    diff *= wt * h3 * pd->etm[0][eqn][(LOG2_DIFFUSION)];
	    ...
	    div *= wt * h3 * pd->etm[0][eqn][(LOG2_DIVERGENCE)];
...
assemble_lubrication_curvature_2(
  ...
  /* Bail out fast if there's nothing to do */
  if ( !pd->e[0][eqn]  ) return(status);
  if ( !pd->e[0][PHASE1] ) EH(-1, "Must activate phase1 equation to calculate curvature.");
 ...
 struct Level_Set_Data *ls_old;
  ...
  /* Rotate grad(F) and grad(kappa) to shell coordinates */
  if(upd->ep[0][R_MESH1] > -1) EH(-1," Must add mesh dependence to phase field");
  dbl gradII_F[DIM], gradII_kappa[DIM];
  dbl d_grad_F_dmesh[DIM][DIM][MDE], d_gradII_F_dmesh[DIM][DIM][MDE];
  dbl d_grad_kappa_dmesh[DIM][DIM][MDE], d_gradII_kappa_dmesh[DIM][DIM][MDE];
  memset(d_grad_F_dmesh, 0.0, sizeof(double)*DIM*DIM*MDE);
                  ...
                  if (pd->e[0][eqn] & T_SOURCE)
                    {
                     for (ii = 0; ii < DIM; ii++)
                      ...
                      source_plus *= pd->etm[0][eqn][(LOG2_SOURCE)];
                    }
                  ...
                  if (pd->e[0][eqn] & T_SOURCE)
                    {
                     for (ii = 0; ii < DIM; ii++)
                      ...
                      source_minus *= pd->etm[0][eqn][(LOG2_SOURCE)];
                    }
