...
{
  int i;
  for (i = 0; i < num; i++) (void) printf("%c", ch);
}
/*****************************************************************************/
/*****************************************************************************/
/*****************************************************************************/
...
static void
StrngDefaultDatabase(char *str, int ID)
...
void
matrl_prop_print(MATRL_PROP_STRUCT *mat_ptr, int mn)
     ...
     *************************************************************************/
{
  ...
  /* Code cleanup crew is not happy with the unimportant information provided by this function to the
   * terminal.  In the calling program we now make this call dependent on the Debug_Flag level
   */

  printf("\n\n"); print_char('=', 80); printf("\n\n");
  printf("\tMaterial Property Printout for Material %d: %s\n", mn,
	 (char *) mat_ptr->Material_Name);
  ...
  if (pd_ptr->Num_Species > 0) {
    ...
    for (i = 0; i < pd_ptr->Num_Species; i++) {
      printf("%s\t%d\t%g\t",
	     mat_ptr->Species_Names[i],  mat_ptr->PhaseID[i],
...
int
goma_mat_prop_init(MATRL_PROP_STRUCT *mat_ptr, int mn,
     ...
     ************************************************************************/
{
  ...
  if (mat_ptr->Species_Names == NULL) {
    mat_ptr->Species_Names = alloc_VecFixedStrings(mat_ptr->Num_Species + 1,
						   sizeof(CK_NAME_STR));
    if (mat_ptr->Species_Names != NULL) {
  ...
  if (mat_ptr->Porous_Names == NULL) {
    mat_ptr->Porous_Names = alloc_VecFixedStrings(mat_ptr->Num_Porous_Eqn,
				 	          sizeof(CK_NAME_STR));
    if (mat_ptr->Porous_Names != NULL) {
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {    
	 ...
	 */
        if (bc->BC_Data_Float[0] != upd->Pressure_Datum) {
		  ...
		  yo, upd->Pressure_Datum, bc->BC_Data_Float[0]);
...
double 
calc_density(MATRL_PROP_STRUCT *matrl, int doJac, 
	     PROPERTYJAC_STRUCT *densityJac, double time)
     ...
     ***************************************************************************/
{
  ...
  if (doJac) {
    ...
    if (upd->vp[0][TEMPERATURE] != -1) {
      num_dep = 1;
    } else {
      num_dep = 0;
    }
    if (upd->vp[0][MASS_FRACTION] != -1 || upd->vp[0][SPECIES_UNK_0] != -1) {
  ...
  if (matrl->DensityModel == CONSTANT ) 
  ...
  else if (matrl->DensityModel == DENSITY_FOAM ||
	   matrl->DensityModel == DENSITY_FOAM_CONC ||
    ...
    if (F > alpha) {
      ...
      rho = ((param[0] + param[1]) + (param[1] - param[0])
	      * sin(M_PIE * F / (2.0*alpha)))/ 2.0;
    }
    ...
    if (vol < 0.) {
      ...
      if (doJac) {
	propertyJac_add1SpEnd(densityJac, matrl, species,
			      (rho_s - rho_f), rho);
      }
    }      
    ...
    {
    rho  = matrl->u_density[0] 
	- matrl->u_density[1]*(2.0*stateVector[SPECIES_UNK_0]);    
    if (doJac) {
      propertyJac_addEnd(densityJac, MASS_FRACTION,
			 matID, 0, -matrl->u_density[1]*2.0, rho);
    }
	...
	avgMolecWeight = wt_from_Yk(num_species, 
				    stateVector + SPECIES_UNK_0, mw);
	rho = pressureThermo * avgMolecWeight / 
	    ( RGAS_CONST * stateVector[TEMPERATURE]);
	break;
    case SPECIES_MOLE_FRACTION:
	avgMolecWeight = wt_from_Xk(num_species,
				    stateVector + SPECIES_UNK_0, mw);
	rho = pressureThermo * avgMolecWeight / 
	    (RGAS_CONST * stateVector[TEMPERATURE]);
	break;
    ...
    default:
	fprintf(stderr,
		"Density error: species var type not handled: %d\n",
		speciesVT);
	exit(-1);
    }
...
double 
calc_concentration(MATRL_PROP_STRUCT *matrl, int doJac, 
		   PROPERTYJAC_STRUCT *concJac)
...
void
load_properties(MATRL_PROP_STRUCT *matrl, double time)
