...
/* 
 *$Id: mm_fill_porous.c,v 5.19 2008-12-19 22:54:25 rbsecor Exp $
 */
     ...
     */
{
  ...
  if (mp->PorousMediaType == POROUS_SATURATED) {
    ...
    EH(err,"problem in getting the partially-saturated porous  terms, decoupled");
    if (neg_elem_volume) return(err);
  }
  ...
  if (af->Assemble_Residual) {
     ...
     *   not active.  The test for activity is pd->e[0][var] were 
	...
	for (i = 0; i < ei->dof[eqn]; i++) {
	  ...
	  if (ei->active_interp_ledof[ledof]) {
	    ...
	    if (pd->TimeIntegration != STEADY) {
	      if (pd->e[0][eqn] & T_MASS) {
		if (mp->Porous_Mass_Lump) {
		  mass = - (pmv_ml->Inventory_Solvent_dot[i][w] *
			    wt_func * wt_total * pd->etm[0][eqn][LOG2_MASS]);
		} else {
		  mass = - (pm_terms.Inventory_solvent_dot[w] *
			    wt_func * wt_total * pd->etm[0][eqn][LOG2_MASS]);
		}
	      }
	    }
		...
		advection_a += pm_terms.conv_flux[w][p];
	      ...
	      advection *= wt_total * pd->etm[0][eqn][(LOG2_ADVECTION)]; 
	    }
	    ...
	    advection_supg = 0.0;
	    if (supg > 0.0 && eqn == R_POR_LIQ_PRES) {
	      advection_supg = - supg * pm_terms.pi_supg[i] * 
		  pm_terms.conv_flux_supg[w] *
		  wt_total * pd->etm[0][eqn][(LOG2_DIFFUSION)];
	    }
	     ...
	     */
	    diffusion = 0.0;
	    if (pd->e[0][eqn] & T_DIFFUSION) {
	      for (p = 0; p < VIM; p++) {
		diffusion += grad_phi_i[p] * pm_terms.diff_flux[w][p];
	      }
	      diffusion *= wt_total * pd->etm[0][eqn][(LOG2_DIFFUSION)];
	    }
	    /*
	     ...
	     */
	    source = 0.0;
	    if (pd->e[0][eqn] & T_SOURCE) {
	      source = (pm_terms.MassSource[w] * wt_func * wt_total *
			pd->etm[0][eqn][(LOG2_SOURCE)]);
	    }
	  ...
	  if (ei->active_interp_ledof[ledof]) {
		...
		for (j = 0; j < ei->dof[var]; j++) {
		  ...
		  if (pd->TimeIntegration != STEADY) {
			...
			if (var == POR_LIQ_PRES && supg != 0.0 &&
			  ...
			  mass = - supg * pm_terms.d_pi_supg_dpmv[i][i_pl][j] *
			      pmv_ml->Inventory_Solvent_dot[i][w] *
			      wt_total * pd->etm[0][eqn][LOG2_MASS];
			}
		      } else {
			mass = - pm_terms.d_Inventory_solvent_dot_dpmv[w][w1][j] *
			    wt_func * wt_total * pd->etm[0][eqn][(LOG2_MASS)];
			...
			if (var == POR_LIQ_PRES && supg != 0.0 &&
			   ...
			   */
			  mass -= supg * pm_terms.d_pi_supg_dpmv[i][i_pl][j] *
			      pm_terms.Inventory_solvent_dot[w] *
			      wt_total * pd->etm[0][eqn][LOG2_MASS];
			}
		      }
		    }
		  ...
		  if (pd->e[0][eqn] & T_ADVECTION) {
			...
			advection_c += pm_terms.conv_flux[w][p];
		      ...
		      advection -= supg * pm_terms.d_pi_supg_dpmv[i][i_pl][j] *
			  advection_c;
		    }
		    advection *= wt_total * pd->etm[0][eqn][(LOG2_ADVECTION)];
		  }
		  ...
		  if (supg > 0.0) {
			...
			eqn == POR_LIQ_PRES) {
		      advection_supg = 
			  (- pm_terms.d_pi_supg_dpmv[i][i_pl][j] *
			   pm_terms.conv_flux_supg[w] -
			   pm_terms.pi_supg[i] * 
			   pm_terms.d_conv_flux_supg_dpmv[i_pl][i_pl][j]);
		    }
		    advection_supg *= supg * wt_total * pd->etm[0][eqn][(LOG2_DIFFUSION)];
		  }
		  ...
		  if (pd->e[0][eqn] & T_DIFFUSION) {
		    for (p = 0; p < VIM; p++) {
			  ...
			  pm_terms.d_diff_flux_dpmv[w][p][w1][j];
		    }
		    diffusion *= wt_total * pd->etm[0][eqn][(LOG2_DIFFUSION)];
		  }	  
		  ...
		  source = 0.0;
		  if (pd->e[0][eqn] & T_SOURCE) {
		    source = (pm_terms.d_MassSource_dpmv[w][w1][j] * wt_func *
			      wt_total * pd->etm[0][eqn][(LOG2_SOURCE)]);
		    if (var == POR_LIQ_PRES && supg != 0.0 &&
			eqn == POR_LIQ_PRES) {
		...
		for (j = 0; j < ei->dof[var]; j++) {
		  ...
		  if (pd->TimeIntegration != STEADY) {
		    if ( pd->e[0][eqn] & T_MASS ) {
		      mass  = - (pm_terms.Inventory_solvent_dot[w] * 
				 wt_func * wt * pd->etm[0][eqn][(LOG2_MASS)] *
		  ...
		  if (pd->e[0][eqn] & T_DIFFUSION) {
		    ...
		    for (p = 0; p < VIM; p++)  {
			  ...
			  pm_terms.d_diff_flux_dmesh[w][p][b][j];
		    }
		    diff_b *= h3 * det_J * wt;
		    ...
		    diffusion *= pd->etm[0][eqn][LOG2_DIFFUSION];
		  }
		  ...
		  if (pd->e[0][eqn] & T_SOURCE) {
			...
			* wt * phi_i; 
		    source += pm_terms.d_MassSource_dmesh[w][b][j] * 
			det_J * h3 * wt * phi_i;
		    source *= pd->etm[0][eqn][LOG2_SOURCE];
		  }
	     ...
	     */
	    var = TEMPERATURE;
	    if (pd->e[0][eqn] && pd->v[0][var] && !pd->e[0][R_POR_ENERGY]) {
	      pvar = upd->vp[0][var];
	      for (j = 0; j < ei->dof[var]; j++) {
		phi_j = bf[var]->phi[j];
		...
		if (pd->e[0][eqn] & T_ADVECTION) {
		  ...
		  advection *= pd->etm[0][eqn][(LOG2_ADVECTION)] * h3 * det_J * wt;
		}
		...
		if (pd->e[0][eqn] & T_DIFFUSION) {
		  ...
		  diffusion *= h3 * det_J * wt * pd->etm[0][eqn][(LOG2_DIFFUSION)];
		}
		...
		for (j = 0; j < ei->dof[var]; j++) {
		  ...
		  diffusion *= pd->etm[0][eqn][(LOG2_DIFFUSION)] * h3 * det_J * wt;
      ...
      **********************************************************************/
{
  ...
  static int is_initialized = FALSE ; /* This static flag is set TRUE after the first pass through.  This is to
				       * reduce the tremendous overhead associated with unnecessarily reinializing the mp, pmt, pmv structures 
				       */
  
  if (mp->PorousMediaType == CONTINUOUS) return 0; /* shouldn't get here */
  ...
  if (mp->PorosityModel == CONSTANT)
      ...
      if (pd->TimeIntegration == TRANSIENT)
	{
	  ...
	  if (pd->v[0][POR_POROSITY]) mp_old->d_porosity[POR_POROSITY] = 0.0;
	}
    }
	...
	EH(-1, "Variable porosity not allowed with brinkman");
      ...
      if (!pd->v[0][POR_POROSITY]) EH(-1, "Can't solve for DEFORM w/o solving for porosity");
  ...
  if (mp->PermeabilityModel == CONSTANT ||
      mp->PermeabilityModel == K_TENSOR ||
      mp->PermeabilityModel == SOLIDIFICATION) 
    {
    /* do nothing */
    }
  else if (mp->PermeabilityModel == ORTHOTROPIC ||
  ...
  else if (mp->PermeabilityModel == EXTERNAL_FIELD)
    {
      ...
      EH(i_por_ev, "Permeability external field not found!");
  ...
  else
    {
      if (mp->PorousMediaType == POROUS_BRINKMAN) 
	{
	  WH(-1, "Variable permeability not allowed with brinkman");
	}
 ...
 if (mp->PorousMediaType == POROUS_SATURATED) {
    ...
    */
   if (mp->PermeabilityModel != K_TENSOR && 
 ...
 if (mp->PorousMediaType == POROUS_UNSATURATED) {
   ...
   p_gas_star =  mp->u_porous_gas_constants[3];
   if (pd->v[0][SHELL_PRESS_OPEN]) 
     {
       p_liq_open=fv->sh_p_open;
       p_liq_open_old=fv_old->sh_p_open;
     }
   else if (pd->v[0][SHELL_PRESS_OPEN_2]) 
     {
...
if ( !is_initialized ) {
  ...
  */
 if( !is_initialized ) {
   /* this section really should be masked by some if( pd->v[0][ var ] ).  But which one ?.  I cannot tell */
 ...
 load_MandE_flux(porosity, cap_pres, saturation, d_cap_pres);
  ...
  for (j=0; j<8; j++){
  printf("\t j=%d d_rel_mass_flux_dpmv[i_pl][0][i_pl]=%g \n",j,pmv->d_rel_mass_flux_dpmv[i_pl][0][i_pl][j]);
  printf("\t j=%d d_rel_mass_flux_dpmv[i_pl][1][i_pl]=%g \n",j,pmv->d_rel_mass_flux_dpmv[i_pl][1][i_pl][j]);
 ...
 if (mp->Porous_wt_funcModel == SUPG) {
    h_elem_siz(Stab->hsquared, Stab->hhv, Stab->dhv_dxnode, pd->e[0][R_MESH1]);
    Stab->Grid_Peclet_Number[POR_LIQ_PRES] = 
	get_supg_terms_porous(Stab->hsquared, Stab->hhv);
 }
 is_initialized = TRUE; 
 return 0;
...
/*ARGSUSED*/
static void
load_porous_properties_nodes(int lnn)
     ...
     **********************************************************************/
{
  ...
  if (mp->PorosityModel == CONSTANT) 
	...
	mp_old->porosity = mp->porosity;
	if (pd->v[0][POR_POROSITY]) mp_old->d_porosity[POR_POROSITY] = 0.0;
	else                     mp_old->d_porosity[POR_POROSITY] = 1.0;
      }
    }
  else if (mp->PorosityModel == EXTERNAL_FIELD)
    {
   ...
   *      in both gas and liquid phases
   */
  load_bulk_density(mp->porosity, pmv->cap_pres, mp->saturation, 
		    d_cap_pres);
}
/**************************************************************************/
/**************************************************************************/
...
double
get_supg_terms_porous(double hsquare[DIM], double hh[DIM][DIM])
     ...
     *********************************************************************/
{
  ...
  case 2:
      ...
      h_veloc_elem = (h_veloc_lcd[0] * h_veloc_lcd[0] +
		      h_veloc_lcd[1] * h_veloc_lcd[1]);
      break;
  case 3:
      h_veloc_lcd[0] = (u_supg[0] * hh[0][0] + u_supg[1] * hh[0][1] + 
			u_supg[2] * hh[0][2]);
      h_veloc_lcd[1] = (u_supg[0] * hh[1][0] + u_supg[1] * hh[1][1] +
			u_supg[2] * hh[1][2]);
      h_veloc_lcd[2] = (u_supg[0] * hh[2][0] + u_supg[1] * hh[2][1] +
			u_supg[2] * hh[2][2]);
      h_veloc_elem = (h_veloc_lcd[0] * h_veloc_lcd[0] + 
		      h_veloc_lcd[1] * h_veloc_lcd[1] + 
...
void
load_nodal_porous_properties(double tt, double dt)
     ...
     *********************************************************************/
{
  ...
  for (lvd = 0; lvd < ei->Lvdesc_Numdof[i_lvdesc]; lvd++) {
    ...
    if (tt > 0.0) {
	  ...
	  (1.0 + 2.0 * tt) * 
	  (pmv_ml->Inventory_Solvent[idof][i_pg] -
	   pmv_ml->Inventory_Solvent_old[idof][i_pg]) / dt
	  ...
	  (1.0 + 2.0 * tt) * 
	  (pmv_ml->Inventory_Solvent[idof][i_pe] -
	   pmv_ml->Inventory_Solvent_old[idof][i_pe]) / dt
	...
	pmv_ml->d_Inventory_Solvent_dot_dpmv[idof][i_pl][w1] = 
	    (1 + 2. * tt) *
	  pmv_ml->d_Bulk_Density[idof][i_pl][POR_LIQ_PRES + w1]/ dt ;

	if (pd->e[0][R_POR_GAS_PRES]) 
	  pmv_ml->d_Inventory_Solvent_dot_dpmv[idof][i_pg][w1] +=
	      (1 + 2. * tt) *
	    pmv_ml->d_Bulk_Density[idof][i_pg][POR_LIQ_PRES + w1]/ dt;
 
	if (pd->e[0][R_POR_ENERGY]) 
	  pmv_ml->d_Inventory_Solvent_dot_dpmv[idof][i_pe][w1] +=
	      (1 + 2. * tt) *
	    pmv_ml->d_Bulk_Density[idof][i_pe][POR_LIQ_PRES + w1]/ dt; 
     ...
     **********************************************************************/
{
  ...
  if (cr->PorousFluxModel == DARCY_FICKIAN || 
    ...
    for (a = 0; a < VIM; a++) {
      pmt->diff_flux[i_pl][a] = (pmv->rel_mass_flux[i_pl][a] +
				 vconv[a] * pmv->bulk_density[i_pl]);
      if (pd->e[0][R_POR_GAS_PRES]) {
	pmt->diff_flux[i_pg][a] = (pmv->rel_mass_flux[i_pg][a] +
   ...
   */
  if (pd->e[0][R_POR_POROSITY]) {
    if(pd->e[0][R_POR_SINK_MASS])
      {
       	pmt->conv_flux[i_pore][0] = (fv->volume_change -
      ...
      {
	pmt->conv_flux[i_pore][0] = (fv->volume_change -
				 (1.0 - mp->u_porosity[0]) / (1.0 - fv->porosity));
      }
  }
   ...
   */
    ...
    if(pd->e[0][R_POR_SINK_MASS] && mp->PorousSinkConstantsModel == LINEAR)
      {
  ...
  if (mp->Porous_wt_funcModel == SUPG) {
    ...
    dlamdPl = (mp->density * mp->permeability *
	       mp->d_rel_liq_perm[POR_LIQ_PRES]);
  ...
  if (af->Assemble_Jacobian) {
    ...
    var = TEMPERATURE;
    if ( pd->v[0][var]  && !pd->e[0][R_POR_ENERGY]) {
      if (cr->PorousFluxModel == DARCY_FICKIAN ||
	  cr->PorousFluxModel == POWERLAW_DARCY_FICKIAN) {
	for ( a=0; a<VIM; a++) {
	  for ( j=0; j<ei->dof[var]; j++) {
	...
	if (pd->e[0][var]) {
	  for ( j=0; j<ei->dof[var]; j++) {
	    ...
	    pmt->d_conv_flux_dpmv[i_pe][a] [i_pg][j] = fv->grad_d_dot[a][a]*
	            pmv->d_bulk_density[i_pe][POR_GAS_PRES]*bf[var]->phi[j];
	...
	if (pd->e[0][var]) {
	  for ( j=0; j<ei->dof[var]; j++) {
	    ...
	    pmt->d_conv_flux_dpmv[i_pe][a] [i_pe][j] = fv->grad_d_dot[a][a]*
	            pmv->d_bulk_density[i_pe][POR_TEMP]*bf[var]->phi[j];
	...
	case 2:
	    ...
	    tmp = (fv->grad_p_liq[0] * fv->grad_p_liq[0] +
		   fv->grad_p_liq[1] * fv->grad_p_liq[1]);
	    break;
	case 3:
	    pmv->d_U_supg_hnorm_dmde[0][j] =
		(pmv->U_supg_hnorm[0] - pmv->d_U_supg_hnorm_dmde[0][j]) / delta_val;
	...
	tmp *= - dlamdPl;
	pmt->d_conv_flux_supg_dpmv[i_pl][i_pl][j] =
	    (tmp - pmt->conv_flux_supg[i_pl]) / delta_val;
	...
	case 2:
	    for (j = 0; j < ei->dof[var]; j++) {	  
	      pmt->d_pi_supg_dpmv[i][i_pl][j] =
		  pmv->d_U_supg_hnorm_dmde[0][j] * grad_phi_i[0] + 
...
int
get_porous_part_sat_terms_decoupled(struct Porous_Media_Terms *pmt,
				    double tt, /* time integration scheme param */
				    double dt) /* current time step size */
     ...
     **********************************************************************/
{
   ...
   * CHANGE ACCORDINGLY 
   */
  if (pd->e[0][R_POR_GAS_PRES]) EH(-1,"Must update get_porous_part_sat_terms_decouple for two phase");
  ...
  if (pd->TimeIntegration != STEADY) {
    ...
    pmt->Inventory_solvent_dot[i_pl] = pmv->bulk_density[i_pl] * 
       mp->porous_compressibility * fv_dot->p_liq ; 
     ...
     * Gas solvent component, e.g. air 
     */
    if (pd->e[0][R_POR_GAS_PRES]) EH(-1,"Need to implement gas phase transport in decoupled version");
    if (pd->e[0][R_POR_POROSITY]) EH(-1,"Need to implement solid transport in decoupled version");
   ...
   */
	...
	/* old pmt->conv_flux[i_pl][a] =  0.; */
     ...
     pmt->conv_flux[i_pl][0] = pmv_old->bulk_density[i_pl] * fv->external_field[efv->ev_dpordt_index]/(1. - fv->porosity);
  /*
   * DIFFUSIVE FLUX TERM - this is the term that is integrated by parts in the 
   *   species conservation residual equation.  In a porous medium it contains 
   *   five parts of the species flux:
  ...
  if (af->Assemble_Jacobian) {
    ...
    if (pd->TimeIntegration != STEADY) {
      for (j = 0; j < ei->dof[var]; j++) {
	  pmt->d_Inventory_solvent_dot_dpmv[i_pl][i_pl] [j] += pmv->bulk_density[i_pl] *
	    mp->porous_compressibility * (1. + 2.*tt) *bf[var]->phi[j]/dt 
	  + pmv->d_bulk_density[i_pl][POR_LIQ_PRES] * bf[var]->phi[j] *
	    mp->porous_compressibility * fv_dot->p_liq;

	   pmt->d_Inventory_solvent_dot_dpmv[i_pl][i_pl] [j] += fv->porosity*
	     (mp->density - pmv->gas_density_solvents[i_pl])*
	     (mp->d_d_saturation[POR_LIQ_PRES][POR_LIQ_PRES]* bf[var]->phi[j] *fv_dot->p_liq + 
	            (1.+2.*tt) * mp->d_saturation[POR_LIQ_PRES]*bf[var]->phi[j]/dt)
	     - fv->porosity*(pmv->d_gas_density_solvents[i_pl][i_pl]*
			     bf[var]->phi[j])*mp->d_saturation[POR_LIQ_PRES]*fv_dot->p_liq;;
...
int
get_porous_fully_sat_terms(struct Porous_Media_Terms *pmt,
			   double tt, double dt)
     ...
     ************************************************************************/
{
  ...
  if(efv->ev_porous_decouple)
	...
	mp->porous_compressibility * fv_dot->p_liq;
      ...
      /* pmt->conv_flux[i_pl][0] = mp->density*fv->porosity*(fv->porosity - 0.947)/(1 - fv->porosity)/5.e-9; */

      pmt->conv_flux[i_pl][0] = mp->density*fv->porosity * fv->external_field[efv->ev_dpordt_index]/(1.- fv->porosity);
	...
	if (pd->TimeIntegration != STEADY) {
	  ...
	  pmt->Inventory_solvent_dot[i_pl] =  ((1 + 2. * tt) * 
					   (pmt->Inventory_solvent[i_pl] - 
					    pmt->Inventory_solvent_old[i_pl])/dt
      ...
      else
	{
       ...
       */
      if (pd->v[0][POR_LIQ_PRES] && pd->TimeIntegration != STEADY) 
	{
	  for (a = 0; a < VIM; a++) {
  ...
  if (cr->PorousFluxModel == DARCY_FICKIAN ||
    ...
    for (a = 0; a < VIM; a++) 
      {
	pmt->diff_flux[i_pl][a] = mp->density * 
   ...
   */
   ...
   if(pd->e[0][R_POR_SINK_MASS] && mp->PorousSinkConstantsModel == LINEAR)
      {
	pmt->MassSource[i_pl] -= mp->u_porous_sink_constants[0]*
	                         mp->u_porous_sink_constants[2]*
	                         (mp->u_porous_sink_constants[1] - fv->sink_mass)
 ...
 if (af->Assemble_Jacobian) {
	...
	*/
	 ...
	 pmt->d_Inventory_solvent_dot_dpmv[i_pl][i_pl][j]  = 
	   mp->density * mp->porous_compressibility * 
	       (1. + 2.*tt) * phi_ptr[j] /dt; 
       }
     }
    ...
    */      
   var = POR_POROSITY;
   if (pd->TimeIntegration != STEADY && pd->v[0][POR_POROSITY]) {
     phi_ptr = bf[var]->phi;
     for (j = 0; j < ei->dof[var]; j++) {
	...
	for (j = 0; j < ei->dof[var]; j++) {
	  pmt->d_diff_flux_dpmv[i_pl][a][i_pore][j] = 
	      mp->density * vconv[a]
	    * mp->d_porosity[POR_POROSITY] * phi_ptr[j];
	}
      }
    }
	...
	mp->PermeabilityModel != KC_TENSOR ) {
	  ...
	  for (j = 0; j < ei->dof[var]; j++) 
	    {
	      pmt->d_diff_flux_dpmv[i_pl][a][i_pore][j] += 
		mp->density * pmv->liq_darcy_velocity[a] *
	 ...
	 */
	  if(cr->PorousFluxModel == DARCY_FICKIAN)
		...
		pmt->d_diff_flux_dpmv[i_pl][a][i_pl][j] -= 
		  mp->density * mp->permeability / mp->viscosity * 
		  bf[var]->grad_phi[j][a]; 
	      }
	    }
	  else if (cr->PorousFluxModel == POWERLAW_DARCY_FICKIAN)
		...
		pmt->d_diff_flux_dpmv[i_pl][a][i_pl][j] -= 
		  mp->density * mp->permeability / mp->viscosity * 
		  n_pow*pow(fv->grad_p_liq[a], n_pow-1)*bf[var]->grad_phi[j][a]; 
	      }
	    }
	...
	for (b = 0; b < VIM; b++) {
	  for (j = 0; j < ei->dof[var]; j++) {
	     ...
	     */
	    pmt->d_diff_flux_dpmv[i_pl][a][i_pl][j] -= 
	      mp->density * mp->perm_tensor[a][b] / mp->viscosity *
	      bf[var]->grad_phi[j][b]; 
	  }
	}
      }
	...
	for (a = 0; a < VIM; a++) {
	  for (b = 0; b < VIM; b++) {
	    for (j = 0; j < ei->dof[var]; j++) {
	       ...
	       */
	      pmt->d_diff_flux_dpmv[i_pl][a][i_pore][j] -= 
		mp->density * mp->d_perm_tensor[a][b][POR_POROSITY] * bf[var]->phi[j] / mp->viscosity *
		fv->grad_p_liq[b]; 
	    }
	  }
	}
	...
	for (j = 0; j < ei->dof[var]; j++) {
	  for (a = 0; a < VIM; a++) {
		...
		pmt->d_diff_flux_dmesh[i_pl][a][b][j] -=
	      ...
	      for (p = 0; p < VIM; p++) {
		pmt->d_diff_flux_dmesh[i_pl][a][b][j] -= 
		    mp->density * mp->perm_tensor[a][p] / mp->viscosity * 
		...
		if(mp->PermeabilityModel == ORTHOTROPIC ||
		  ...
		  {
		    pmt->d_diff_flux_dmesh[i_pl][a][b][j] -= 
		      mp->density * mp->d_perm_tensor_dx[a][p][b][j] / mp->viscosity * 
		      fv->grad_p_liq[p];
		  }
	      }
	    }
    ...
    var= POR_POROSITY;
    if (pd->v[0][var] && pd->TimeIntegration != STEADY) 
      {
	for(a = 0; a < VIM; a++)
	  {      
	    for (j = 0; j < ei->dof[var]; j++) 
    ...
    if (pd->v[0][POR_POROSITY] && pd->TimeIntegration != STEADY) {
      for (b=0; b < pd->Num_Dim; b++) {
	var = MESH_DISPLACEMENT1 + b;
	phi_ptr = bf[var]->phi;
	if (pd->v[0][var]) {
	...
	for(b=0; b<pd->Num_Dim; b++)
	  { 
		...
		for ( j=0; j<ei->dof[var]; j++)
		  {
		    pmt->d_MassSource_dmesh[i_pl][b][j] += fv->d_volume_change_dx[b][j] *
		                                              mp->u_porous_sink_constants[0]*
	                                                      mp->u_porous_sink_constants[2]*
	                                    (mp->u_porous_sink_constants[1] - fv->sink_mass)/
	                                     mp->u_porous_sink_constants[1]/
		                             fv->volume_change/fv->volume_change ;
...
void 
porous_mass_flux_surf_bc(double func[DIM],
	  ...
	  for (w1 = 0; w1 < pd->Num_Porous_Eqn; w1++ )
		...
		+ mp->d_porosity[first_porous_var + w1] * (pmv->gas_density_solvents[0]  - Y_c))
		  * mass_tran_coeff * phi_j;
	      for (p=0; p<dim; p++)
		{
		  d_func[0][first_porous_var + w1][j_id] -= fv->snormal[p] * (
				d_vconv->v[p][w1][j_id] * pmv->bulk_density[0]
			      + vconv[p] * pmv->d_bulk_density[0][first_porous_var + w1] * phi_j);
		}
	    }
	  if(fv->p_liq >= p_0)
	    {
	      d_func[0][first_porous_var][j_id] -= mp->porosity *
		mass_tran_coeff1 * phi_j;
	    }
	}
	  ...
	  for (j_id = 0; j_id < ei->dof[var]; j_id++) 
	    {
	      ...
	      d_func[0][var][j_id] -= mp->porosity * 
		mass_tran_coeff * pmv->d_gas_density_solvents[0][var]
		  * phi_j;
	      for (p=0; p<dim; p++)
		{
		  d_func[0][var][j_id] -= fv->snormal[p] * (
				d_vconv->T[p][j_id] * pmv->bulk_density[0]
      ...
      for (a=0; a<dim; a++)
	{
	  ...
	  if (pd->v[0][var])
	      ...
	      for (j_id = 0; j_id < ei->dof[var]; j_id++) 
		{
		  ...
		  for (p=0; p<dim; p++)
		    {
		      d_func[0][var][j_id] -= pmv->bulk_density[0] * (
				fv->dsnormal_dx[p][a][j_id] * vconv[p] +
...
void 
porous_convection_bc (double func[DIM],
		      double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		      int wspec,               /* species number of this boundary condition    */
		      dbl dt,                  /* current value of the time step               */
		      dbl tt)                  /* parameter to vary time integration from */
  ...
  if (af->Assemble_Jacobian ) 
	...
	if (i_pore != -1) {
	  for (j_id = 0; j_id < ei->dof[var]; j_id++) {
		...
		for (p=0; p<dim; p++)
		  {
		    d_func[0][first_porous_var + i_pore][j_id] += fv->snormal[p]  * mp->density
		      * (
			 d_vconv->v[p][i_pore][j_id] );
		  }
	  }
	}
      } else {
	...
	for ( j_id=0; j_id<ei->dof[var]; j_id++)
	  {
		...
		for ( a=0; a<VIM; a++)
		  {
		    d_func[0][var][j_id] -= 
			pmv->d_rel_mass_flux_dT[i_pl][a][j_id] / 
			dens_vap * fv->snormal[a];
		  }			
	      }
	  }
			...
			for(w=0; w<pd->Num_Porous_Eqn; w++) 
			  {
				...
				pmv->d_rel_mass_flux_dpmv[i_pl][a] [w][j_id] 
			      / dens_vap * fv->snormal[a];			
			  }
		      }
		  }
			...
			for ( a=0; a<VIM; a++)
			  {
			    /*     d( )/dx        */
			    d_func[0][var][j_id] -=
				pmv->d_rel_mass_flux_dpmv[i_pl][a] [jvar][j_id] 
			      / dens_vap * fv->snormal[a] + pmv->rel_mass_flux[i_pl][a] 
			      / dens_vap * fv->dsnormal_dx[a] [jvar][j_id];			
			  }
		      }
		  }
	      }
...
void 
put_gas_flux_in_pores (double func[],
		       double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		       double x_dot[DIM], /* mesh velocity vector        */
		       dbl tt, /* parameter to vary time integration from 
				* explicit (tt = 1) to implicit (tt = 0) */
		...
		for ( a=0; a<VIM; a++)
		  {
		    for(w=0; w<pd->Num_Porous_Eqn; w++) 
		      {
			d_func[0][POR_LIQ_PRES + w][j_id] += 
			    pmv->d_rel_mass_flux_dpmv[wspec][a] [w][j_id] 
			  / dens_vap * fv->snormal[a];			
		      }
		  }
	      }
		...
		if (pd->v[0][var])
		  {
			...
			/*     d( )/dx        */
			d_func[0][var][j_id] +=
			    pmv->d_rel_mass_flux_dmesh[wspec][a] [jvar][j_id] 
			    / dens_vap * fv->snormal[a] 
			  + pmv->rel_mass_flux[wspec][a] 
			    / dens_vap * fv->dsnormal_dx[a] [jvar][j_id];			
		      }
		  }
	      }
   ...
   */
  else if (Current_EB_ptr->Elem_Blk_Id == eb_mat_fluid)
    {
      ...
      *func = 0;
      for ( a=0; a<VIM; a++)
	{
	  *func += fv->snormal[a] * ( -mp->thermal_conductivity * fv->grad_T[a]) 
	    / (1 - vapor_recoil * fv->T);
	}
	...
	for ( j_id=0; j_id<ei->dof[var]; j_id++)
	  {
		...
		for(a=0; a<dim; a++) 
		  {
		    d_func[0][var][j_id] -= mp->thermal_conductivity * fv->snormal[a] 
...
void 
porous_vapor_equil_bc(double func[],
		      double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
		      double x_dot[DIM], /* mesh velocity vector        */
		      dbl tt, /* parameter to vary time integration from 
			       * explicit (tt = 1) to implicit (tt = 0) */
  ...
  if (Current_EB_ptr->Elem_Blk_Id == eb_mat_solid) {
    ...
    *func = mp->porous_vapor_pressure[wspec] / amb_pres;
	...
	if (pd->v[0][var]) {
	  ...
	  for (w = 0; w < pd->Num_Porous_Eqn; w++)  {
	    d_func[0][POR_LIQ_PRES + w][j_id] += 
		mp->d_porous_vapor_pressure[wspec][POR_LIQ_PRES + w]
		/ amb_pres * phi_j;			
	  }
 	}
      }
  ...
  if (mp->PermeabilityModel == CONSTANT || 
      ...
      if (pd->v[0][POR_POROSITY])
	{
	  mp->d_permeability[POR_POROSITY] = 
	    ...
	    + pow(mp->porosity, 3.0) * 
	          2.0*mp->u_permeability[0]*pow(mp->u_permeability[1], 2.0)*(1.0-mp->porosity))/
	    (pow(pow(1. - mp->porosity, 2.0)*mp->u_permeability[0]*pow(mp->u_permeability[1], 2.0),2.0));
	   ...
	   mp->PermeabilityModel == PSD_SEXP )
    {
       ...
       * of the porosity */
      factor = mp->u_permeability[1] 
	* pow(1.-mp->u_permeability[0],1./3.) / pow(mp->u_permeability[0],0.5);
      ...
      for (i=0; i<MAX_VARIABLE_TYPES + MAX_CONC; i++)
	{
      ...
      /* Permeablity only depends on porosity currently */
      if (pd->v[0][POR_POROSITY]) {
	pmv->d_r_pore[POR_POROSITY] = factor * mp->d_porosity[POR_POROSITY] *
	  (
      ...
      if (pd->TimeIntegration == TRANSIENT)
	{
	  ...
	  if (pd->v[0][POR_POROSITY]) {
	    pmv_old->d_r_pore[POR_POROSITY] = factor * mp_old->d_porosity[POR_POROSITY] *
	      (
	...
	{
	  factor = mp->u_permeability[3] / 60. 
	    * (1. - pow(mp->u_permeability[2], 3.0)) / (1. - mp->u_permeability[2]);
	}
      ...
      else if (mp->PermeabilityModel == PSD_SEXP)
	{
      ...
      mp->permeability = factor * mp->porosity 
	* pmv->r_pore * pmv->r_pore;
      
      /* Permeablity only depends on porosity currently */
      if (pd->v[0][POR_POROSITY]) {
	mp->d_permeability[POR_POROSITY] = factor *
	  (
	   mp->d_porosity[POR_POROSITY] * pmv->r_pore * pmv->r_pore
     ...
     *************************************************************************/
{
  ...
  memset(mp->d_perm_tensor, 0, sizeof(double)*DIM*DIM*(MAX_VARIABLE_TYPES+MAX_CONC+MAX_PMV));
  ...
  if (mp->PermeabilityModel == ORTHOTROPIC)
	 ...
	 if (pd->v[0][POR_SINK_MASS])
	   {
	     ...
	     mp->d_perm_tensor[1][1][POR_SINK_MASS] =  mp->d_perm_tensor[0][0][POR_SINK_MASS]*mp->u_permeability[4];
	     mp->d_perm_tensor[2][2][POR_SINK_MASS] =  mp->d_perm_tensor[0][0][POR_SINK_MASS]*mp->u_permeability[5];
  ...
  else if (mp->PermeabilityModel == KC_TENSOR)
    {
       ...
       */
      K11 = pow(mp->porosity, 3.0)/pow(1. - mp->porosity, 2.0)
	/mp->u_permeability[0]/pow(mp->u_permeability[1], 2.0);
      ...
      if (pd->v[0][POR_POROSITY])
	{
	  mp->d_perm_tensor[0][0][POR_POROSITY] =
	    ...
	    + pow(mp->porosity, 3.0) * 
	     ...
	     mp->d_perm_tensor[1][1][POR_POROSITY] =  mp->d_perm_tensor[0][0][POR_POROSITY]*mp->u_permeability[2];

	     if(pd->Num_Dim == 3)
	     mp->d_perm_tensor[2][2][POR_POROSITY] =  mp->d_perm_tensor[0][0][POR_POROSITY]*mp->u_permeability[3];
  ...
  if (!pd->e[0][R_MESH1] && !efv->ev_porous_decouple)
    {
      for(aa = 0; aa < DIM; aa++)
	{
	  for(bb = 0; bb < DIM; bb++)
	    {	
	      mp->perm_tensor[aa][bb] =  a[0][aa]*a[0][bb]*K11 + 
	                               	 a[1][aa]*a[1][bb]*K22 + 
		                         a[2][aa]*a[2][bb]*K33 ;
	    }
	}
    }
  else if(pd->e[0][R_MESH1] && !efv->ev_porous_decouple)
    {
      for(aa = 0; aa < VIM; aa++)
	{
	  for(bb = 0; bb < VIM; bb++)
		...
		{	
		  ...
		  if(af->Assemble_Jacobian)
			...
			{	
			  ...
			  for ( j=0; j<ei->dof[var]; j++)
			    { 
			      d_a1_dot_F_dx[cc][dd][j] += a[0][aa]*fv->d_deform_grad_dx[bb][cc][dd][j]*delta(aa,bb);
			      d_a2_dot_F_dx[cc][dd][j] += a[1][aa]*fv->d_deform_grad_dx[bb][cc][dd][j]*delta(aa,bb);
			      d_a3_dot_F_dx[cc][dd][j] += a[2][aa]*fv->d_deform_grad_dx[bb][cc][dd][j]*delta(aa,bb);
			    }
		...
		{	
		  for(dd = 0; dd < VIM; dd++)
		    {	
		      ...
		      for ( j=0; j<ei->dof[var]; j++)
			{ 
			  mp->d_perm_tensor_dx[aa][bb][dd][j] = 
				 d_a1_dot_F_dx[aa][dd][j]*a1_dot_F[bb]*K11 + 
  ...
  else if(efv->ev_porous_decouple)
    {
      ...
      WH(-1,"BIG WARNING! Any significant deformation will cast doubt on these results as we have no Def Grad Tens for OrthoPerms");
...
void 
load_shell_permeability_tensor(void)
     ...
     *************************************************************************/
{
  ...
  if (mp->PermeabilityModel == SHELL_CYLINDER_SQUARE) {
    ...
    /* Parse properties */
    if (e > 0.3) WH(-1,"Your cylinder density is out of range.  Model only applicable for e < 0.3.  SHELL_CYLINDER_SQUARE");
...
double 
load_saturation(double porosity, double cap_pres, double d_cap_pres[2])
     ...
     ************************************************************************/
{
  ...
  if (mp->SaturationModel == VAN_GENUCHTEN) {
    ...
    if (cap_pres <= 0.0)  {
      ...
      saturation = mp->saturation = mp->u_saturation[0] + 
	...
	-(1. - mp->u_saturation[0] - mp->u_saturation[1]) *
	expon2 * pow((1. + pow(suction, mp->u_saturation[2])), expon2 - 1.0)
	...
	* pow(suction, mp->u_saturation[2] - 1);
	...
	mp->d_d_saturation[POR_LIQ_PRES][POR_LIQ_PRES] = 
	  -(1. - mp->u_saturation[0] - mp->u_saturation[1]) * expon2 
	  * mp->u_saturation[2] 
	  * pow(mp->u_saturation[3], 2.0) * 
	   ...
	   * (mp->u_saturation[2] - 1.) *
	   pow(suction, mp->u_saturation[2] - 2.0));
	if (pd->e[0][R_POR_GAS_PRES]) {
	...
	mp_old->saturation = mp->u_saturation[0] + 
	  (1. - mp->u_saturation[0] - mp->u_saturation[1]) *
  ...
  } else if(mp->SaturationModel == PSD_VOL  ||
	    mp->SaturationModel == PSD_WEXP ||
    ...
    if (pmv->cap_pres > 1.0e-20) {
      pmv->r_cap = 2. * mp->u_saturation[0] * cos(mp->u_saturation[1]) /
	pmv->cap_pres;
    } else {
      pmv->r_cap = 2.0 * mp->u_saturation[0] * cos(mp->u_saturation[1]) / 
	1.0e-20;
    }
    ...
    if (pmv->cap_pres > 1.0e-20) {
      ...
      pmv->d_d_r_cap[POR_LIQ_PRES][POR_LIQ_PRES] = -2.0 *
	pmv->d_r_cap[POR_LIQ_PRES] / pmv->cap_pres * d_cap_pres[i_pl];
      
      if (pd->e[0][R_POR_GAS_PRES]) {
	pmv->d_r_cap[POR_GAS_PRES] = -pmv->r_cap / pmv->cap_pres * d_cap_pres[i_pg];
	pmv->d_d_r_cap[POR_GAS_PRES][POR_GAS_PRES] = -2.  
	  * pmv->d_r_cap[POR_GAS_PRES]
	  / pmv->cap_pres * d_cap_pres[i_pg];
	pmv->d_d_r_cap[POR_GAS_PRES][POR_LIQ_PRES] = -2.  
	  * pmv->d_r_cap[POR_GAS_PRES]
	  / pmv->cap_pres * d_cap_pres[i_pl];
	pmv->d_d_r_cap[POR_LIQ_PRES][POR_GAS_PRES] = 
	...
	WH(-1, "adsorption allowed with negative Hamaker constant, disjoining");
	pmv->r_cap += pow(- mp->u_saturation[2] / pmv->cap_pres , 1./3.);
      }
      if (mp->u_saturation[2] > 0) {
	WH(-1, "adsorption allowed with positive Hamaker constant, conjoining");
	pmv->r_cap -= pow(mp->u_saturation[2] / pmv->cap_pres , 1./3.);
	if (pmv->r_cap < 0) pmv->r_cap = 0;
      }
      
      /* check for adsorbed film (assume additive thickness) */
      if (mp->u_saturation[2] < 0) {
	pmv->d_r_cap[POR_GAS_PRES] += 1./3. * pow(-mp->u_saturation[2], 1./3.)
	  / pow(pmv->cap_pres , 2./3.) 
	  * d_cap_pres[i_pl];
	if (pd->e[0][R_POR_GAS_PRES]) pmv->d_r_cap[POR_GAS_PRES] += 
				     1./3. * pow(-mp->u_saturation[2], 1./3.) 
				     / pow(pmv->cap_pres , 2./3.) 
				     * d_cap_pres[i_pg];
      }
      if (mp->u_saturation[2] > 0) {
	...
	if (pmv->r_cap == 0) {
	  ...
	  if (pd->e[0][R_POR_GAS_PRES]) pmv->d_r_cap[POR_GAS_PRES] = 0;
	}
      }
      
      /* check for adsorbed film (assume additive thickness) */
      if (mp->u_saturation[2] < 0) {
	pmv->d_d_r_cap[POR_LIQ_PRES][POR_LIQ_PRES] -= 2./9. 
	  * pow(-mp->u_saturation[2], 1./3.) / pow(pmv->cap_pres , 5./3.);
	if (pd->e[0][R_POR_GAS_PRES]) {
	  pmv->d_d_r_cap[POR_GAS_PRES][POR_GAS_PRES] -= 2./9. 
	    * pow(-mp->u_saturation[2], 1./3.) / pow(pmv->cap_pres , 5./3.);
	  pmv->d_d_r_cap[POR_GAS_PRES][POR_LIQ_PRES] -= 2./9. 
	    * pow(-mp->u_saturation[2], 1./3.) / pow(pmv->cap_pres , 5./3.) 
	...
	pmv_old->r_cap = 2. * mp->u_saturation[0] * cos(mp->u_saturation[1]) 
	  / pmv_old->cap_pres;
      } else {
	pmv_old->r_cap = 2. * mp->u_saturation[0] * cos(mp->u_saturation[1]) / 1e-20;
      }
      if (pmv_old->cap_pres > 1e-20) {
	pmv_old->d_r_cap[POR_LIQ_PRES] = -pmv_old->r_cap / pmv_old->cap_pres 
	  * d_cap_pres[i_pl];
	if (pd->e[0][R_POR_GAS_PRES]) pmv_old->d_r_cap[POR_GAS_PRES] = 
				     -pmv_old->r_cap / pmv_old->cap_pres * d_cap_pres[i_pg];
      } else {
	pmv_old->d_r_cap[POR_LIQ_PRES] = 0;
	if (pd->e[0][R_POR_GAS_PRES]) pmv_old->d_r_cap[POR_GAS_PRES] = 0;
      }
    }
	 ...
	 */
      ...
      if (pd->e[0][R_POR_POROSITY]) r_pore = pmv->r_pore;
      if (pmv->r_cap > r_pore) {
	saturation = mp->saturation = 1.0;
      } else if (pmv->r_cap < mp->u_permeability[2] * r_pore) {
	saturation = mp->saturation = 0.0;
      } else {
	saturation = mp->saturation = ((pmv->r_cap / r_pore) 
				       - mp->u_permeability[2])
	  / (1. - mp->u_permeability[2]);
	
	mp->d_saturation[POR_LIQ_PRES] = pmv->d_r_cap[POR_LIQ_PRES] / r_pore
	  / (1. - mp->u_permeability[2]);
	if (pd->e[0][R_POR_GAS_PRES]) {
	  mp->d_saturation[POR_GAS_PRES] = 
	    pmv->d_r_cap[POR_GAS_PRES] / r_pore
	    / (1. - mp->u_permeability[2]);
	}
	if (pd->e[0][R_POR_POROSITY]) {
	  mp->d_saturation[POR_POROSITY] = 
	    - pmv->r_cap / r_pore / r_pore 
	...
	if (af->Assemble_Jacobian) {
	  mp->d_d_saturation[POR_LIQ_PRES][POR_LIQ_PRES] = 
	    pmv->d_d_r_cap[POR_LIQ_PRES][POR_LIQ_PRES] / r_pore
	    / (1. - mp->u_permeability[2]);
	  
	  if (pd->e[0][R_POR_GAS_PRES]) {
	    mp->d_d_saturation[POR_GAS_PRES][POR_GAS_PRES] = 
	      pmv->d_d_r_cap[POR_GAS_PRES][POR_GAS_PRES] / r_pore
	      / (1. - mp->u_permeability[2]);
	    mp->d_d_saturation[POR_GAS_PRES][POR_LIQ_PRES] = 
	      pmv->d_d_r_cap[POR_GAS_PRES][POR_LIQ_PRES] / r_pore
	      / (1. - mp->u_permeability[2]);
	    mp->d_d_saturation[POR_LIQ_PRES][POR_GAS_PRES] = 
	      pmv->d_d_r_cap[POR_LIQ_PRES][POR_GAS_PRES] / r_pore
	      / (1. - mp->u_permeability[2]);
	  }
	  
	  if (pd->e[0][R_POR_POROSITY]) {
	    mp->d_d_saturation[POR_POROSITY][POR_POROSITY] = 
	      -( 2 * mp->d_saturation[POR_POROSITY] / r_pore 
		 * pmv->d_r_pore[POR_POROSITY]
		 + pmv->r_cap / r_pore / r_pore * 
		 pmv->d_d_r_pore[POR_POROSITY][POR_POROSITY])
	      / (1. - mp->u_permeability[2]);
	    mp->d_d_saturation[POR_POROSITY][POR_LIQ_PRES] = 
	      - pmv->d_r_cap[POR_LIQ_PRES] / r_pore 
      ...
      if (pd->TimeIntegration == TRANSIENT) {
	r_pore = mp->u_saturation[3];
	if (pd->e[0][R_POR_POROSITY]) r_pore = pmv_old->r_pore;
	if (pmv_old->r_cap > r_pore) {
	  ...
	  mp_old->saturation = 
	    ((pmv_old->r_cap / r_pore) - mp->u_permeability[2]) /
	    (1.0 - mp->u_permeability[2]);
	  
	  mp_old->d_saturation[POR_LIQ_PRES] = 
	    pmv_old->d_r_cap[POR_LIQ_PRES] / r_pore
	    / (1.0 - mp->u_permeability[2]);
	  if (pd->e[0][R_POR_GAS_PRES]) mp_old->d_saturation[POR_GAS_PRES] = 
				       pmv_old->d_r_cap[POR_GAS_PRES] / r_pore
				       / (1.0 - mp->u_permeability[2]);
	  if (pd->e[0][R_POR_POROSITY]) mp_old->d_saturation[POR_POROSITY] = 
				       - pmv_old->r_cap / r_pore / r_pore 
				       * pmv_old->d_r_pore[POR_POROSITY]
				       / (1.0 - mp->u_permeability[2]);
	}
      }
    }
    else if (mp->SaturationModel == PSD_WEXP ||
	     mp->SaturationModel == PSD_SEXP) {
      r_pore = mp->u_saturation[3];
      if (pd->e[0][R_POR_POROSITY]) r_pore = pmv->r_pore;
	...
	saturation = mp->saturation = 1 - exp(-2. * rad) *
	...
	frac = 9. * M_PIE / 16.;
	saturation = mp->saturation = 1 - exp(-frac * rad * rad) *
	  ( 1. + frac * rad * rad );
	
	d_sat_d_rad = 
	  ( 2 * frac * rad * exp(-frac * rad *rad) *
	    ( 1. + frac * rad * rad )
	    - exp(-frac * rad * rad) *
	    (2 * frac * rad));
	
	d_d_sat_d_rad = - 2 * frac * rad * d_sat_d_rad 
	  - exp(-frac * rad * rad) * (2 * frac)
	  + 2 * frac * rad * exp(-frac * rad * rad) * (2 * frac * rad)
	  + 2 * frac * exp(-frac * rad *rad) * ( 1. + frac * rad * rad );
      }
      
      mp->d_saturation[POR_LIQ_PRES] = pmv->d_r_cap[POR_LIQ_PRES] 
	/ r_pore * d_sat_d_rad;
      if (pd->e[0][R_POR_GAS_PRES]) mp->d_saturation[POR_GAS_PRES] = 
				   - mp->d_saturation[POR_LIQ_PRES];
      if (pd->e[0][R_POR_POROSITY]) mp->d_saturation[POR_POROSITY] = 
				   - rad / r_pore * pmv->d_r_pore[POR_POROSITY] *
				   d_sat_d_rad;
	...
	mp->d_d_saturation[POR_LIQ_PRES][POR_LIQ_PRES] = 
	  ...
	  +  pmv->d_r_cap[POR_LIQ_PRES] / r_pore 
	  * d_d_sat_d_rad * pmv->d_r_cap[POR_LIQ_PRES] / r_pore;
	...
	if (pd->e[0][R_POR_GAS_PRES]) {
	  ...
	  mp->d_d_saturation[POR_LIQ_PRES][POR_GAS_PRES] = 
	    ...
	    +  pmv->d_r_cap[POR_LIQ_PRES] / r_pore * d_d_sat_d_rad 
	    * pmv->d_r_cap[POR_GAS_PRES];
	}
	...
	if (pd->e[0][R_POR_POROSITY]) {
	  mp->d_d_saturation[POR_POROSITY][POR_POROSITY] = 
	    - 2 * mp->d_saturation[POR_POROSITY] / r_pore 
	    * pmv->d_r_pore[POR_POROSITY]
	    -  rad / r_pore * d_sat_d_rad 
	    * pmv->d_d_r_pore[POR_POROSITY][POR_POROSITY]
	    + rad/r_pore * rad/r_pore * d_d_sat_d_rad * 
	    pmv->d_r_pore[POR_POROSITY] * pmv->d_r_pore[POR_POROSITY];
	  mp->d_d_saturation[POR_POROSITY][POR_LIQ_PRES] = 
	...
	if (mp->SaturationModel == PSD_WEXP) {	      
	  ...
	  mp_old->d_saturation[POR_LIQ_PRES] = pmv_old->d_r_cap[POR_LIQ_PRES] 
	    / r_pore * d_sat_d_rad;
	  if (pd->e[0][R_POR_GAS_PRES]) mp_old->d_saturation[POR_GAS_PRES] = 
				       - mp_old->d_saturation[POR_LIQ_PRES];
	  if (pd->e[0][R_POR_POROSITY]) mp_old->d_saturation[POR_POROSITY] = 
	...
	if (mp->SaturationModel == PSD_SEXP) {
	  frac = 9. * M_PIE / 16.;
	  mp_old->saturation = 1 - exp(-frac * rad * rad) *
	    ( 1. + frac * rad * rad );
   ...
   **********************************************************************/
     ...
     */
    mp->d_saturation[POR_LIQ_PRES] = con_b*con_d*(1.0-pow(tanh(con_c-con_d/cap_pres_clip),2.0))/
                                     cap_pres_clip/cap_pres_clip;
    if (pd->e[0][R_POR_GAS_PRES]) {
      mp->d_saturation[POR_GAS_PRES] = -mp->d_saturation[POR_LIQ_PRES];
    }

    if (pd->e[0][R_SHELL_SAT_OPEN])
      {
	/*Ignore the above sensitivities and use this one because you cannot have both*/
	mp->d_saturation[SHELL_PRESS_OPEN] = con_b*con_d*(1.0-pow(tanh(con_c-con_d/cap_pres_clip),2.0))/
                                     cap_pres_clip/cap_pres_clip;
      }

    if (pd->e[0][R_SHELL_SAT_OPEN_2])
      {
	/*Ignore the above sensitivities and use this one because you cannot have both*/
	...
	as I think it should be positive due to double derivative..*/
           mp->d_d_saturation[POR_LIQ_PRES][POR_LIQ_PRES] = 
	-(2.0*con_b*con_d*con_d/pow(cap_pres_clip,4.0)*tanh(con_c-con_d/cap_pres_clip)
	  *(1.0-pow(tanh(con_c-con_d/cap_pres_clip),2.0)) 
	  + 2.0*con_b*con_d/pow(cap_pres_clip,3.0)*(1.0-pow(tanh(con_c-con_d/cap_pres_clip),2.0)) );
      if (pd->e[0][R_POR_GAS_PRES]) {
	mp->d_d_saturation[POR_GAS_PRES][POR_LIQ_PRES] = -
	  mp->d_d_saturation[POR_GAS_PRES][POR_GAS_PRES];
	mp->d_d_saturation[POR_LIQ_PRES][POR_GAS_PRES] = 
     ...
     * const = fv->external_field[efv->ev_SAT_index] * (mp->u_saturation[0] - mp->u_saturation[4]) - mp->u_saturation[4])
   */
    /*  
     * FOR TANH EQUATION
     *  mp->u_saturation[0] is the irreduceable water saturation
    ...
    EH(i_sat_ev, "Saturation external field not found!");
    con_c=fv->external_field[i_sat_ev] * (mp->u_saturation[2] - mp->u_saturation[6]) +  mp->u_saturation[6];
    con_d=fv->external_field[i_sat_ev] * (mp->u_saturation[3] - mp->u_saturation[7]) +  mp->u_saturation[7];
    con_a = (mp->u_saturation[0] + tanh(con_c))/(1. + tanh(con_c));
    con_b = (1. - mp->u_saturation[0])/(1. + tanh(con_c));
   ...
   **********************************************************************/
    ...
    if(Element_Blocks[ei->elem_blk_index].ElemStorage[mat_ielem].sat_curve_type[ip] == 1.0) 
      {
	alpha_drain = mp->u_saturation[7];
	beta_drain  = mp->u_saturation[6];
	s_min       = mp->u_saturation[4];
	
	sat_switch = Element_Blocks[ei->elem_blk_index].ElemStorage[mat_ielem].Sat_QP_tn[ip];
	pc_switch = Element_Blocks[ei->elem_blk_index].ElemStorage[mat_ielem].p_cap_QP[ip];
	...
	con_b = (sat_switch-s_min)/
	  (-1 + tanh(beta_drain - alpha_drain/pc_switch_clip));
	con_a = s_min - con_b;
    ...
    if(Element_Blocks[ei->elem_blk_index].ElemStorage[mat_ielem].sat_curve_type[ip] == 1.0)
      {
	mp->d_saturation[POR_LIQ_PRES] = -con_b*alpha_drain/cap_pres_clip/cap_pres_clip*
	...
	if(Element_Blocks[ei->elem_blk_index].ElemStorage[mat_ielem].sat_curve_type[ip] == 1.0)
	  {
	    mp->d_saturation[SHELL_PRESS_OPEN] = -con_b*alpha_drain/cap_pres_clip/cap_pres_clip*
	      (1-pow(tanh(beta_drain-alpha_drain/cap_pres_clip),2.0));	   
	  }
	else
	  {
	    mp->d_saturation[SHELL_PRESS_OPEN] = -con_b*alpha_wet/cap_pres_clip/cap_pres_clip*
	      (1-pow(tanh(beta_wet-alpha_wet/cap_pres_clip),2.0));
	  }
      }
	...
	if(Element_Blocks[ei->elem_blk_index].ElemStorage[mat_ielem].sat_curve_type[ip] == 1.0)
	  {
	    mp->d_saturation[SHELL_PRESS_OPEN_2] = -con_b*alpha_drain/cap_pres_clip/cap_pres_clip*
	      (1-pow(tanh(beta_drain-alpha_drain/cap_pres_clip),2.0));
	  }
	else
	  {
	    mp->d_saturation[SHELL_PRESS_OPEN_2] = -con_b*alpha_wet/cap_pres_clip/cap_pres_clip*
	      (1-pow(tanh(beta_wet-alpha_wet/cap_pres_clip),2.0));
	  }
      }
      ...
      if(Element_Blocks[ei->elem_blk_index].ElemStorage[mat_ielem].sat_curve_type[ip] == 1.0)
	{
	  ...
	  mp_old->d_saturation[POR_LIQ_PRES] = con_b*alpha_drain/cap_pres_clip/cap_pres_clip*
	    (1-pow(tanh(beta_drain-alpha_drain/cap_pres_clip),2.0));
      ...
      else
	{
	  ...
	  mp_old->d_saturation[POR_LIQ_PRES] = -con_b*alpha_wet/cap_pres_clip/cap_pres_clip*
	    (1-pow(tanh(beta_wet-alpha_wet/cap_pres_clip),2.0));
	  if (pd->e[0][R_POR_GAS_PRES]) {
	    mp_old->d_saturation[POR_GAS_PRES] = -mp->d_saturation[POR_LIQ_PRES];
	  }
     ...
     ************************************************************************/
{
  ...
  if (mp->PorousVaporPressureModel[i_pl] == KELVIN  && 
	...
	|| mp->PorousMediaType == POROUS_SHELL_UNSATURATED))
    {
       ...
       */
      if (pd->e[0][R_POR_GAS_PRES]) Pg = fv->p_gas; else Pg =  mp->u_porous_gas_constants[3];
      if (pd->e[0][R_POR_ENERGY]) {
          T = fv->T; 
      ...
      if (pd->TimeIntegration == TRANSIENT) {
         if (pd->e[0][R_POR_GAS_PRES]) Pg_old = fv_old->p_gas; 
         else Pg_old = Pg;
         if (pd->e[0][R_POR_ENERGY]) {
            T_old = fv_old->T;
      ...
      /* precalculate some parameters */
       ...
       m_rtw = mp->u_porous_vapor_pressure[i_pl][2]
	           / mp->u_porous_vapor_pressure[i_pl][3] / T;

      if (pd->e[0][R_POR_GAS_PRES])
	{	 
	  m_rta =   mp->u_porous_gas_constants[0]
	    / mp->u_porous_gas_constants[1] / T;
      ...
      if (pd->e[0][R_POR_ENERGY])
	{
      ...
      if (pd->TimeIntegration == TRANSIENT)
	{
          m_rtw_old = mp->u_porous_vapor_pressure[i_pl][2]
	               / mp->u_porous_vapor_pressure[i_pl][3] / T_old;
	  m_rta_old = mp->u_porous_gas_constants[0]
	              / mp->u_porous_gas_constants[1] / T_old;
	  ...
	  pmv_old->gas_density_solvents[i_pl] = rhosat_old *
	      exp(-pmv_old->cap_pres * m_rtw_old / rho_l );
	  mp_old->porous_vapor_pressure[i_pl] = pmv_old->gas_density_solvents[i_pl] / 
              m_rtw_old;

          if (pd->e[0][R_POR_GAS_PRES])
              pmv_old->gas_density_solvents[i_pg] =  Pg_old * m_rta_old - 
	          pmv_old->gas_density_solvents[i_pl] * ma_mw; 
      ...
      if (pd->e[0][R_POR_ENERGY]) 
	{
	  ...
	  if (pd->e[0][R_POR_GAS_PRES]) {
      	      pmv->d_gas_density_solvents[i_pg][POR_TEMP] = -Pg*m_rta/T - 
	                 pmv->d_gas_density_solvents[i_pl][POR_TEMP]*ma_mw;
      ...
      if (pd->TimeIntegration == TRANSIENT)
	{
	  pmv_old->d_gas_density_solvents[i_pl][POR_LIQ_PRES] = 
	           -pmv_old->gas_density_solvents[i_pl] *
	            m_rtw_old / rho_l * d_cap_pres[i_pl];
	  ...
	  if (pd->e[0][R_POR_GAS_PRES]) 
		...
		pmv_old->d_gas_density_solvents[i_pl][POR_GAS_PRES] / m_rtw_old;
	      ...
	      pmv_old->d_gas_density_solvents[i_pg][POR_GAS_PRES] = m_rta_old - 
		pmv_old->d_gas_density_solvents[i_pl][POR_GAS_PRES] * ma_mw;
          ...
          if (pd->e[0][R_POR_ENERGY]) 
	    {
            ...
            /* Add energy terms to water eqn senstivities */
              term_1o = pmv_old->gas_density_solvents[i_pl] * 
                        pc_rhort_old/T_old;
	   ...
	   /* Add energy terms to air eqn sensitivies */
	      if (pd->e[0][R_POR_GAS_PRES]) {
      	          pmv_old->d_gas_density_solvents[i_pg][POR_TEMP] = 
                           -Pg_old*m_rta_old/T_old - 
	                    pmv_old->d_gas_density_solvents[i_pl][POR_TEMP]*ma_mw;
	          pmv_old->d_gas_density_solvents[i_pe][POR_GAS_PRES] = 
                       pmv_old->gas_density_solvents[i_pl] * 
                       pmv_old->d_enthalpy[1][POR_GAS_PRES] +
                       pmv_old->enthalpy[1] *
                       pmv_old->d_gas_density_solvents[i_pl][POR_GAS_PRES] +
                       pmv_old->gas_density_solvents[i_pg] *
                       pmv_old->d_enthalpy[2][POR_GAS_PRES] +
                       pmv_old->enthalpy[2] *
                       pmv_old->d_gas_density_solvents[i_pg][POR_GAS_PRES] - 1.0;
              }

	      pmv_old->d_gas_density_solvents[i_pe][POR_TEMP] = 
                       pmv_old->gas_density_solvents[i_pl] *
                       pmv_old->d_enthalpy[1][POR_TEMP] +
                       pmv_old->enthalpy[1] *
                       pmv_old->d_gas_density_solvents[i_pl][POR_TEMP] +
                       pmv_old->gas_density_solvents[i_pg] *
                       pmv_old->d_enthalpy[2][POR_TEMP] +
                       pmv_old->enthalpy[2] *
                       pmv_old->d_gas_density_solvents[i_pg][POR_TEMP];
	    }
	}
      ...
      if (af->Assemble_Jacobian)
	{
	  pmv->d_d_gas_density_solvents[i_pl][POR_LIQ_PRES][POR_LIQ_PRES] = 
	    pmv->gas_density_solvents[i_pl] * m_rtw * m_rtw
	    / rho_l / rho_l * d_cap_pres[i_pl] * d_cap_pres[i_pl];
	  ...
	  if (pd->e[0][R_POR_GAS_PRES]) 
		...
		pmv->gas_density_solvents[i_pl] * m_rtw * m_rtw
		/ rho_l / rho_l * d_cap_pres[i_pl] * d_cap_pres[i_pg];
             /* replace above with the next statement */
              pmv->d_d_gas_density_solvents[i_pl][POR_LIQ_PRES][POR_GAS_PRES] = 
                   pmv->d_gas_density_solvents[i_pl][POR_GAS_PRES] *
                   pmv->d_gas_density_solvents[i_pl][POR_LIQ_PRES] / 
	      ...
	      pmv->d_d_gas_density_solvents[i_pl][POR_GAS_PRES][POR_LIQ_PRES] = 
		   pmv->gas_density_solvents[i_pl] * m_rtw * m_rtw
		   / rho_l / rho_l * d_cap_pres[i_pg] * d_cap_pres[i_pl];
             /* replace above with the next statement */
              pmv->d_d_gas_density_solvents[i_pl][POR_GAS_PRES][POR_LIQ_PRES] = 
                   pmv->d_gas_density_solvents[i_pl][POR_LIQ_PRES] *
                   pmv->d_gas_density_solvents[i_pl][POR_GAS_PRES] / 
	      ...
	      pmv->d_d_gas_density_solvents[i_pl][POR_GAS_PRES][POR_GAS_PRES] = 
		   pmv->gas_density_solvents[i_pl] * m_rtw * m_rtw
		   / rho_l / rho_l * d_cap_pres[i_pg] * d_cap_pres[i_pg];
             /* replace above with the next statement */
              pmv->d_d_gas_density_solvents[i_pl][POR_GAS_PRES][POR_GAS_PRES] = 
                   pmv->d_gas_density_solvents[i_pl][POR_GAS_PRES] *
                   pmv->d_gas_density_solvents[i_pl][POR_GAS_PRES] / 
          ...
          /* Add energy terms to water eqn senstivities */
              pmv->d_d_gas_density_solvents[i_pl][POR_LIQ_PRES][POR_TEMP] = 
                   ...
                   (pmv->d_gas_density_solvents[i_pl][POR_TEMP] / 
                    pmv->gas_density_solvents[i_pl] - 1.0/T);
...
void
load_gas_conc_flat(double porosity, double cap_pres, double d_cap_pres[2])
      ...
      */
{
  ...
  if (mp->PorousVaporPressureModel[i_pl] == FLAT  && 
      ...
      if (pd->TimeIntegration == TRANSIENT)
	{
      ...
      pmv->d_gas_density_solvents[i_pl][POR_LIQ_PRES] = 
                       rho_sat * mp->d_saturation[POR_LIQ_PRES];
      mp->d_porous_vapor_pressure[i_pl][POR_LIQ_PRES] = 
	    pmv->d_gas_density_solvents[i_pl][POR_LIQ_PRES] / m_rtw;
      ...
      if (pd->TimeIntegration == TRANSIENT)
	{
	  ...
	  if (pd->e[0][R_POR_GAS_PRES]) 
          ...
          if (pd->e[0][R_POR_ENERGY]) 
	    {
            ...
            /* Add energy terms to water eqn senstivities */
              pmv_old->d_gas_density_solvents[i_pl][POR_TEMP] = 
                            drhosat_dT_old * mp_old->saturation;
              pmv_old->d_gas_density_solvents[i_pe][POR_LIQ_PRES] = 
                       pmv_old->enthalpy[1] * 
                       pmv_old->d_gas_density_solvents[i_pl][POR_LIQ_PRES] +
                       pmv_old->enthalpy[2] *
	  ...
	  if (pd->e[0][R_POR_ENERGY]) 
	    {
          /* Add energy terms to water eqn senstivities */
              ...
              pmv->d_d_gas_density_solvents[i_pl][POR_TEMP][POR_TEMP] = 
                   drhosat_dT * mp->d_saturation[POR_TEMP] +
                   d_drhosat_dT * mp->saturation;
...
/*****************************************************************************/
void
load_gas_conc_EOS(double porosity, double cap_pres, double d_cap_pres[2])
      ...
      */
{
  ...
  if (mp->PorousVaporPressureModel[i_pl] == FLAT  && 
     ...
     */
      ...
      if (pd->e[0][R_POR_GAS_PRES]) Pg = fv->p_gas; else Pg =  mp->u_porous_gas_constants[3];
      if (Pg == 0.) return;
      if (pd->e[0][R_POR_ENERGY]) T = fv->T; else T = mp->u_porous_vapor_pressure[i_pl][4] - 273.15;

      /* precalculate some parameters */

      rho_gas = calc_rho_gas(Pg, T, &Pvsat, &dPvsat_dT, &drho_gas_dPg, &drho_gas_dT, 
                             d_drho_gas_dPg, d_drho_gas_dT, &rho_wg_sat,
                             &drho_wgsat_dT, &d_drho_wgsat_dT);

      Ywg = calc_Ywg(rho_wg_sat, drho_wgsat_dT, d_drho_wgsat_dT,
                     rho_gas, drho_gas_dPg, drho_gas_dT, d_drho_gas_dPg,
                     d_drho_gas_dT, &dYwg_dPg, &dYwg_dT, d_dYwg_dPg, d_dYwg_dT); 
      ...
      /*      printf("\t Pg=%g, T=%g, Pvsat=%g, rho_gas=%g, sat=%g, Ywg=%g \n",Pg,T,Pvsat,rho_gas,mp->saturation,Ywg);
	      printf("\t pmv->gas_density_solvents[i_pl]=%g \n", pmv->gas_density_solvents[i_pl]);   */
	...
	/* For now, assume binary so Yag = 1 - Ywg */
      ...
      pmv->d_gas_density_solvents[i_pl][POR_LIQ_PRES] = Ywg * rho_gas * mp->d_saturation[POR_LIQ_PRES];
      mp->d_porous_vapor_pressure[i_pl][POR_LIQ_PRES] = 0.0;
   ...
   if (pd->TimeIntegration == TRANSIENT) {
      ...
      if (pd->e[0][R_POR_GAS_PRES]) Pg = fv_old->p_gas; else Pg =  mp->u_porous_gas_constants[3];
      if (pd->e[0][R_POR_ENERGY]) T = fv_old->T; else T = mp->u_porous_vapor_pressure[i_pl][4]-273.15;
      ...
      if (pd->e[0][R_POR_ENERGY]) 
	{
      ...
      for (w1=0; w1<MAX_PMV; w1++)
         pmv_old->d_gas_density_solvents[i_pl][POR_LIQ_PRES+w1] =               
              pmv_old->d_Ywg[w1]*rho_gas*mp_old->saturation + Ywg*pmv_old->d_rhog[w1]*mp_old->saturation
      ...
      for (w1=0; w1<MAX_PMV; w1++)
         pmv_old->d_gas_density_solvents[i_pg][POR_LIQ_PRES+w1] =               
              pmv_old->d_Yag[w1]*rho_gas*mp_old->saturation + Yag*pmv_old->d_rhog[w1]*mp_old->saturation
...
void 
load_bulk_density(double porosity, double cap_pres,  double saturation, 
		  double d_cap_pres[2])
     ...
     ************************************************************************/
{
   ...
   */
  liquid_density = mp->density *
      (1 + mp->PorousLiqCompress * (fv->p_liq - mp->PorousLiqRefPress)); 
  ...
  if (pd->TimeIntegration == TRANSIENT) {
    liquid_density_old =  mp->density * 
	(1 + mp->PorousLiqCompress * (fv_old->p_liq - mp->PorousLiqRefPress));
  }

  for (w = 0; w < MAX_PMV; w++) {
    /*short circuit if the equation is not active */
    if(pd->e[0][R_POR_LIQ_PRES + w])
      {
	pmv->bulk_density[w] =
	  pmv->gas_density_solvents[w] * porosity * (1. - saturation) 
	  + liquid_density * pmv->liq_Xvol_solvents[w] * porosity * saturation;

	if (pd->TimeIntegration == TRANSIENT) {
	  pmv_old->bulk_density[w] =  
	    pmv_old->gas_density_solvents[w] * mp_old->porosity 
	    * (1. - mp_old->saturation) 
  ...
  if(pd->e[0][R_SHELL_SAT_OPEN] || pd->e[0][R_SHELL_SAT_OPEN_2] ) 
    {
  ...
  for (w = 0; w < MAX_PMV; w++) {
    ...
    if(pd->e[0][eqn] && pd->v[0][var])
      {
	pmv->d_bulk_density[w][POR_LIQ_PRES] = 
	  pmv->d_gas_density_solvents[w][POR_LIQ_PRES] * porosity *
	  (1. - saturation);
	pmv->d_bulk_density[w][POR_LIQ_PRES] +=
	  mp->d_saturation[POR_LIQ_PRES] * porosity * 
	  (liquid_density * pmv->liq_Xvol_solvents[w] - 
	   pmv->gas_density_solvents[w]);
	pmv->d_bulk_density[w][POR_LIQ_PRES] += 
	  d_liquid_density_dpliq * porosity * saturation * pmv->liq_Xvol_solvents[w];
	pmv->d_bulk_density[w][POR_LIQ_PRES] +=
	  mp->d_porosity[POR_LIQ_PRES] * 
	  (pmv->gas_density_solvents[w] * (1. - saturation) +
     ...
     */
	pmv->d_bulk_density[w][POR_LIQ_PRES] +=
	  pmv->d_liq_Xvol_solvents[w][POR_LIQ_PRES] * porosity * liquid_density *
	  saturation; 
	...
	pmv->d_bulk_density[w][POR_GAS_PRES] = 
	  pmv->d_gas_density_solvents[w][POR_GAS_PRES] * porosity * 
	  (1. - saturation);
	pmv->d_bulk_density[w][POR_GAS_PRES] += 
	 ...
	 */
	pmv->d_bulk_density[w][POR_GAS_PRES] +=
	  pmv->d_liq_Xvol_solvents[w][POR_GAS_PRES] * porosity * liquid_density *
	  saturation; 
	...
	pmv->d_bulk_density[w][POR_POROSITY] = 
	  pmv->d_gas_density_solvents[w][POR_POROSITY] * porosity *
	  (1. - saturation);
	pmv->d_bulk_density[w][POR_POROSITY] -= 
	  pmv->gas_density_solvents[w] * porosity * mp->d_saturation[POR_POROSITY];
	pmv->d_bulk_density[w][POR_POROSITY] += 
	  liquid_density * porosity * mp->d_saturation[POR_POROSITY] * 
	  pmv->liq_Xvol_solvents[w];
	pmv->d_bulk_density[w][POR_POROSITY] += mp->d_porosity[POR_POROSITY]
	  * ( pmv->gas_density_solvents[w] * (1. - saturation) 
	      + liquid_density * saturation * pmv->liq_Xvol_solvents[w]);
	 ...
	 */
	pmv->d_bulk_density[w][POR_POROSITY] +=
	  pmv->d_liq_Xvol_solvents[w][POR_POROSITY] * porosity * liquid_density *
	  saturation; 
      }
    /*
	...
	pmv->d_bulk_density[w][POR_TEMP] = 
	  pmv->d_gas_density_solvents[w][POR_TEMP] * porosity * 
	  (1. - saturation);
	 ...
	 */
	pmv->d_bulk_density[w][POR_TEMP] +=
	  pmv->d_liq_Xvol_solvents[w][POR_TEMP] * porosity * liquid_density *
	  saturation; 
  ...
  if (pd->e[0][R_POR_ENERGY]) {
    for (w1=0; w1<MAX_PMV; w1++) {
         pmv->d_bulk_density[i_pe][POR_LIQ_PRES + w1] += 
              -mp->d_porosity[w1] * rhoCp * fv->T;
    }
	...
	if(pd->e[0][eqn] && pd->v[0][var])
	  {
	    pmv_old->d_bulk_density[w][var] = 
	      pmv_old->d_gas_density_solvents[w][var] *
	      mp_old->porosity  * (1. - mp_old->saturation);
	    pmv_old->d_bulk_density[w][var] -= 
	      pmv_old->gas_density_solvents[w] * mp_old->porosity 
	      * mp_old->d_saturation[var];
	    pmv_old->d_bulk_density[w][var] += 
	      liquid_density_old * mp_old->porosity *
	      mp_old->d_saturation[var] 
	      * pmv_old->liq_Xvol_solvents[w];
	    pmv_old->d_bulk_density[w][var] += 
	      d_liquid_density_dpliq_old * mp_old->porosity *
	      mp_old->saturation  * pmv_old->liq_Xvol_solvents[w];
	    pmv_old->d_bulk_density[w][var] +=
	      mp_old->d_porosity[var] *
	      (pmv_old->gas_density_solvents[w] * (1. - mp_old->saturation) +
	       liquid_density_old * mp_old->saturation * 
	       pmv_old->liq_Xvol_solvents[w]);
	    /* variation of mass fraction of component w in the liquid phase:
	     *  d_liq_Xvol_solvents currently zero for all but energy eqn
	     */
	    pmv_old->d_bulk_density[w][var] +=
	      pmv_old->d_liq_Xvol_solvents[w][var] * mp_old->porosity * 
	      liquid_density_old * mp_old->saturation; 
  ...
  if (af->Assemble_Jacobian) {
    ...
    for  (w=0; w<MAX_PMV; w++) mp->d_density[w] = 0.0;
	...
	for (w2=0; w2<MAX_PMV; w2++) {
	  ...
	  if(pd->e[0][eqn] && pd->v[0][var1] && pd->v[0][var2])
	    {
	      pmv->d_d_bulk_density[w][POR_LIQ_PRES + w1][POR_LIQ_PRES + w2] = porosity * 
		(
		 pmv->d_d_gas_density_solvents[w][POR_LIQ_PRES + w1][POR_LIQ_PRES + w2] * (1. - saturation)
		  - pmv->d_gas_density_solvents[w][POR_LIQ_PRES + w1] * mp->d_saturation[POR_LIQ_PRES + w2]
		  - pmv->d_gas_density_solvents[w][POR_LIQ_PRES + w2] * mp->d_saturation[POR_LIQ_PRES + w1]
		  - pmv->gas_density_solvents[w] * mp->d_d_saturation[POR_LIQ_PRES + w1][POR_LIQ_PRES + w2]
		  + liquid_density * 
                     (mp->d_d_saturation[POR_LIQ_PRES + w1][POR_LIQ_PRES + w2] * pmv->liq_Xvol_solvents[w]
                      + mp->d_saturation[POR_LIQ_PRES + w1] * pmv->d_liq_Xvol_solvents[w][POR_LIQ_PRES + w2]
                      + mp->d_saturation[POR_LIQ_PRES + w2] * pmv->d_liq_Xvol_solvents[w][POR_LIQ_PRES + w1]
                      + saturation * pmv->d_d_liq_Xvol_solvents[w][POR_LIQ_PRES + w1][POR_LIQ_PRES + w2])
	      ...
	      pmv->d_d_bulk_density[w][POR_LIQ_PRES + w1][POR_LIQ_PRES + w2] += mp->d_porosity[POR_LIQ_PRES + w2] 
		* (  
	      ...
	      pmv->d_d_bulk_density[w][POR_LIQ_PRES + w1][POR_LIQ_PRES + w2] += mp->d_porosity[POR_LIQ_PRES + w1] 
		* (  
...
void
load_liq_perm(double porosity, double cap_pres, double saturation,
	      double d_cap_pres[2])
     ...
     **************************************************************************/
{
  ...
  if (mp->RelLiqPermModel == VAN_GENUCHTEN) {
	...
	mp->d_rel_liq_perm[POR_LIQ_PRES] = 0.0;
      ...
      a1 = 1.0 - factor2;
      rel_liq_perm = mp->rel_liq_perm = sqrt(s_eff) * a1 * a1 /
	  viscosity;
	...
	if (pd->e[0][R_POR_POROSITY]) {
	  mp->d_rel_liq_perm[POR_POROSITY] = 
	      -3. * CUBE(pmv->r_cap) / 
	      pow(r_pore,4.0) * pmv->d_r_pore[POR_POROSITY] * factor;
	}
      }
    }
  }
    ...
    mp->rel_liq_perm = (( 1.0 - exp(-2.0 * rad) * 
			 (1 + 2. * rad + 2. * rad * rad + 
			  4./3. * pow(rad,3.0) + 2./3. * pow(rad,4.0) +
			  4./15. * pow(rad,5.0))) 
			/ mp->u_rel_liq_perm[0]);
    ...
    if (af->Assemble_Jacobian) {
	  ...
	  / r_pore;
      if (pd->e[0][R_POR_GAS_PRES]) mp->d_rel_liq_perm[POR_GAS_PRES] = d_rel_d_rad 
				     * pmv->d_r_cap[POR_GAS_PRES] 
				     / r_pore;
      if (pd->e[0][R_POR_POROSITY]) mp->d_rel_liq_perm[POR_POROSITY] = - d_rel_d_rad 
...
void
load_gas_perm(double porosity, double cap_pres, double saturation,
	      double d_cap_pres[2])
  ...
  if (mp->RelGasPermModel == SUM_TO_ONE) {
    ...
    if (mp->RelLiqPermModel == VAN_GENUCHTEN) {
      liq_visc = mp->u_rel_liq_perm[3];
	       ...
	       mp->RelLiqPermModel == PSD_WEXP ||
	       mp->RelLiqPermModel == PSD_SEXP ) {
      liq_visc = mp->u_rel_liq_perm[0];
    } else {
      liq_visc = mp->viscosity;
      WH(-1, "Need liq visc in calc_rel_gas_perm");
    }

    mp->rel_gas_perm = (1. - mp->rel_liq_perm * liq_visc)
	/ mp->u_rel_gas_perm[0];
	...
	mp->d_rel_gas_perm[POR_LIQ_PRES] = 
	    - mp->d_rel_liq_perm[POR_LIQ_PRES]
	    * liq_visc / mp->u_rel_gas_perm[0];

	if (pd->e[0][R_POR_GAS_PRES]) mp->d_rel_gas_perm[POR_GAS_PRES] = 
				       - mp->d_rel_liq_perm[POR_GAS_PRES]
				       *  liq_visc / mp->u_rel_gas_perm[0];

	if (pd->e[0][R_POR_POROSITY]) mp->d_rel_gas_perm[POR_POROSITY] = 
				       - mp->d_rel_liq_perm[POR_POROSITY]
				       * liq_visc / mp->u_rel_gas_perm[0];
      }
    }
...
void
load_gas_diff(double porosity, double cap_pres, double saturation,
	      double d_cap_pres[2], int spec)
     ...
     ************************************************************************/
{
  ...
  if (mp->PorousDiffusivityModel[spec] == POROUS) {
    ...
    factor = (mp->u_porous_diffusivity[spec][0] /
	      mp->u_porous_diffusivity[spec][1]);
	...
	if (pd->v[0][TEMPERATURE]) {
      ...
      if (pd->e[0][R_POR_GAS_PRES]) {
	mp->d_porous_diffusivity[spec][POR_GAS_PRES] -= 
	    mp->porous_diffusivity[spec] / fv->p_gas;
      }
      if (pd->v[0][TEMPERATURE]) {
	mp->d_porous_diffusivity[spec][TEMPERATURE] = 
	    mp->porous_diffusivity[spec] *
	    mp->u_porous_diffusivity[spec][4] / fv->T;
      }
    }
  } else {
...
void 
load_MandE_flux(double porosity, double cap_pres, double saturation, 
	       double d_cap_pres[2])
     ...
     ************************************************************************/
{
   ...
   */      
	...
	pmv->rel_mass_flux[i_pe][a] = 
	    pmv->liq_darcy_velocity[a]*pmv->liq_Xvol_solvents[i_pl]*mp->density*pmv->enthalpy[0] +
    ...
    if (cr->PorousFluxModel == DARCY_FICKIAN) {
       ...
       */
       ...
       for (a = 0; a < WIM; a++) {
	     ...
	     */
            grad_Ywg[a] = pmv->d_Ywg[i_pl]   * fv->grad_p_liq[a] +
		          pmv->d_Ywg[i_pg]   * fv->grad_p_gas[a]  +
		          pmv->d_Ywg[i_pore] * fv->grad_porosity[a] +
                          pmv->d_Ywg[i_pe]   * fv->grad_T[a];

	    for ( j=0; j<ei->dof[var]; j++) {
	      for (w2=0; w2<MAX_PMV; w2++) {
		   d_grad_Ywg[a][w2][j] = pmv->d_Ywg[w2] * bf[var]->grad_phi[j][a] +
		                 bf[var]->phi[j] * ( 
                                 pmv->d_dYwg[i_pl][w2]   * fv->grad_p_liq[a] +
	    ...
	    pmv->rel_mass_flux[i_pl][a] +=  difflux[w][a];
            ...
            if (pd->e[0][R_POR_ENERGY]) pmv->rel_mass_flux[i_pe][a] +=  
                                     difflux[i_pl][a]*(pmv->enthalpy[1]-pmv->enthalpy[2]);
       }

	    
  } else {
      EH( -1, "Unimplemented flux constitutive relation in continuous media. Note: POWERLAW_DARCY_FICKIAN not available for POROUS_TWO_PHASE"); 
  }

  /* Add conduction in the solid */

  if (pd->e[0][R_POR_ENERGY]) {
      for ( a=0; a<WIM; a++) pmv->rel_mass_flux[i_pe][a] += 
			...
			if(pd->v[0][var])
			  {
			   for ( j=0; j<ei->dof[var]; j++) 
				...
				t2 = bf[var]->phi[j] * 
				  pmv->liq_darcy_velocity[a] * mp->density *
				  pmv->d_liq_Xvol_solvents[w][POR_LIQ_PRES + w1];

				t3 = pmv->d_liq_darcy_velocity[a][w1][j] *
				  pmv->liq_Xvol_solvents[w] * mp->density;
		      
				t4 = pmv->d_gas_darcy_velocity[a][w1][j] *
				  pmv->gas_density_solvents[w];
				...
				if (pd->e[0][R_POR_ENERGY]) {
				  pmv->d_rel_mass_flux_dpmv[i_pe][a] [w1][j] += 
				    ...
				    bf[var]->phi[j] *  
				    (pmv->liq_darcy_velocity[a]*pmv->liq_Xvol_solvents[w]*
				     mp->density*pmv->d_enthalpy[0][POR_LIQ_PRES + w1] +
      ...
      if ( cr->PorousFluxModel == DARCY_FICKIAN)
	{
	  ...
	  /* Derivative with respect to Pliq */
             ...
             for ( a=0; a<WIM; a++)
		{
		  for ( j=0; j<ei->dof[var]; j++)
		    {
                          difflux_term =  -bf[var]->phi[j] * (
                                (mp->porous_diffusivity[w] * pmv->d_rhog[i_pl] +
			  ...
			  pmv->d_rel_mass_flux_dpmv[w+1][a] [i_pl][j] +=  -difflux_term;
		              ...
		              pmv->d_rel_mass_flux_dpmv[i_pe][a] [i_pl][j] += 
                                   ...
                                   (pmv->d_enthalpy[1][POR_LIQ_PRES]-
                                    pmv->d_enthalpy[2][POR_LIQ_PRES]);  

			  /* Derivative with respect to Pgas */
			  if(pd->e[0][R_POR_GAS_PRES])
			    {
			      difflux_term =  -bf[var]->phi[j] * (
                                (mp->porous_diffusivity[w] * pmv->d_rhog[i_pg] +
				...
				pmv->d_rel_mass_flux_dpmv[i_pe][a] [i_pg][j] += 
				  difflux_term * (pmv->enthalpy[1]-pmv->enthalpy[2]) + 
				  difflux[i_pl][a] * bf[var]->phi[j] *
				  (pmv->d_enthalpy[1][POR_GAS_PRES]-
				   pmv->d_enthalpy[2][POR_GAS_PRES]);  
			    }

			  if(pd->e[0][R_POR_POROSITY])
			    {
			      difflux_term =   
				pmv->d_Ywg[i_pore] * pmv->rhog *
				...
				{
				  difflux_term =  bf[var]->phi[j] * (
								 mp->porous_diffusivity[w] * pmv->rhog * fv->grad_porosity[a]
			  ...
			  if(pd->e[0][R_POR_ENERGY])
			    {
			      difflux_term =  -bf[var]->phi[j] * (
				 (mp->porous_diffusivity[w] * pmv->d_rhog[i_pe] +
				...
				difflux_term * (pmv->enthalpy[1]-pmv->enthalpy[2]) + 
				difflux[i_pl][a] *  bf[var]->phi[j] *
				(pmv->d_enthalpy[1][POR_TEMP]-
				 pmv->d_enthalpy[2][POR_TEMP]);  
			    }
	...
	for ( a=0; a<WIM; a++)
	  {
		...
		pmv->d_rel_mass_flux_dpmv[i_pe][a] [i_pe][j] +=   
			...
			mp->d_thermal_conductivity[var] * bf[var]->phi[j]
                        * fv->grad_T[a];
	      }
	  }
      }
		  ...
		  for (a=0; a < WIM; a++)
		      ...
		      if (cr->PorousFluxModel == DARCY_FICKIAN)
			{
			  for ( w=0; w<MAX_PMV; w++) 
			    {
			      if (w != i_pore) 
				{  /* no diffusion term for solid phase */
				  if (mp->PermeabilityModel != K_TENSOR && 
				      mp->PermeabilityModel != ORTHOTROPIC &&
				      mp->PermeabilityModel != KC_TENSOR &&
				      mp->PermeabilityModel != SM_TENSOR) 
...
void
calc_grad_Ywg (int var, double *grad_Ywg)
{
  int dofs, p, i;
  int i_pl=0;
  double Pg, T, Ywg;
  ...
  for ( p=0; p<VIM; p++) {
    ...
    for ( i=0; i<dofs; i++) {
      ...
      if (pd->e[0][R_POR_GAS_PRES]) Pg = *esp->p_gas[i]; else Pg =  mp->u_porous_gas_constants[3];
      /* perturb temperature */
      if (pd->e[0][R_POR_ENERGY]) T = *esp->T[i]*1.01; else T = mp->u_porous_vapor_pressure[i_pl][4] - 273.15;
...
calc_darcy_velocity()
{
  ...
  if (mp->PermeabilityModel != K_TENSOR &&
	...
	if (pd->e[0][R_POR_GAS_PRES]) {
	  ...
	  pmv->gas_darcy_velocity[a] = 
	    - mp->permeability * mp->rel_gas_perm * fv->grad_p_gas[a];
	} else {
	...
	for ( j=0; j<ei->dof[var]; j++) {
	  for ( w1=0; w1<MAX_PMV; w1++) {
	    ...
	    for ( b=0; b<WIM; b++) {
	      ...
	      pmv->d_liq_darcy_velocity[a][w1][j] -= bf[var]->phi[j] * 
		(mp->perm_tensor[a][b] * mp->d_rel_liq_perm[POR_LIQ_PRES + w1] * 
		 (fv->grad_p_liq[b] - mp->momentum_source[b] ) ); 
		      
	      if (pd->e[0][R_POR_GAS_PRES]) {
		pmv->d_gas_darcy_velocity[a] [w1][j] -= bf[var]->phi[j] * 
		  mp->perm_tensor[a][b] * 
	  ...
	  for ( b=0; b<WIM; b++) {
	    ...
	    pmv->d_liq_darcy_velocity[a] [i_pl][j] -= 
	      mp->perm_tensor[a][b] * mp->rel_liq_perm * 
	      bf[var]->grad_phi[j][b]; 
	    ...
	    if (pd->e[0][R_POR_GAS_PRES]) {
	      pmv->d_gas_darcy_velocity[a] [i_pg][j] -=
		mp->perm_tensor[a][b] * 
		mp->rel_gas_perm * bf[var]->grad_phi[j][b];
	    }
	  } /* for (b=0 ...) */
	} /* for (J=0 ...) */
      } /* for (a=0 ...) */
      ...
      var = POR_POROSITY;
      if(pd->v[0][var] && mp->PermeabilityModel == KC_TENSOR)
	{
	  for ( a=0; a<WIM; a++) {
	    for ( b=0; b<WIM; b++) {
	      for ( j=0; j<ei->dof[var]; j++) {
...
void 
load_mass_flux(double porosity, double cap_pres, double saturation, 
	       double d_cap_pres[2])
      ...
      ************************************************************************/
{
  ...
  if (cr->PorousFluxModel == DARCY_FICKIAN) {     /*PRS (12/19/01): add other models here */
	...
	pmv->rel_mass_flux[i_pe][a] = 
            -mp->thermal_conductivity * fv->grad_T[a] +
	    pmv->liq_darcy_velocity[a]*pmv->liq_Xvol_solvents[i_pe]*mp->density*pmv->enthalpy[0] +
	    pmv->gas_darcy_velocity[a]*(pmv->gas_density_solvents[i_pl]*pmv->enthalpy[1] +
                                        pmv->gas_density_solvents[i_pg]*pmv->enthalpy[2]);
      }
    }
	...
	if (w != i_pore) {  /* no capacity term for solid phase */
	  for (a = 0; a < VIM; a++) {
	     ...
	     * of gas phase concentration
	     */
	    pmv->rel_mass_flux[w][a] -=  mp->porous_diffusivity[w] * (
		 pmv->d_gas_density_solvents[w][POR_LIQ_PRES] * fv->grad_p_liq[a] +
		 pmv->d_gas_density_solvents[w][POR_GAS_PRES] * fv->grad_p_gas[a]  +
		 pmv->d_gas_density_solvents[w][POR_POROSITY] * fv->grad_porosity[a]);
	  }
    ...
    {
    EH( -1, "Unimplemented flux constitutive relation in continuous media.");
    }
    ...
    if (cr->PorousFluxModel == DARCY_FICKIAN) {
	  ...
	  for (a = 0; a < VIM; a++) {
	    ...
	    pmv->rel_mass_flux[w][a] +=  difflux[w][a];
            if (pd->e[0][R_POR_GAS_PRES]) pmv->rel_mass_flux[i_pg][a] += difflux[w+1][a];
            if (pd->e[0][R_POR_ENERGY]) pmv->rel_mass_flux[i_pe][a] +=  
                                     difflux[w][a]*(pmv->enthalpy[1]-pmv->enthalpy[2]);
	  }
	  ...
	  for ( w=0; w<MAX_PMV; w++) {
	      ...
	      for ( a=0; a<VIM; a++)
		{
		  for ( j=0; j<ei->dof[var]; j++) {
		    for ( w1=0; w1<MAX_PMV; w1++)
		      {
			pmv->d_rel_mass_flux_dpmv[w][a] [w1][j] += bf[var]->phi[j] * 
			    pmv->gas_darcy_velocity[a] * 
			    pmv->d_gas_density_solvents[w][POR_LIQ_PRES + w1];
			pmv->d_rel_mass_flux_dpmv[w][a] [w1][j] += bf[var]->phi[j] * 
			    pmv->liq_darcy_velocity[a] * mp->density *
		...
		for (a = 0; a < VIM; a++) {
		  for (j = 0; j < ei->dof[var]; j++) {
			...
			* ( mp->permeability * mp->d_rel_liq_perm[POR_LIQ_PRES + w1] 
		      ...
		      if (pd->e[0][R_POR_GAS_PRES]) {
			pmv->d_rel_mass_flux_dpmv[w][a] [w1][j] -= 
			    bf[var]->phi[j] * pmv->gas_density_solvents[w] *
			    fv->grad_p_gas[a] *
			    (mp->permeability * mp->d_rel_gas_perm[POR_LIQ_PRES + w1] + 
			     mp->d_permeability[POR_LIQ_PRES + w1] * mp->rel_gas_perm);
		      }
			...
			mp->permeability * mp->rel_liq_perm *
			bf[var]->grad_phi[j][a] * pmv->liq_Xvol_solvents[w] * mp->density;
		    
		    /* sensitivity to gas pressure gradient and permeability */
		    if (pd->e[0][R_POR_GAS_PRES]) {
		      pmv->d_rel_mass_flux_dpmv[w][a] [i_pg][j] -= 
			  mp->permeability * pmv->gas_density_solvents[w] * 
			  mp->rel_gas_perm * bf[var]->grad_phi[j][a];
		    }
		  }
		}
	      } else { /* Permeability is a tensor, and it is constant */
		for ( a=0; a<VIM; a++) {
		  for ( b=0; b<VIM; b++) {
			...
			if (pd->e[0][R_POR_GAS_PRES]) {
			  pmv->d_rel_mass_flux_dpmv[w][a] [w1][j] -= bf[var]->phi[j] * 
			    mp->perm_tensor[a][b] * pmv->gas_density_solvents[w] *
			    mp->d_rel_gas_perm[POR_LIQ_PRES + w1] * fv->grad_p_gas[b];
		      ...
		      pmv->d_rel_mass_flux_dpmv[w][a] [i_pl][j] -= 
			mp->perm_tensor[a][b] * mp->rel_liq_perm * 
			bf[var]->grad_phi[j][b] * pmv->liq_Xvol_solvents[w] * mp->density;
		...
		for ( a=0; a<VIM; a++) {
		  for ( b=0; b<VIM; b++) {
			...
			if (pd->e[0][R_POR_GAS_PRES]) {
			  ...
			  pmv->d_rel_mass_flux_dpmv[w][a] [w1][j] -= bf[var]->phi[j] * 
			    mp->d_perm_tensor[a][b][POR_LIQ_PRES + w1] * pmv->gas_density_solvents[w] *
			    mp->rel_gas_perm * fv->grad_p_gas[b];
			}
		      }
		      /* sensitivity to liquid pressure gradient */
		      pmv->d_rel_mass_flux_dpmv[w][a] [i_pl][j] -= 
	      ...
	      for ( a=0; a<VIM; a++)
		{
		  for ( j=0; j<ei->dof[var]; j++)
		      ...
		      if(pd->e[0][R_POR_POROSITY])
			{
			  ...
			  for ( w2=0; w2<MAX_PMV; w2++)
			    {
			      pmv->d_rel_mass_flux_dpmv[w][a] [w2][j] -=  bf[var]->phi[j] * (
		  ...
		  for ( j=0; j<ei->dof[var]; j++)
		      ...
		      for ( w2=0; w2<MAX_PMV; w2++)
			{
			  pmv->d_rel_mass_flux_dpmv[w][a] [w2][j] -=  bf[var]->phi[j] * 
				       ...
				       * pmv->d_d_gas_density_solvents[w][POR_LIQ_PRES][POR_LIQ_PRES + w2]
				       + mp->d_porous_diffusivity[w][POR_LIQ_PRES + w2]
				       * pmv->d_gas_density_solvents[w][POR_LIQ_PRES] * pow(fv->grad_p_liq[a], n_pow) );
			}
...
void 
porous_pressure(double *func, double d_func[], int eb_mat_solid, 
		int eb_mat_fluid)
...
void 
porous_pressure_lub(double *func, double d_func[], 
		    const int id_side,
		    double xi[DIM],
		    const Exo_DB *exo,
		    const double scale)
...
void 
sat_darcy_continuous_bc(double func[],
			double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
			const double tt,
                        const double dt, 
 ...
 *****************************************************************************/
{
  ...
  attached = 1.0 ; near = 0.; /*default case */
  ...
  if (Current_EB_ptr->Elem_Blk_Id == eb_mat_solid) {
     ...
     */
    if ((pd->MeshMotion == LAGRANGIAN ||
	 cr->MeshMotion == DYNAMIC_LAGRANGIAN) && pd->MeshInertia == 1) {
      err = belly_flop(elc->lame_mu);
      EH(err, "error in belly flop");
      if( neg_elem_volume ) return;
    }
	...
	mp->PermeabilityModel != KC_TENSOR ) {
      for (a = 0; a < VIM; a++) {
	pmv->liq_darcy_velocity[a] = - factor * mp->permeability * 
	    (fv->grad_p_liq[a] - mp->momentum_source[a]);
      }
    } else {
      for (a = 0; a < VIM; a++) {
	pmv->liq_darcy_velocity[a] = 0.0; 
	for (b = 0; b < VIM; b++) {
	  pmv->liq_darcy_velocity[a] += - factor * mp->perm_tensor[a][b] * 
	      (fv->grad_p_liq[b] - mp->momentum_source[b]);
	}
      }
    }
    *func = 0.0;
	...
	if (mp->PermeabilityModel != K_TENSOR &&
	  ...
	  for (a = 0; a < VIM; a++) {
	    d_func[0][var][j_id] -= 
			attached * fv->snormal[a] * mp->density * factor *  
			mp->permeability * bf[var]->grad_phi[j_id][a];
		d_func[0][var][j_id] -= 
			attached * fv->snormal[a] * mp->density * d_factor[0] *  bf[var]->phi[j_id] *
			mp->permeability * fv->grad_p_liq[a];
	  ...
	  for (a = 0; a < VIM; a++) {
	    for (b = 0; b < VIM; b++) {
	      d_func[0][var][j_id] -= 
			attached * fv->snormal[a] * mp->density *  
			factor * mp->perm_tensor[a][b] * bf[var]->grad_phi[j_id][b];
	      d_func[0][var][j_id] -= 
			attached * fv->snormal[a] * mp->density *  d_factor[0] * bf[var]->phi[j_id] 
			* mp->perm_tensor[a][b] * fv->grad_p_liq[b];
	    }
	  }
	}
      }
	...
	if (mp->PermeabilityModel != K_TENSOR &&
	  ...
	  for (a = 0; a < VIM; a++) {
	    for (b = 0; b < VIM; b++) {
	      d_func[0][var][j_id] -= 
		  attached * fv->snormal[a] * mp->density *  
		  factor * mp->d_perm_tensor[a][b][POR_POROSITY] * fv->grad_p_liq[b];
	    }
	  }
	}
      }
	...
	if (pd->v[0][var]) {
	  for (j_id = 0; j_id < ei->dof[var]; j_id++) {
	    ...
	    for (p = 0; p < dim; p++) {
	      d_func[0][var][j_id] +=
		  attached * fv->dsnormal_dx[p][a][j_id] * mp->density *
		  (vconv[p] * mp->porosity + pmv->liq_darcy_velocity[p]);
	      d_func[0][var][j_id] += 
		  attached * fv->snormal[p] * mp->density * 
		  (mp->porosity * d_vconv->X[p][a][j_id]);
	    }
	    if (mp->PermeabilityModel != K_TENSOR &&
		mp->PermeabilityModel != ORTHOTROPIC &&
		mp->PermeabilityModel != SM_TENSOR &&
		mp->PermeabilityModel != KC_TENSOR) {
	      for (b = 0; b < VIM; b++) {
		d_func[0][var][j_id] -=
		    attached * fv->snormal[b] * mp->density * factor *
		...
		for (p = 0; p < VIM; p++) {
		  d_func[0][var][j_id] -= 
		      attached * fv->snormal[b] * mp->density * factor *
		      mp->perm_tensor[b][p] * fv->d_grad_p_liq_dmesh[p][a][j_id];
		  if(mp->PermeabilityModel == ORTHOTROPIC ||
		     mp->PermeabilityModel == KC_TENSOR ||
		     mp->PermeabilityModel == SM_TENSOR)
		    {
	...
	*func += vnormal*rho_liq;
	...
	if (pd->v[0][var]) {
	  for (j_id = 0; j_id < ei->dof[var]; j_id++) {
	    for (a = 0; a < dim; a++) {
	      d_func[0][var][j_id] += 
		  fv->dsnormal_dx[a][jvar][j_id] * rho_liq* 
		  (fv->v[a] - x_dot[a]);
	    }
	  }
	}
      ...
      if ( pd->TimeIntegration != STEADY )
	{
	  for (jvar = 0; jvar < dim; jvar++) {
		...
		phi_j = bf[var]->phi[j_id];
		for (a = 0; a < dim; a++) {
		  d_func[0][var][j_id] -= fv->snormal[a] * rho_liq * 
		            ( 1.0 + 2.0*tt)*(phi_j/dt)* (double) delta(a,jvar);
		}
	      }
	    }
	  }
...
void 
porous_liq_fill(double *func,
		double d_func[],
		const int eb_mat_solid, const int eb_mat_fluid,
		double pc1, double pc2, double lsls, MATRL_PROP_STRUCT *mp2)
...

{
  ...
  case BILINEAR:  /* BILINEAR Interpolation Scheme */
	...
	EH(-1, "Table interpolation not implemented");
      ...
      table->slope[1]= (1.0-uu)*((f[istarty+1+Np1*iad]-f[istarty+Np1*iad])/(t2[istarty+1]-t2[istarty]))+
	  uu*((f[istarty+1+iinter+Np1*iad]-f[istarty+iinter+Np1*iad])/(t2[istarty+1+iinter]-t2[istarty+iinter]));
      /* slope0 not needed */
...
/******************************************************************************/
int
evaluate_sat_hyst_criterion(int ip,
			    int ielem,
