...
#ifndef COUPLED_FILL
void
semi_lagrange_step( const int num_total_nodes,
...

{
  ...
  if( dim > 2 ) EH( -1, "SEMI_LAGRANGE time stepping not implemented in 3D.");
  if( ls->Isosurface_Subsurf_Type != LS_SURF_FACET ) EH(-1, "Need FACET Isosurface type .");
  ...
  for( inode = 0 ; inode < num_total_nodes; inode++ )
      ...
      if ( num_varType_at_node( inode, eqn ) == 1 )
	{
	  ...
	  if ( ( num_varType_at_node( inode, VELOCITY1) == 1 ) && 
	    ...
	    {
	      EH(-1, "Need equal interpolation order LS and VELOCITY1, VELOCITY2 for SEMI_LAGRANGE.\n" );
	    }
  ...
  global_LS_flux = find_LS_global_flux ( exo,
					 dpi,
					 NULL,
					 NULL,
					 x,
  ...
  while (delta_norm >= 1.e-5 && max_its > 0 )
      ...
      for ( k = 0, delta_norm = 0.0 ; k< num_fill_unknowns; k++)
	{
	  ...
	  if( ext_dof[k] == FALSE ) delta_norm += pow( -0.5*R[k] - 0.5*b[k]*d_lamda, 2.0 );
	}
  ...
  if( max_its == 0 ) 
    {
      for( i=0; i < num_fill_unknowns; i++ ) F[i] = F_[i];
    }
...
int
apply_ls_inlet_bc ( double afill[],
		    int    ijaf[],
		    double    x[],
		    double   rf[],
		    int node_to_fill[],
  ...
  for( i=0 ; i<elem_side_bc->num_nodes_on_side; i++ )
      ...
      if( num_varType_at_node( I, ls->var ) == 1 )
	{
	  ...
	  if( closest->closest_point->confidence == 0 )
	    {
	      if ( x[ie] * distance < 0.0 ) distance *= -1.0;
	    }
...
apply_strong_fill_ca_bc (
          ...
          struct elem_side_bc_struct *elem_side_bc,   /* Pointer to an element side boundary condition structure */
	      ...
	      if ( af->Assemble_Residual )
		{
		  ...
		  for ( i=0; i<ei->num_local_nodes; i++)
		      ...
		      for ( ki=0; ki<nvdofi; ki++)
			{
			  ...
			  rf[node_to_fill[I]+ki] += BIG_PENALTY * wt * fv->sdet * phi_i * 
			    delta_func * rhs;
			}
		    }
		}
			  ...
			  for( j=0; j< ei->num_local_nodes; j++) 
			      ...
			      for ( kj=0; kj<nvdofj; kj++)
				{
				  ...
				  EH(ja, "Could not find vbl in sparse matrix.");
                                  ...
                                  for ( a=0; a<dim; a++)
			            {
				      afill[ja] += BIG_PENALTY * wt * fv->sdet * phi_i * 
                                                   delta_func * bf[eqn]->grad_phi[jdof][a]*fv->snormal[a];
                                    }
				}
			    }
			}
		...
		const int Renorm_Now )
{
  ...
  if ( ls->Length_Scale == 0. )
    {
      renorm_width =
        ls->Control_Width * global_h_elem_siz(x, x_old_static, xdot_static, x, exo, dpi);
    }
  ...
  if ( Renorm_Now || ls_err > tolerance )
  {
	  ...
	  /* this call cleanses the LS field of "droplets" that surround exactly one node */
	  ...
	  ls->Renorm_Countdown = ls->Renorm_Freq;
		...
		/*
		 * the following flag's function is turn off the potential for saturation hysteresis curve switching
		 * for 4 time steps after a renormalization step.  This hopefully avoids spurious curve switching related 
		 * to renormalization induced weirdness
		 */
...
void
correct_level_set ( struct Aztec_Linear_Solver_System *ams, 
		    double xf[],
		    double rf[],
		    double x[],
		    double x_old[],
                    ...
                    Comm_Ex *cx )
{
  ...
  if ( step_size == 0.0 ) step_size = init_step_size;
  ...
  if ( fabs( ave_level_grad_err ) > tolerance )
    {
      ...
      while ( ( fabs( ave_level_grad_err ) > tolerance ) && 
	      step++ < num_steps  )
	{
	  int its;
	  ...
	  its = integrate_explicit_eqn(ams, rf, xf, xf_old, xfdot, 
				       xfdot_old, x, x_old, x_oldest,
				       step_size, theta, &time,
...
/* assemble_level_correct -- Assemble Jacobian and rhs to level-set distance function
 *                           correction evolution equation.
 *                          
 *
 *
 */
...
int 
assemble_level_correct(double afill[],	/* Jacobian matrix for fill equation  */
		       int ijaf[],	/* pointer to nonzeros in Jacobian matrix   */
	      ...
	      for( j=0; j< ei->num_local_nodes; j++) 
		{
		  ...
		  for ( kj=0; kj<nvdofj; kj++)
		      ...
		      for ( a=0; !EXPLICIT && a<dim; a++)
			{
			  ...
			  afill[ja] += wt * h3 * det_J * rhs * upwind*alpha* ( d_w_dF[a][jdof]*bf[eqn]->grad_phi[idof][a]);
...
int 
assemble_level_project(double afill[],	/* Jacobian matrix for fill equation  */
		       int ijaf[],	/* pointer to nonzeros in Jacobian matrix   */
		       double rf[],	/* rhs vector   */
		       double dt,	/* current time step size */
		       double tt,	/* parameter to vary time integration from
...
{
  int a, I, ie;
  int DeformingMesh = upd->ep[0][R_MESH1];
  double r[DIM];
  double **Disp = NULL;
   ...
   * Note that the current implementation doesn't make sense to combine this with 
   * other surfaces.
   */
...
#define PRINT_LS 0
  ...
  if ( list->start->type == LS_SURF_ISOSURFACE &&
       ls->SubElemIntegration ) subelement_mesh_output(x, exo);
#endif
  ...
  for( I=0; I < num_total_nodes; I++)
  {
	  ...
	  {	  
		  ...
		  if ( ls->Periodic_Planes )
		  {
		      ...
		      double dist;
		      double rbest[3];
		      rbest[0] = r[0]; rbest[1] = r[1]; rbest[2] = r[2];
		      /* X periodicity */
			  ...
			  if ( dist < closest_dist )
			    {
			      closest_dist = dist;
			      rbest[0] = rperiodic[0]; rbest[1] = rperiodic[1]; rbest[2] = rperiodic[2];
			    }
			  rperiodic[0] = r[0] - delta;
			  closest = closest_surf( list, x, exo, rperiodic );
			  dist = fabs(closest->closest_point->distance);
		  ...
		  */
		  ...
		  if ( closest->type == LS_SURF_ISOSURFACE )
		  {
			  ...
			  if ( ( x[ie] - s->isoval ) * 
				   closest->closest_point->distance < 0. ) 
				  closest->closest_point->distance *= -1;
		  }
...
void 
free_surf_list ( struct LS_Surf_List **list_p )
{
  struct LS_Surf_List *list;
  list = *list_p;
  ...
  if ( list != NULL )
      ...
      while( list->start != NULL )
	{
	  list->current = list->start->next;
          ...
          if (list->start->subsurf_list) free_surf_list( &(list->start->subsurf_list) );
...
void
free_subsurfs ( struct LS_Surf_List *list )
{
  struct LS_Surf *surf;
  ...
  if( list != NULL )
    {
      ...
      while(surf)
        {
          if (surf->subsurf_list) free_surf_list( &(surf->subsurf_list) );
          surf = surf->next;
        }
...
struct LS_Surf *
closest_surf ( struct LS_Surf_List *list,
               double *x,
               Exo_DB *exo,
	       double r[DIM] )
{
...
void
find_surf_closest_point ( struct LS_Surf *surf,
		          double *x,
                          Exo_DB *exo,
                          double *r )
{
   ...
   * which has a CA condition applied (NOTE: the ls->Contact_Inflection must also be
   * turn on).  Currently, this is possible for facets or point based surfaces.
   */
  ...
  switch( surf->type )
	...
	if( pd->Num_Dim == 3 ) dot_prod += r[2]*n[2];
            ...
            else
              {
                if (d_dot_ray >= ray_mag_squared) /* we are closest to point p2 */
                ...
                else /* we are closest to some point on the segment */
                  {
                    ...
                    d[0] -= fraction * ray[0];
                    d[1] -= fraction * ray[1];
                    if ( s1->inflection && s2->inflection ) cp->inflection = TRUE;
                  }
              }
    ...
    case LS_SURF_USER :
      {
          ...
          cp->distance = user_surf_object( s->Int_Data,
			  	           s->Real_Data,
			  	           r);
...
create_surfs_from_ns( int ns_id, 
                      double *x,
                      Exo_DB *exo )
/* construct list of surface objects from node set */
{
  ...
  if ( ns >= exo->num_node_sets )
    {
      if( Num_Proc == 1 )
         EH(-1,"Error: cannot find initial level set nodeset for Huygens initialization \n");
    }
   else
    {
	  ...
	  retrieve_node_coordinates( I,x,p, NULL  );
          ...
          /* Not supplying elem or xi here since we don't really have elem accessible. */
...
static struct LS_Surf_List *
create_surfs_from_ss ( int ss_id,
                       double *x,
                       Exo_DB *exo )
...
int
stash_node_displacements( double **d, int num_total_nodes, double *x, Exo_DB *exo) 
{
	int *moved;
...
static void
retrieve_node_coordinates( int I,
						   double *x,
						   double *p,
						   double **d )
{
...
create_surfs_from_arc ( double c[DIM], double r, double n[DIM], double d )
{
	...
	while( theta < 360.0 )
	{
		...
		if( knockout_point( test_point, n, d ) == FALSE )
		{
			...
			s->x[0] = test_point[0]; s->x[1] = test_point[1]; s->x[2] =0.0;
...
static int
knockout_point( double test[DIM], double normal[DIM], double d )
{
	double R;
...
static double
find_arc_region_sign ( struct LS_Surf_Arc_Data *s, double *r )
{
...
static struct LS_Surf_List *
create_surfs_from_iso (int isovar,
                       double isoval,
                       double x[],
		       Exo_DB *exo)
{
      ...
      if ( pd->e[0][isovar] )
	{
	  for( e = e_start ; e < e_end ; e++)
              ...
              if ( elem_on_isosurface ( e, x, exo, isovar, isoval ) )
		{
                  ...
                  load_elem_dofptr(e, exo, x_static, x_old_static,
...
/* 			      print_shape_fcn_tree ( ntree ); */
  ...
  if ( isosurf_exists == FALSE)
	smooth_stop_with_msg("\n\tUnable to locate points on zero level set isosurface.  Stopping Goma.\n");
...
void 
create_subsurfs ( struct LS_Surf_List *list,
                  double *x,
                  Exo_DB *exo )
{
...
static void
initialize_sign( int PosEB_id, double *x, Exo_DB *e )
{
  int ie,i,I;
  int eb;
...
void
assemble_Global_surf_list( struct LS_Surf_List *list )
{
#ifdef PARALLEL
  ...
  if ( list == NULL ) EH(-1,"Lists must exist before calling assemble_Global_surf_list");
  ...
  do
    {
      ...
      if (surf) my_list_size++;
    } while (surf);
  
  /* find out how much everybody is going to tell us
   */
  MPI_Allgather( &my_list_size, 1, MPI_INT, list_size,
...
/* (f1 < 0. ) ? (f2 >= 0.) : (f2 < 0.) */
int
sign_change( double f1, double f2 )
{
  if ( f1 < 0. )
    {
                     ...
                     double isoval )
{
  ...
  mn = Matilda[ebn];
  if (!(pd_glob[mn]->v[0][isovar])) return (FALSE);
  
  ielem_type = Elem_Type(exo, elem);
  ielem_shape  = type2shape(ielem_type);
  interpolation = pd_glob[mn]->i[0][isovar];
                  ...
                  int var )
{
  ...
  ebn = find_elemblock_index(elem, exo);
  mn = Matilda[ebn];
  if (!(pd_glob[mn]->v[0][var])) return (0.);
  
  ielem_type = Elem_Type(exo, elem);
  ielem_shape  = type2shape(ielem_type);
  dofs = getdofs(ielem_shape, pd_glob[mn]->i[0][var]);
...
int
current_elem_on_isosurface ( int isovar,
                             double isoval )
{
  double *esp;
  double f[MDE];
                               ...
                               const Exo_DB* exo )
{
  ...
  ebn = find_elemblock_index(elem, exo);
  mn = Matilda[ebn];
  if (!(pd_glob[mn]->v[0][ls->var])) return (FALSE);

  ielem_type = Elem_Type(exo, elem);
  ielem_shape  = type2shape(ielem_type);
  dofs = getdofs(ielem_shape, pd_glob[mn]->i[0][ls->var]);
...
#ifdef PARALLEL
	  if( Num_Proc > 1 ) external = ext_dof[k];   
	  /* when accumulating the dot products it is important not to add the external dofs */
#endif
...
double
find_LS_global_flux ( const Exo_DB *exo,
		      const Dpi *dpi,
		      const double *params,
		      double *dC,
		      double x[],
		      int num_total_unkns)
{
  ...
  if ( dC != NULL )  memset( dC, 0, num_total_unkns*sizeof(double) );
  ...
  for( eb=0, M = 0.0 ; eb< dpi->num_elem_blocks_global; eb++) 
	...
	M += evaluate_global_flux ( exo,
				    dpi, 
				    ( ls->Mass_Sign == I_NEG_FILL ? NEG_LS_FLUX : POS_LS_FLUX ),
				    blk_id,
				    0,
				    NULL,
				    NULL,
 ...
 *                      in lieu of ls->Length_Scale. NB: compute_volume_integrand()
 *                      uses H(F) so this really doesn't matter.
 * chosen_vel         = Specifies which velocity component and phase to average.
 * x                  = The solution vector.
 * num_total_unknowns = Number of total unknowns.
 *
 * Returns
 * =======
  ...
  for( eb=0 ; eb< dpi->num_elem_blocks_global; eb++) 
      ...
      if( pd_glob[mn]->e[0][R_LEVEL_SET] )
	{
	  Vel += evaluate_volume_integral(exo,
					  ...
					  0.0,
					  0);
	  Vol += evaluate_volume_integral(exo,
					  dpi,
					  phase,
					  NULL,
					  blk_id,
...

{
  ...
  if ( ProcID == 0 ) fclose(ofp);

}

int
generate_facet_list ( double (**point0)[DIM],
                      double (**point1)[DIM],
                      int ** owning_elem,
                      double *x,
		      Exo_DB *exo )

{
  ...
  ls->Isosurface_Subsurf_Type = LS_SURF_FACET;  /* We are interested in a facet description of interface */
        ...
        case LS_SURF_POINT :
          {
            ...
            fprintf(g, "POINT %f\t %d\t %f\t %f\t %f\t %d\n",
                    time, i, s->x[0], s->x[1], s->x[2], s->inflection);
        ...
        case LS_SURF_SPHERE :
          {
            ...
            fprintf(f, "CICLE/SPHERE %d\t %f\t %f\t %f\t %f\n",
                    i, s->center[0], s->center[1], s->center[2], s->r );
            fprintf(g, "CICLE/SPHERE %f\t %d\t %f\t %f\t %f\t %f\n",
                    time, i, s->center[0], s->center[1], s->center[2], s->r );
          }
...
static void
find_intersections( struct LS_Surf_List *list, 
                    int isovar, double isoval,
		    Exo_DB *exo )
{
  double  xi[3] = {0., 0., 0.};
  double  yi[3] = {0., 0., 0.};
    ...
    case HEXAHEDRON:
      ...
      switch( pd->i[0][isovar] ) {
        ...
        case I_Q2:                     /* triquadratic hex */
          {
            double links[86][2] = { {0,11}, {11,21},{21,8},{8,0},
                                    {12,23},{23,20},{20,25},{25,12},
                                    {11,23},{21,20},{8,25},{0,12},
                                    {11,3}, {3,10}, {10,21}, {23,15},
                                    {15,26},{26,20},{3,15},{10,26},
                                    {21,9},{9,1},{1,8},{20,24},
                                    {24,13},{13,25},{9,24},{1,13},
                                    {10,3},{2,9},{26,14},{14,24},
                                    {2,19},{4,19},{19,22},{22,16},
                                    {16,4},{12,4},{23,19},{20,22},
                                    {25,16},{19,7},{7,18},{18,22},
                                    {15,7},{26,18},{22,17},{17,5},
                                    {5,16},{24,17},{13,5},{18,6},
                                    {6,17},{19,6}, {0,20},{12,21},
                                    {23,8},{11,25}, {11,26},{3,20},
                                    {10,23},{21,15},{8,24},{21,13},
                                    {9,25},{1,20},{21,14},{10,24},
                                    {3,20},{9,26},{12,22},{23,16},
                                    {20,4},{25,19},{23,18},{15,22},
                                    {26,19},{20,7},{25,17},{20,5},
                                    {24,16},{13,22},{20,6},{26,17},
                                    {14,22},{24,18} };  /* This set of links is not all the links that exist on a 27node brick */
  ...
  case TRISHELL:
    ...
    default:
      EH(-1, "Huygens renormalization not implemented for this interpolation on TRIs");
      break;
    }
    break;
...
void
find_facets( struct LS_Surf_List *list,
             int isovar, double isoval,
	     Exo_DB *exo )
{
  ...
  switch ( ei->ielem_shape ) {
    ...
    case SHELL: 
      ...
      switch( pd->i[0][isovar] ) {
        ...
        default:
          printf("isovar=%d, pd->i[0][isovar]=%d\n",isovar,pd->i[0][isovar]);
...
static void
find_quad_facets( struct LS_Surf_List *list,
                  int isovar, 
                  double isoval,
		  int *quad,
		  Exo_DB *exo )
      ...
      for (ibc = 0; ibc < Num_BC; ibc++)
        {
	      ...
	      if (num_ss_on_ca_boundary >= 20)
                EH(-1, "Need to increase array for ss_on_ca_boundary[] in point_on_ca_boundary");
...
int
find_link_intersection( double *xi, double *yi, int isovar, double isoval,
                        Integ_Elem * subelement )
{
  ...
  if (fabs(F1) < tol * fabs(F2)) { return (TRUE) ; }; /* that was easy */
  ...
  if ( step >= MAX_STEP ) {
    fprintf(stderr, "The maximum iteration count was exceeded in find_link_intersection!");
    return ( FALSE );
  }
...
static void
compute_link_level_set (double *xi, 
                        double *yi,
                        double r,
                        int isovar,
                        double isoval,
...
/* generic surface creator */
{
  ...
  surf->type = type;
  surf->closest_point = (struct LS_Surf_Closest_Point *) 
                        smalloc( sizeof( struct LS_Surf_Closest_Point ) );
...
void
append_surf( struct LS_Surf_List *list,
             struct LS_Surf      *surf )
...
void
append_surf_isosurf( struct LS_Surf_List *list,
			int isovar,
			double isoval )
{
...
distance ( double *x, double *y )
{
  ...
  dist = pow( (x[0] - y[0]), 2.0 ) +
         pow( (x[1] - y[1]), 2.0 );
         
  if (pd->Num_Dim == 3) dist += pow( (x[2] - y[2]), 2.0 );
...
int
unique_surf( struct LS_Surf_List *list,
             struct LS_Surf      *surf )
...
struct LS_Surf *
create_surf_point( double *x, int elem, double *xi, int inflection )
{
  struct LS_Surf *surf;
  struct LS_Surf_Point_Data *s;
  ...
  s->xi[0] = xi[0]; s->xi[1] = xi[1]; s->xi[2] = xi[2];
...
struct LS_Surf *
create_surf_facet_line( struct LS_Surf *v1,
                        struct LS_Surf *v2,
                        int elem,
                        int elem_side )
{
...
next_surf_or_subsurf( struct LS_Surf_List *list,
                      int  *level )
...
static struct LS_Surf *
create_next_surf_or_subsurf( struct LS_Surf_List *list, 
                             int          level,
                             int          type )
  ...
  if ( level > 0 )
    {
      if (list->end->subsurf_list == NULL)
        {
          if (level > 1) EH(-1, "Didn't think this could happen");
...
/*
 * ls_var_initialization :  Initializes field variables that are indexed to level set
 *                          These would include species in two-phase transport or temperature in casting.
 *                          
 *   Input
...
static double
scalar_value_at_local_node ( int ielem,
			     int ielem_type,
			     int lnode,
			     int var,
			     int ktype,
...
*/
void
iso_contour_on_side( double isoval,
		     int dim,
		     int ielem_type,
		     int id_side,
		     int *ip_total,
	...
	int local_elem_node_id[MAX_NODES_PER_SIDE];
		...
		for ( i = 1; i < nodes_per_side; i++ )
		{
			...
			if( lvdof != -1)
			{
				...
				if ( sign_change( *esp->F[local_elem_node_id[0]] - isoval, 
								  *esp->F[local_elem_node_id[i]] - isoval ) ) have_crossing = TRUE;
			}
		}
        ...
        if( have_crossing )
		{
			...
			*wt[0] = 1.0;
			...
			else /* unusual case of having two crossings on this side, hmmm, what to do? */
			{
				if ( nodes_per_side != 3 ) EH(-1,"Unexpected element type!");
 ...
 *        are calculated if we're not near the interface or if do_deriv is FALSE.
 *
 * Input
 * -----
 ...
 ******************************************************************************/
int
level_set_interface(const double F,
 ...
 ******************************************************************************/
int
level_set_property(const double p0,
		   const double p1,
		   const double width,
		   double *pp,
		   double d_pp_dF[MDE])
{
  ...
  if ( ! pd->v[0][MESH_DISPLACEMENT1] ) return(0);
 ...
 *                           Computes "diffuse" level set Hv function of material 
 *                           property.  Differs from level_set_property in that it
 *                           returns sensitivity wrt F field instead of F nodal unknowns.
 *                           It is used primarily for diffusivities, thermal conductivities
 *
 *
 * Input
 * -----
 ...
 ******************************************************************************/
int
ls_transport_property(const double p0,
		      const double p1,
		      const double width,
		      double *pp,
		      double *d_pp_dF)
{
  ...
  /* Calculate the deriviatives of the material property w.r.t. FILL. */
  if ( ls->Elem_Sign == 0 ) *d_pp_dF = (p1 - p0) * lsi->dH;
  else *d_pp_dF = 0.;
...
{
  /* For level set problems we need to know the characteristics of all of this element */
  ...
  if ( ei->ielem != xfem->ielem || Debug_Flag < 0 )
    {
      ...
      for (eqn = V_FIRST; eqn < V_LAST && !xfem->have_XG; eqn++)
        {
          if ( pd->i[0][eqn] == I_P1_XG ||
               pd->i[0][eqn] == I_Q1_XG ||
               pd->i[0][eqn] == I_Q2_XG ||
               pd->i[0][eqn] == I_Q1_HG ||
               pd->i[0][eqn] == I_Q1_HVG ||
               pd->i[0][eqn] == I_Q2_HG ||
               pd->i[0][eqn] == I_Q2_HVG ) xfem->have_XG = TRUE;
        }
      ...
      for (eqn = V_FIRST; eqn < V_LAST && !xfem->have_disc; eqn++)
        {
          if ( pd->i[0][eqn] == I_Q1_HV ||
               pd->i[0][eqn] == I_Q1_HG ||
               pd->i[0][eqn] == I_Q1_HVG  ||
               pd->i[0][eqn] == I_Q2_HV ||
               pd->i[0][eqn] == I_Q2_HG ||
               pd->i[0][eqn] == I_Q2_HVG ) xfem->have_disc = TRUE;
        }
#if 0
	      ...
	      for (i = 0; i < ei->dof[eqn]; i++) 
	        {
	          ledof = ei->lvdof_to_ledof[eqn][i];
                  ...
                  xfem_dof_state( i, pd->i[0][eqn], ei->ielem_shape,
...
void
load_xfem_for_stu( const double xi[] )
{
  int i, F_elem_type = -1;
  int dof_ls;
      ...
      if ( xi[0] != xfem->xi[0] || xi[1] != xfem->xi[1] || xi[2] != xfem->xi[2] )
        {
          ...
          if ( pd->i[0][ls->var] == I_Q1 ) 
	    {
	      F_elem_type = pd->Num_Dim == 2 ? BILINEAR_QUAD : TRILINEAR_HEX;
	    }
          else if ( pd->i[0][FILL] == I_Q2 ) 
	    {
	      F_elem_type = pd->Num_Dim == 2 ? BIQUAD_QUAD : TRIQUAD_HEX;
	    }
          else if ( pd->i[0][FILL] == I_NOTHING )
	    {
              xfem->near = FALSE;
              xfem->F = 2. * ls->Length_Scale;
              xfem->H = 1.;
              xfem->delta = 0.;
              xfem->dF_xi[0] = 0.; xfem->dF_xi[1] = 0.; xfem->dF_xi[2] = 0.;
              xfem->bf_plus = 0.;
              xfem->F_plus = 0.;
              xfem->grad_bf_plus[0] = 0.; xfem->grad_bf_plus[1] = 0.; xfem->grad_bf_plus[2] = 0.;
              xfem->grad_F_plus[0] = 0.; xfem->grad_F_plus[1] = 0.; xfem->grad_F_plus[2] = 0.;
              return;
	    }
          else EH(-1, "Unexpected interpolation for FILL");
          ...
          /* drn - this contains somewhat convoluted logic to desparately try to keep
             the costs low but keep from having too many if tests

             basically, we need the following
             xfem->near:      need xfem->dF_xi[]
             xfem->have_XG:   need xfem->dF_xi[] & xfem->F_plus & xfem->grad_F_plus[]
             xfem->have_disc: need xfem->F_plus & xfem->grad_F_plus[] & xfem->bf_plus & xfem->grad_bf_plus[]

             the most important thing is to avoid extra calls to shape.  the logic below sometimes
             calculates unnecessary quantities, but should avoid unnecessary calls to shape.
           */
...
void
xfem_correct( int num_total_nodes,
              double x[],
              double xdot[],
              double x_old[],
              double xdot_old[],
      ...
      for (lvdesc = 0; lvdesc < nv->Num_Var_Desc; lvdesc++)
        {
          ...
          interp = pd_glob[MatID]->i[0][var_type];
...
void
xfem_predict( int num_total_nodes,
              int numProcUnknowns,
              double delta_t,
              double delta_t_old,
                    ...
                    case I_Q2_XV:
                      {
                        ...
                        if ( sign_change( F, F_older ) ) {
                          if ( F_older < 0. ) sign_older = -1;
                          else sign_older = 1;
                        }
                        if ( sign_change( F, F_oldest ) ) {
                          if ( F_oldest < 0. ) sign_oldest = -1;
                          else sign_oldest = 1;
                        }
 ...
 ******************************************************************************/
void
xfem_var_diff( int var,
               double *vdiff,
               double phidiff[MDE],
               double gradvdiff[DIM] )
{
  int interp = pd->i[0][var];
...
/**** Shield the operations below since they are very expensive relative to the previous
  ...
  if (pd->v[0][LUBP]  || pd->v[0][LUBP_2] || pd->v[0][SHELL_SAT_CLOSED] || pd->v[0][SHELL_PRESS_OPEN ] ||
      pd->v[0][SHELL_PRESS_OPEN_2] || pd->v[0][SHELL_SAT_GASN] )
    {
      ...
      if(pd->v[0][LUBP] || pd->v[0][SHELL_SAT_CLOSED] || pd->v[0][SHELL_PRESS_OPEN ] || pd->v[0][SHELL_SAT_GASN] ) 
	{
	  for ( i = 0; i < ei->dof[eqn]; i++ ) {
	    Fi = *esp->F[i];
      ...
      else if (pd->v[0][LUBP_2] || pd->v[0][SHELL_PRESS_OPEN_2]) 
	{
	  eqn = R_PHASE1;
	  for ( i = 0; i < ei->dof[eqn]; i++ ) {
	    Fi = *esp->pF[0][i];
...
/**** Shield the operations below since they are very expensive relative to the previous
      operations in the load_lsi routine. Add your variables as needed.   The balance force
      formulation here.   compute at nodes first, and then spread to gauss points ********/
  ...
  memset(lsi->d_Hn_dmesh, 0.0, sizeof(double)*DIM*MDE);
  memset(lsi->d_gradHn_dmesh, 0.0, sizeof(double)*DIM*DIM*MDE);
  if (upd->vp[0][LUBP_2] >= 0 || upd->vp[0][SHELL_PRESS_OPEN_2] >= 0) 
    {
      eqn = R_PHASE1;
      for ( i = 0; i < ei->dof[eqn]; i++ ) {
	Fi = *esp->pF[0][i];
...
load_lsi_derivs()
{
  ...
  /*
   * If we're here, the pd->v[0]ar[ls->var] is true...
   *
  ...
  for ( j=0; j < ei->dof[var]; j++ )
      ...
      for ( a=0; a < VIM; a++ )
	{
      ...
      lsi->d_gfmaginv_dF[j] = ( lsi->gfmag == 0.0 ) ? 0.0 : - pow(lsi->gfmaginv, 2.0) *  lsi->d_gfmag_dF[j];

      /* Derivative of the normal vector. */
      for ( a=0; a < VIM; a++)
	{
	  lsi->d_normal_dF[a][j] = grad_phi_j[a] * lsi->gfmaginv
	    + lsi->d_gfmaginv_dF[j] * grad_F[a];
  ...
  if ( pd->v[0][MESH_DISPLACEMENT1] )
  {
      ...
      for ( b=0; b < VIM; b++)
	  {
          ...
          for ( j=0; j < ei->dof[var] ; j++ )
		  {
              ...
              for ( a=0; a < VIM; a++ )
			  {
                  lsi->d_gfmag_dmesh[b][j] = grad_F[a] * fv->d_grad_F_dmesh[a][b][j]
				  * lsi->gfmaginv;
			  }
			  
              /* gfmaginv */
			  lsi->d_gfmaginv_dmesh[b][j] = ( lsi->gfmag == 0.0 ) ? 0.0 : 
				  - pow(lsi->gfmaginv, 2.0) *  lsi->d_gfmag_dmesh[b][j];
			  
              /* normal */
              for ( a=0; a < VIM; a++ )
			  {
                  lsi->d_normal_dmesh[a][b][j] = fv->d_grad_F_dmesh[a][b][j] *lsi->gfmaginv
 ...
 *   
 *    Not that only the value of the level set function and values derived from it
 *    (H, delta, d_H_dF
 */

int
load_lsi_adjmatr( const double width )
{
  ...
  /* 
   * Note that we can't compute grad_F in the adjmatr (Yet)  so all quantities related 
  ...
  if ( ls->on_sharp_surf )
    ...
    {
      lsi->H     = 0.5 * (1. + F / alpha + sin(M_PIE * F / alpha) / M_PIE);
      lsi->delta = 0.5 * (1. + cos(M_PIE * F / alpha)) / alpha; /* this is an approximation since we can't compute gfmag */
      lsi->dH = 0.5 * (1.0/alpha) * (1. + cos(M_PIE * F / alpha));
...
double
ls_modulate_property ( double p1, 
		       double p2, 
		       double width,
		       double pm_minus,
		       double pm_plus,
...
static int
current_elem_xfem_state ( int node_var_state[],
                          int * elem_var_state,
			  double x[],       /* Solution vector for the current processor    */
			  const Exo_DB *exo )
{
  ...
  /* we also need to classify the nodes of this element:
     node_var_state[i] == 0 -> this node does not have active enriched dofs in any element
     node_var_state[i] == 1 -> this node has active enriched dofs in this element
     node_var_state[i] == 2 -> this node does not have active enriched dofs in this element but
			   does in some other element
   */
  ...
  /* return = 0 -> no extending fns active in this element
     return = 1 -> at least one of the nodes of this element have node_var_state == 1 or elem_vars_state == 1
     return = 2 -> at least one of the nodes of this element have node_var_state == 2
   */
  int i, j, e, I;
  int elem_state = 0;
...
int
is_xfem_interp( const int interp )
{
  return ( interp == I_P0_G ||
           interp == I_P1_G ||
           interp == I_Q1_G ||
           interp == I_Q2_G ||
	   interp == I_P0_GP ||
...
void
xfem_dof_state( const int ledof,
  ...
  switch (interpolation)
    ...
    case I_Q2_XG:
      *extended_dof = (ledof%2 == 1);
      if ( *extended_dof ) *base_dof = (ledof-1)/2;
      else *base_dof = ledof/2;
      break;
    case I_Q1_HV:
    case I_Q1_HG:
    case I_Q1_HVG:
    ...
    case I_Q2_HVG:
      *extended_dof = (ledof >= getdofs( eshape, *base_interp ));
      if ( *extended_dof ) *base_dof = ledof - getdofs( eshape, *base_interp ); /* watch out used differently here */
      else *base_dof = ledof;
      break;
    case I_Q1_GP:
    case I_Q2_GP:
    ...
    case I_P1_GP:
      *base_dof = ledof;
      *extended_dof = xfem->elem_var_state == 1 ||
                      lnn_distance( *base_dof ) < 0.;
      break;
    case I_Q1_GN:
    case I_Q2_GN:
      *base_dof = ledof;
 ...
 *  determined.  The other grids are ignored.  Those grids that have an interface
 *  are divided once again by precisely the same method.  In fact, they are
 *  divided by a recursive call to the function which divided the first element
 *  This process continues until a preset level of division is reached.
 *
 *  The Search_Grid_Structure accomodates this division.  Each SGRID structure represents
 *  a single grid.  It has members for the s,t,u coordinates of the nodes and value for the
 *  LS function at the nodes.  It also has an array of pointers that are populated by the 
 *  grids that appear when the parent grid is divided. Each of these subgrids might have 
 *  subgrids of their own and so on.  Thus, each SGRID structure might be the starting node
 *  of an entire tree of smaller subgrids.  
 *
 */

SGRID * 
create_search_grid ( NTREE *ntree )
{
  SGRID *new_grid;
 ...
 *   call to this same function.  When this function returns, the parent structure
 *   pointer stands at the top of a branching tree structure extending downward 
 *   max_level - parent->level branches. Pretty cool huh ?
 *
 *  returns void.
 *********************************************************************/

void
divide_search_grid ( SGRID *parent,
		     int max_level )
{
  if( parent->level < max_level)   /* If my level is the max level, return to end the recursion */
...

	{
	  ...
	  /* 
	   * Next statements test whether zero LS contour passes through subgrids[index]
	   * search grid.  
	   */
...
map_local_coordinates( double *xi, double *x )
{
  ...
  int DeformingMesh = pd->e[0][R_MESH1];
	  ...
	  for( j=0; j<mdof; j++)
	    {
	      ...
	      phi_j = newshape( xi, ei->ielem_type, PSI, ln,
                                ei->ielem_shape, pd->i[0][ShapeVar], j );
...
void
free_search_grid ( SGRID **ptr_grid )
{
  int i;
  SGRID *grid = *ptr_grid;
...
void
find_grid_intersections ( SGRID *grid, struct LS_Surf_List *list )
{
  int index;
  int i,j, l, link;
  ...
  if( grid->num_subgrids == 0 )  /* Search only the lowest level grids for intersections */
	...
	case 3:
	  {
 	    static int links[4][2] = { {0,6}, {1,7}, {2,4}, {3,5} };  /* These are the diagonals.  Saves us having to check 
 ...
 *
 *   I should really discuss this particular structure in more detail.  It became
 *   apparent to me when setting up the Search_Grid recursive structures for both interface
 *   reconstruction and adaptive subgrid integration, that I was having to compute locations 
 *   of grid vertices and values of shape functions at these points  and locations of integration points
 *   over and over.  All of these quantities can be computed up front on the (s,u,t) master element
 *   Hence the notion of the shape function tree structure was aborned.  
 *
 *   The NTREE structure is similar to the SGRID structure in that it is a self-similar, recursively-connected
 *   tree structure.  That is, the parent structure points to children structures of the same type, but the parent 
 *   structure might be the child of a larger parent itself.  A given NTREE structure stores its dimensionality, its 
 *   division depth, the number of vertices it has, the (s,u,t) coordinates of these vertices, the values of a single
 *   shape function interpolant at these vertices, the location and weights of numerical integration points on the
 *   the structure.  It knows how many children it has and has pointers to all of them.  It, however, doesn't know who its
 *   parent is.  Sad really.
 *   
 *   The SGRID structure can be be sparesly populated, that is, some branches terminate prematurely before they reach the 
 *   maximum division depth set (max_level).  This is because some grid's don't overlap the interface or the "interface region."
 *   This is not so with the NTREE structure.  All branches are fully populated to the maximum division depth.  This is because the
 *   NTREE structure is to be created up front and used as a resource when constructing the SGRID structures.  One can't know 
 *   *a priori* which grids any SGRID reconstruction will need, so we have to compute ALL of them.  This can mean a lot of 
 *   information.  For example, an 3D NTREE structure to a division depth of 4 will have 1 + 8 + 64 + 512 + 4096 = 4681 distinct
 *   grids within it.  All of them with 8 vertices along with associated shape function values not to mention the
 *   integration point information.  A lot of computations and something you don't want to reconstruct for every element.
 */

NTREE *
create_shape_fcn_tree ( int max_level )
{
  NTREE *tree;
...
static void
compute_shape_fcn_values ( NTREE *tree )

{
  ...
  for( i=0; i<tree->num_verts; i++ )
    {
      ri[0] = tree->xi[i][0]; ri[1] = tree->xi[i][1]; ri[2] = tree->xi[i][2]; 
			...
			int max_level )
{
  if ( parent->level < max_level )
      ...
      for ( index = 0 ; index < num_subtrees; index++ )
	{
	  ...
	  if( TRUE ) 
	    {
	      if ( parent->dim == 3 ) num_gpts =  (parent->subtrees[index]->level < 8)  ? 8 : 1;
	      if ( parent->dim == 2 ) num_gpts =  (parent->subtrees[index]->level < 8)  ? 4 : 1;
...
static void
gather_subtree_coords ( NTREE * tree, 
			double *xi_m,
			double ( *sub_xi )[DIM] )
{
...
static void
load_subtree_coords( int index, NTREE *tree, double (*sub_xi )[DIM] )
{
  int node;
  switch ( tree->dim )
    {
    ...
    case 3:
      {
	static int map3D[8][8] = { { 0,8,21,11,12,25,20,23},
...
void
free_shape_fcn_tree ( NTREE *tree )
{
  int i;
...
int
build_integration_grid ( SGRID *parent,
			 int max_level,
			 double width )
{
  int total_gpts = 0;
...
void
compute_tree_size( NTREE *tree, double *l1, double *l2, double *l3, double *s_m )
{
  switch ( tree->dim ) 
    {
    case 2:
...
int
grid_overlaps_interface( SGRID *grid, double width )
{
...
int
gather_integration_pts ( SGRID *grid, double (*s)[DIM], double *wt, int num_gpts )
{
  int index;
...
int
print_subgrid_integration_pts ( double (*s)[DIM], double *wt, int num_gpts )
{
  int i;
  double x[DIM];
...
int
print_subgrid_surface_integration_pts ( double (*s)[DIM], double *wt, int num_gpts )
{
  int i;
  double x[DIM];
...
int
current_elem_overlaps_interface( double width )
{
...
int
elem_overlaps_interface( int elem,
		         double x[],
		         const Exo_DB* exo,
                         double width )
{
...
int
get_subgrid_integration_pts ( NTREE *start_tree, SGRID **start_grid, double (**s)[DIM], double **weight, double width )
{
...
int
gather_surface_subgrid_integration_pts ( SGRID *grid, int id_side, double surface_centroid[DIM], double (*s)[DIM], double *wt, int num_gpts )
{
	int index;
	...
	if( grid->num_subgrids != 0 )
	{
		for( index=0; index < grid->num_subgrids; index++ )
		{
			num_gpts = gather_surface_subgrid_integration_pts ( grid->subgrids[index], id_side, surface_centroid, s, wt, num_gpts );
		}
	}
	else
	{
...
static int
vertex_on_element_boundary( double xi[DIM], double *surface_centroid,  int *id )
{
	int i;
...
void
assemble_boundary_extension_velocity ( double x[],
                                       Exo_DB *exo,
                                       Dpi *dpi )
{
  ...
  for( j=0 ; j < num_exterior_faces; j++ )
    {
      ...
      for (n=0; n<nodes_per_side; n++)
		...
		if(pfd == NULL)
          ...
          for ( a=0; a<dim; a++ )
            {
              if ( F_value < 0. ) sign -= fv->snormal[a] * lsi->normal[a];
              else sign += fv->snormal[a] * lsi->normal[a];
            }
...
int
assemble_extension_velocity (dbl hsquared[DIM], 
			     dbl hh[DIM][DIM], 
...
#define NORMAL_GRADEXTV 0
  ...
  if ( af->Assemble_Residual )
      ...
      for ( i=0; i<ei->dof[eqn]; i++)
	{
	  ...
	  advection = 0.;
	  if ( pd->e[0][eqn] & T_ADVECTION )
	    {
		  advection = resid * wt_func;
		  advection *= det_J * wt * h3;
		  advection *= pd->etm[0][eqn][(LOG2_ADVECTION)];
	    }
	      ...
	      if ( pd->v[0][var] )
		{
		  ...
		  for ( j=0; j<ei->dof[var]; j++)
		      ...
		      if ( pd->e[0][eqn] & T_ADVECTION )
			{
			  ...
			  advection *= wt_func * wt * pd->etm[0][eqn][(LOG2_ADVECTION)];
...
void get_subelement_descriptions(double x[],
		                 Exo_DB *exo,
      ...
      if ( pd->v[0][ls->var] )
	{
	  for( ielem = e_start ; ielem < e_end ; ielem++)
              ...
              if ( elem_on_isosurface ( ielem, x, exo, ls->var, 0. ) )
		{
                  ...
                  load_elem_dofptr(ielem, exo, x, x_old_static,
                                   xdot_static, xdot_old_static,
		   	           x_static, 0);
  ...
  while ( s != NULL )
    {
      for (iside = 0; iside < s->num_sides; iside++)
          ...
          if ( s->bc_sides[iside] ) /* side is on ls interface */
	    {
		  ...
		  if ( first )
		    {
		      min[0] = s->x[lnn[0]][0]; max[0] = s->x[lnn[0]][0];
		      min[1] = s->x[lnn[0]][1]; max[1] = s->x[lnn[0]][1];
		      min[2] = 0.; max[2] = 0.;
		      first = FALSE;
		    }
		  ...
		  for ( n = 0; n < 3; n++ )
		    {
		       if ( s->x[lnn[n]][0] < min[0] ) min[0] = s->x[lnn[n]][0];
		       if ( s->x[lnn[n]][0] > max[0] ) max[0] = s->x[lnn[n]][0];
		       if ( s->x[lnn[n]][1] < min[1] ) min[1] = s->x[lnn[n]][1];
		       if ( s->x[lnn[n]][1] > max[1] ) max[1] = s->x[lnn[n]][1];
		    }
                }
	    }
	}
...
void
free_subelement_descriptions( Integ_Elem_Desc **s_ptr )         
{
...
void
gather_subelement_descriptions( Integ_Elem_Desc_List *list, 
                                Integ_Elem *e )
{
  int i;
...
void
subelement_mesh_output (double x[],
		        Exo_DB *exo)
{
  Integ_Elem_Desc *s;
  Integ_Elem_Desc_List list;
  ...
  while ( s != NULL && s->num_nodes != 3 && s->num_nodes != 6 ) s = s->next;

  if ( s != NULL )
    {
      nodes_per_elem = s->num_nodes;
      if ( nodes_per_elem == 3 ) nodes_per_side = 2;
      else if ( nodes_per_elem == 6 ) nodes_per_side = 3;
      else EH(-1,"Subelement type not supported.");
    }
          ...
          for ( j = 0; j < s->num_nodes; j++ )
            {
              ...
              coord_y[i] = s->x[j][1];
              coord_z[i] = s->x[j][2];
              if ( j < nodes_per_elem ) vconn[ivconn++] = i+1;
              i++;
            }
	  for (iside = 0; iside < s->num_sides; iside++)
            {
		  ...
		  if ( iside == 0 )
		    {
		      ...
		      if ( nodes_per_side == 3 ) sconn[isconn++] = istart + 3 + 1;
		    }
		  else if ( iside == 1 )
		    {
		      sconn[isconn++] = istart + 1 + 1;
		      sconn[isconn++] = istart + 2 + 1;
		      if ( nodes_per_side == 3 ) sconn[isconn++] = istart + 4 + 1;
		    }
		  else if ( iside == 2 )
		    {
		      sconn[isconn++] = istart + 2 + 1;
		      sconn[isconn++] = istart + 0 + 1;
		      if ( nodes_per_side == 3 ) sconn[isconn++] = istart + 5 + 1;
		    }
		  else 
...
int
get_facet_integration_pts ( double (**s)[DIM],
                            double **weight,
                            Exo_DB *exo )
{
  struct LS_Surf_List * list;
...
fail_courant_condition(void)
{
  ...
  if (TimeIntegration == STEADY) return FALSE;
      ...
      if (ls->var != NULL)
	{
	  if ( pd->v[0][ls->var] )
	      ...
	      for( ielem = e_start ; ielem < e_end ; ielem++)
		{
		  ...
		  h_elem_siz(hsquared, hhv, dhv_dxnode, pd->e[0][R_MESH1]);
		  ...
		  if ( pd->v[0][VELOCITY1] && tran->Fill_Equation != FILL_EQN_EXT_V )
		    {
		      for ( i=0; i< ei->dof[VELOCITY1]; i++ )
			{
			  if ( is_xfem_interp( pd->i[0][VELOCITY1] ) )
			    {
			      int xfem_active, extended_dof, base_interp, base_dof;
			      xfem_dof_state( i, pd->i[0][VELOCITY1], ei->ielem_shape,
					      &xfem_active, &extended_dof, &base_interp, &base_dof );
  ...
  if (Num_Proc > 1)
    {
      ...
      if (!got_interface) min_dt = 100.0 * tran->Delta_t_max;
...
void
get_subelement_facets( struct LS_Surf_List *list,
                       double isoval )
{
...
void
gather_subelement_facets( struct LS_Surf_List *list, Integ_Elem * e )
{
  if ( e->num_subelements > 0 )
    {
      int i;
...
Integ_Elem *
create_integ_elements( double isoval )
{
  Integ_Elem * e;
  double (* xi)[DIM];
  int i;
...
void
free_integ_elements( Integ_Elem *e )
{
  int i;
  for ( i = 0; i < e->num_subelements; i++ )
    {
  ...
  if (e->num_subelements > 0) safe_free( e->subelements );
  if (e->bc_sides != NULL) safe_free( e->bc_sides );
  if (e->side_ids != NULL) safe_free( e->side_ids );
...
*/
static double
subelement_quality(Integ_Elem * e)
{
  double yi[3];
  ...
  for (ip = 0; ip < ip_total; ip++)
    {
      ...
      if ( ip == 0 || detJ < mindetJ ) mindetJ = detJ;
      sumdetJ += fabs(detJ);
    }
...
static int 
subelement_side_crossing(Integ_Elem * e, 
                         int iside, 
			 double tol)
/* checks status of side of subelement
   ...
   return = 2: two crossings on this side of the subelement and extent of crossing is significant
   return = -1: at least one crossing of subelement but not considered significant
 */
{
  int nodes_per_side;
  int lnn[MAX_NODES_PER_SIDE];
  ...
  if ( nodes_per_side == 2 )
    {
      if ( sign_change(e->f[lnn[0]], e->f[lnn[1]]) ) num_crossings++;
    }
  else if ( nodes_per_side == 3 )
    {
      if ( sign_change(e->f[lnn[0]], e->f[lnn[2]]) ) num_crossings++;
      if ( sign_change(e->f[lnn[2]], e->f[lnn[1]]) ) num_crossings++;
      if ( num_crossings == 0 &&
           fabs(e->f[lnn[2]]) < 0.25*fabs(e->f[lnn[0]]+e->f[lnn[1]]) - 0.5*sqrt(e->f[lnn[0]]*e->f[lnn[1]]) )
        num_crossings = 2;
    }
  else
    {
  ...
  for ( i = 0; i < nodes_per_side; i++ ) {
    if ( -e->f[lnn[i]] > max_neg ) max_neg = -e->f[lnn[i]];
    if ( e->f[lnn[i]] > max_pos ) max_pos = e->f[lnn[i]];
  }
  ...
  if (nodes_per_side == 3)
	...
	{
	  f = e->f[lnn[2]] - b*b/(8.*a);
	  if ( -f > max_neg ) max_neg = -f;
	  if ( f > max_pos ) max_pos = f;
	}
    }
  ...
  if ( max_neg == 0. && max_pos == 0. ) return(-2); 
    
  /* check for case of no or insignificant crossing */
  if ( (e->sign == -1 && max_pos == 0.) ||
       (e->sign ==  1 && max_neg == 0.) ) return(-1); 
...
void
build_integ_element( Integ_Elem * e, double isoval, int ielem_type,
                     double (* xi)[DIM], int * side_ids, int is_conformal )
{
  ...
  for ( i = 0; i < e->num_local_nodes; i++ )
    {
      e->xi[i][0] = xi[i][0]; e->xi[i][1] = xi[i][1]; e->xi[i][2] = xi[i][2];
   ...
   */
  sum_f = 0.;
  for ( i = 0; i < e->num_local_nodes; i++ ) sum_f += e->f[i];
  if ( sum_f < 0. ) e->sign = -1;
  else e->sign = 1;
  ...
  if ( !e->is_conformal )
    {
      switch ( ielem_type )
	    ...
	    int side_ids[3];
		  ...
		  if ( side_crossing[iside] > 0 ) have_crossing = TRUE;
            ...
            if ( job == 0 ) { /* create 4 triangles */
              ...
              /* triangle 1 */
	      nodes[0][0] = e->xi[0][0]; nodes[0][1] = e->xi[0][1]; nodes[0][2] = 0.;
              nodes[1][0] = e->xi[1][0]; nodes[1][1] = e->xi[1][1]; nodes[1][2] = 0.;
              if ( ielem_type == BIQUAD_QUAD ) {
	        nodes[2][0] = e->xi[8][0]; nodes[2][1] = e->xi[8][1]; nodes[2][2] = 0.;
                nodes[3][0] = e->xi[4][0]; nodes[3][1] = e->xi[4][1]; nodes[3][2] = 0.;
              } else {
                nodes[2][0] = 0.25*(e->xi[0][0]+e->xi[1][0]+e->xi[2][0]+e->xi[3][0]);
                nodes[2][1] = 0.25*(e->xi[0][1]+e->xi[1][1]+e->xi[2][1]+e->xi[3][1]); nodes[2][2] = 0.;
		nodes[3][0] = 0.5*(nodes[0][0]+nodes[1][0]);
                nodes[3][1] = 0.5*(nodes[0][1]+nodes[1][1]); nodes[3][2] = 0.;
              }
	      nodes[4][0] = 0.5*(nodes[2][0]+nodes[1][0]);
              nodes[4][1] = 0.5*(nodes[2][1]+nodes[1][1]); nodes[4][2] = 0.;
              nodes[5][0] = 0.5*(nodes[2][0]+nodes[0][0]);
              nodes[5][1] = 0.5*(nodes[2][1]+nodes[0][1]); nodes[5][2] = 0.;
	      ...
	      /* triangle 2 */
	      nodes[0][0] = e->xi[1][0]; nodes[0][1] = e->xi[1][1]; nodes[0][2] = 0.;
              nodes[1][0] = e->xi[2][0]; nodes[1][1] = e->xi[2][1]; nodes[1][2] = 0.;
              if ( ielem_type == BIQUAD_QUAD ) {
	        nodes[2][0] = e->xi[8][0]; nodes[2][1] = e->xi[8][1]; nodes[2][2] = 0.;
                nodes[3][0] = e->xi[5][0]; nodes[3][1] = e->xi[5][1]; nodes[3][2] = 0.;
              } else {
                nodes[2][0] = 0.25*(e->xi[0][0]+e->xi[1][0]+e->xi[2][0]+e->xi[3][0]);
                nodes[2][1] = 0.25*(e->xi[0][1]+e->xi[1][1]+e->xi[2][1]+e->xi[3][1]); nodes[2][2] = 0.;
		nodes[3][0] = 0.5*(nodes[0][0]+nodes[1][0]);
	      ...
	      /* triangle 3 */
	      nodes[0][0] = e->xi[2][0]; nodes[0][1] = e->xi[2][1]; nodes[0][2] = 0.;
              nodes[1][0] = e->xi[3][0]; nodes[1][1] = e->xi[3][1]; nodes[1][2] = 0.;
              if ( ielem_type == BIQUAD_QUAD ) {
	        nodes[2][0] = e->xi[8][0]; nodes[2][1] = e->xi[8][1]; nodes[2][2] = 0.;
                nodes[3][0] = e->xi[6][0]; nodes[3][1] = e->xi[6][1]; nodes[3][2] = 0.;
              } else {
                nodes[2][0] = 0.25*(e->xi[0][0]+e->xi[1][0]+e->xi[2][0]+e->xi[3][0]);
                nodes[2][1] = 0.25*(e->xi[0][1]+e->xi[1][1]+e->xi[2][1]+e->xi[3][1]); nodes[2][2] = 0.;
		nodes[3][0] = 0.5*(nodes[0][0]+nodes[1][0]);
	      ...
	      /* triangle 4 */
	      nodes[0][0] = e->xi[3][0]; nodes[0][1] = e->xi[3][1]; nodes[0][2] = 0.;
              nodes[1][0] = e->xi[0][0]; nodes[1][1] = e->xi[0][1]; nodes[1][2] = 0.;
              if ( ielem_type == BIQUAD_QUAD ) {
	        nodes[2][0] = e->xi[8][0]; nodes[2][1] = e->xi[8][1]; nodes[2][2] = 0.;
                nodes[3][0] = e->xi[7][0]; nodes[3][1] = e->xi[7][1]; nodes[3][2] = 0.;
              } else {
                nodes[2][0] = 0.25*(e->xi[0][0]+e->xi[1][0]+e->xi[2][0]+e->xi[3][0]);
                nodes[2][1] = 0.25*(e->xi[0][1]+e->xi[1][1]+e->xi[2][1]+e->xi[3][1]); nodes[2][2] = 0.;
		nodes[3][0] = 0.5*(nodes[0][0]+nodes[1][0]);
        ...
        case QUAD6_TRI:
          {
            int job = -1; /* job == -1: no subelements
                             ...
                             job == 1:  create 4 non-conformal subelements by subdividing each side
                           */
            double *f = e->f;
	    ...
	    for (ii = 0; ii < 6; ii++) {
	      ...
	      double yi[DIM], center[DIM] = {0.33333333333333333333,
		...
		length_scale = pow( detJ , 1.0/ ( (double) pd->Num_Dim ) );
                if ( length_scale > ls->Length_Scale / scale ) job = 1;
              }
		  ...
		  side_crossing[iside] = subelement_side_crossing( e, iside, SUBELEM_SIG_CROSS_TOL );
		  if ( side_crossing[iside] > 0 ) have_crossing = TRUE;
		  if ( side_crossing[iside] == 2 ) two_crossings = TRUE;
		}
#if 0
              for ( i=0; i<e->num_sides; i++ ) {
		  ...
		  job = 1;
                  ...
                  if ( !sign_change(f[0],f[1]) ) {
                    index[0] = 0; index[1] = 1; index[2] = 2; index[3] = 3; index[4] = 4; index[5] = 5;
                  } else if ( !sign_change(f[1],f[2]) ) {
                    index[0] = 1; index[1] = 2; index[2] = 0; index[3] = 4; index[4] = 5; index[5] = 3;
                  } else {
                    index[0] = 2; index[1] = 0; index[2] = 1; index[3] = 5; index[4] = 3; index[5] = 4;
                  }
		  
		  xi0[0] = xi[index[2]][0]; xi0[1] = xi[index[2]][1]; xi0[2] = xi[index[2]][2];
                  temp[0] = xi[index[1]][0]; temp[1] = xi[index[1]][1]; temp[2] = xi[index[1]][2];
                  success = find_link_intersection( xi0, temp, ls->var, isoval, NULL );

                  xi2[0] = xi[index[2]][0]; xi2[1] = xi[index[2]][1]; xi2[2] = xi[index[2]][2];
                  temp[0] = xi[index[0]][0]; temp[1] = xi[index[0]][1]; temp[2] = xi[index[0]][2];
		  ...
		  /* the center point isn't quite so easy */
                  ...
                  /* we would like the center point to be located about halfway between the
                     other two crossings
                     So we will find a root between the midpt and node 2 or node 3 depending on
		     which one is on the opposite side of the interface from the midpoint.
                   */
		  temp[0] = 0.5*(xi0[0]+xi2[0]); temp[1] = 0.5*(xi0[1]+xi2[1]); temp[2] = 0.;
		  ...
		  if ( sign_change( f_temp, f[index[2]] ) )
		    ...
		    {
		      xi1[0] = xi[index[3]][0]; xi1[1] = xi[index[3]][1]; xi1[2] = xi[index[3]][2];
		      success &= find_link_intersection( xi1, temp, ls->var, isoval, NULL );
		    }
		  else
		    {
...
*/
	      ...
	      double quality_tol = 0.3;
              ...
              /* triangle 1 */
	      nodes[0][0] = xi0[0];                         nodes[0][1] = xi0[1];                        nodes[0][2] = 0.;
	      nodes[1][0] = xi[index[2]][0];                nodes[1][1] = xi[index[2]][1];               nodes[1][2] = 0.;
	      nodes[2][0] = xi2[0];                         nodes[2][1] = xi2[1];                        nodes[2][2] = 0.;
	      nodes[3][0] = 0.5*(xi0[0]+xi[index[2]][0]);   nodes[3][1] = 0.5*(xi0[1]+xi[index[2]][1]);  nodes[3][2] = 0.;
	      nodes[4][0] = 0.5*(xi2[0]+xi[index[2]][0]);   nodes[4][1] = 0.5*(xi2[1]+xi[index[2]][1]);  nodes[4][2] = 0.;
	      nodes[5][0] = xi1[0];                         nodes[5][1] = xi1[1];                        nodes[5][2] = 0.;
	      ...
	      if ( subelement_quality( e->subelements[0] ) < quality_tol ) err = TRUE;

              /* triangle 2 */
	      nodes[0][0] = xi0[0];                         nodes[0][1] = xi0[1];                        nodes[0][2] = 0.;
	      nodes[1][0] = xi2[0];                         nodes[1][1] = xi2[1];                        nodes[1][2] = 0.;
	      nodes[2][0] = xi[index[3]][0];                nodes[2][1] = xi[index[3]][1];               nodes[2][2] = 0.;
	      nodes[3][0] = xi1[0];                         nodes[3][1] = xi1[1];                        nodes[3][2] = 0.;
	      nodes[4][0] = 0.5*(xi2[0]+xi[index[3]][0]);   nodes[4][1] = 0.5*(xi2[1]+xi[index[3]][1]);  nodes[4][2] = 0.;
	      nodes[5][0] = 0.5*(xi0[0]+xi[index[3]][0]);   nodes[5][1] = 0.5*(xi0[1]+xi[index[3]][1]);  nodes[5][2] = 0.;
	      ...
	      if ( subelement_quality( e->subelements[1] ) < quality_tol ) err = TRUE;

              /* triangle 3 */
	      nodes[0][0] = xi[index[1]][0];                nodes[0][1] = xi[index[1]][1];               nodes[0][2] = 0.;
	      nodes[1][0] = xi0[0];                         nodes[1][1] = xi0[1];                        nodes[1][2] = 0.;
	      nodes[2][0] = xi[index[3]][0];                nodes[2][1] = xi[index[3]][1];               nodes[2][2] = 0.;
	      nodes[3][0] = 0.5*(xi0[0]+xi[index[1]][0]);   nodes[3][1] = 0.5*(xi0[1]+xi[index[1]][1]);  nodes[3][2] = 0.;
	      nodes[4][0] = 0.5*(xi0[0]+xi[index[3]][0]);   nodes[4][1] = 0.5*(xi0[1]+xi[index[3]][1]);  nodes[4][2] = 0.;
	      nodes[5][0] = 0.5*(xi[index[1]][0]+xi[index[3]][0]);   nodes[5][1] = 0.5*(xi[index[1]][1]+xi[index[3]][1]);  nodes[5][2] = 0.;
	      ...
	      if ( subelement_quality( e->subelements[2] ) < quality_tol ) err = TRUE;

              /* triangle 4 */
	      nodes[0][0] = xi[index[3]][0];                nodes[0][1] = xi[index[3]][1];               nodes[0][2] = 0.;
	      nodes[1][0] = xi2[0];                         nodes[1][1] = xi2[1];                        nodes[1][2] = 0.;
	      nodes[2][0] = xi[index[0]][0];                nodes[2][1] = xi[index[0]][1];               nodes[2][2] = 0.;
	      nodes[3][0] = 0.5*(xi2[0]+xi[index[3]][0]);   nodes[3][1] = 0.5*(xi2[1]+xi[index[3]][1]);  nodes[3][2] = 0.;
	      nodes[4][0] = 0.5*(xi2[0]+xi[index[0]][0]);   nodes[4][1] = 0.5*(xi2[1]+xi[index[0]][1]);  nodes[4][2] = 0.;
	      nodes[5][0] = 0.5*(xi[index[0]][0]+xi[index[3]][0]);   nodes[5][1] = 0.5*(xi[index[0]][1]+xi[index[3]][1]);  nodes[5][2] = 0.;
	      ...
	      if ( subelement_quality( e->subelements[3] ) < quality_tol ) err = TRUE;
	    ...
	    if ( job == 0 ) {
              ...
              /* triangle 1*/
              i0 = 0; i1 = 3; i2 = 5;
	      nodes[0][0] = xi[i0][0]; nodes[0][1] = xi[i0][1]; nodes[0][2] = 0.;
	      nodes[1][0] = xi[i1][0]; nodes[1][1] = xi[i1][1]; nodes[1][2] = 0.;
	      nodes[2][0] = xi[i2][0]; nodes[2][1] = xi[i2][1]; nodes[2][2] = 0.;
	      nodes[3][0] = 0.5*(nodes[0][0]+nodes[1][0]); nodes[3][1] = 0.5*(nodes[0][1]+nodes[1][1]); nodes[3][2] = 0.;
	      nodes[4][0] = 0.5*(nodes[1][0]+nodes[2][0]); nodes[4][1] = 0.5*(nodes[1][1]+nodes[2][1]); nodes[4][2] = 0.;
	      nodes[5][0] = 0.5*(nodes[2][0]+nodes[0][0]); nodes[5][1] = 0.5*(nodes[2][1]+nodes[0][1]); nodes[5][2] = 0.;
...
double
subelement_surfdet( Integ_Elem * e,
                    double * yi,
                    int side,
		    int subelement_map )
{
  ...
  /* ick, there is an assumption in surface_determinant_and_normal that we
     ...
     was modified above.
   */
  if ( DeformingMesh ) WH(-1,"Deforming mesh not handled correctly by subelement_surfdet");
...
int
num_subelement_integration_pts( Integ_Elem * e, int gpt_type, int sign )
{
  int num_gpts = 0;
  if ( e->num_subelements > 0 )
    {
      ...
      if ( sign == 0 || sign == e->sign )
          ...
          if ( gpt_type >= 0 ) /* gauss pts along side of parent element */
	    {
	      int iside, ip_total;
              ...
              for (iside = 0; iside < e->num_sides; iside++)
                {
                  if ( e->side_ids[iside] == gpt_type ) num_gpts += ip_total;
                }
	    }
	  else if ( gpt_type == -1 ) /* gauss pts along isosurface */
            {
              int iside, ip_total;
              if ( Do_Overlap && ls->CrossMeshQuadPoints > 0 ) ip_total = ls->CrossMeshQuadPoints;
              else ip_total = elem_info(NQUAD_SURF, e->ielem_type);
              for (iside = 0; iside < e->num_sides; iside++)
                {
                  if ( e->bc_sides[iside] ) num_gpts += ip_total;
                }
            }
...
int
gather_subelement_integration_pts( Integ_Elem * e,
                                double (*s)[DIM], double *wt, int *ip_sign,
                                int gpt_type, int sign,
                                int index )
{
  if ( e->num_subelements > 0 )
    {
      ...
      for ( i = 0; i < e->num_subelements; i++ )
        {
          index = gather_subelement_integration_pts( e->subelements[i], s, wt, ip_sign, gpt_type, sign, index );
        }
    }
  else
    {
                      ...
                      for (ip = 0; ip < ip_total; ip++)
                        {
                          ...
                          if (ip_sign != NULL) ip_sign[index] = e->sign;
                          index++;
                        }
                    }
                }
                      ...
                      for (ip = 0; ip < ip_total; ip++)
                        {
                          if ( Do_Overlap && ls->CrossMeshQuadPoints > 0 )
                            {
                              find_subsurf_st_wt(ip, ls->CrossMeshQuadPoints, 
                                e->ielem_type, iside+1, pd->Num_Dim, yi, &weight );
                            }
                          else
                            {
                              find_surf_st(ip, e->ielem_type, iside+1, pd->Num_Dim, yi, &s_i, &t_i, &u_i);
                          ...
                          wt[index] = weight * sdet;
                          if (ip_sign != NULL) ip_sign[index] = 0;
                          index++;
                        }
                    }
                }
...
compute_xfem_contribution( int N )
{
  ...
  for (eqn = V_FIRST; eqn < V_LAST;  eqn++)
    {
      if ( pd->e[0][eqn] && eqn != R_FILL && eqn != R_EXT_VELOCITY )
        {
...
resolve_ls_adc_old ( struct Boundary_Condition *LS_ADC, Exo_DB *exo, double *x, double delta_t, int *adc_event, int time_step )
{
	...
	if( (iss = in_list( ssid, 0, exo->num_side_sets, exo->ss_id) ) == -1 )
    {
		...
		/* presumably ssid doesn't exist on this proc.  Move along. Nothing to see here */
			return;
    }
	...
	for ( i=0; *adc_event == FALSE && i<exo->ss_num_sides[iss]; i++)
	{
		...
		if(	elem_on_isosurface ( ielem, x, exo, LS, 0.0 ) )
		{
			...
			P = determine_adc_probability ( LS_ADC, ielem, exo, x, side, nodes_per_side, local_elem_node_id, delta_t, &switch_value);
				
			if ( P != 0.0 && ( random <= rand_max*P) ) 
			{
				/*apply_adc_to_elem ( exo, x, ielem, side, nodes_per_side, local_elem_node_id, 0,  switch_value );*/
				*adc_event = TRUE;
			}
		}
	}		
...
static double 
determine_adc_probability ( struct Boundary_Condition *ls_adc, int ielem, Exo_DB *exo, double *x,
							int side, int nodes_per_side, int *local_elem_node_id, double delta_t, 
							double *switch_value)
{
	double P=1.0;
	...
	for( i=1; P != 0.0 && i<nodes_per_side; i++)
	{
		...
		if( fabs(*esp->F[ln]) < 1.e-12 ) P = 0.0; 
		if( *esp->F[ln] * value0 < 0.0 ) P = 0.0; 
	}
	...
	for(ip=0, surf_area=0.0 ; ip<ip_total ; ip++)
	{
		...
		for( a=0; a< pd->Num_Dim; a++ ) average_snormal[a] += fv->snormal[a]/ip_total;
	...
	if(area < 1.e-12)EH(-1,"Error zero area element detected in determine_adc_probability.\n");
	
	avg_cos /= area;
	
	if ( fabs(avg_cos) < fabs( cos(M_PI*capture_angle/180 ) ) )  P = 0.0;
...
static void
apply_adc_to_ss( Exo_DB *exo, double *x,  int iss, double switch_value )

{
	...
	for ( i=0;  i<exo->ss_num_sides[iss]; i++)
	{
		...
		if( apply_to_side[i] == TRUE )
		{
			...
			if ( start_sign == 123.0 ) start_sign = sign_of( *esp->F[local_elem_node_id[0]] );
			
			apply_adc_to_elem( exo, x, ielem, side, nodes_per_side, local_elem_node_id, start_sign, switch_value);
		}
	}
...

{
	...
	if( start_sign == 0.0 )  start_sign = sign_of( *esp->F[local_elem_node_id[0]] );
...
static double
determine_nearest_distance ( Exo_DB *exo,
							 double *x,
							 int nodes_per_side,
							 int *local_elem_node_id )
{
...
struct LS_Surf *
resolve_ls_adc ( struct LS_Surf_List *Surf_list,
	...
	*adc_event = FALSE;
	...
	while ( ( *adc_event == FALSE ) && II != NULL )
	{
		...
		if ( check_alignment( capture_angle ) )
		{
			...
			if( pd->Num_Dim == 3 ) contact_area *= contact_area*M_PI/4.0;
			...
			{
				P = capture_rate*contact_area*delta_t*exp( 1.0 - pow(phase_func/capture_distance, 3.0));
			}
			...
			if ( P != 0.0 && ( random <= rand_max*P) ) 
			{
				...
				printf("ADC event detected at point (%lf, %lf) with probability %lf and contact area %lf\n", dd->x[0],dd->x[1], P, contact_area);
				printf("ADC contact event to be resolved at node %d, distance %lf with value %lf\n", I_adc, distance, val0 );
...
static int
check_alignment( double capture_angle )
{
	int dim = pd->Num_Dim;
	...
	/* So the big assumption here is that the gradient of the phase function field AT THE INTERFACE CURVE can be 
	 * used as a good approximation to the wall normal
	 * This assumption will improve the nearer the level set interface is to the wall itself o'course 
	 */
	 
	nw[0] = -fv->grad_pF[0][0];
	nw[1] = -fv->grad_pF[0][1];
	if( dim == 3 ) nw[2] = -fv->grad_pF[0][2];
	
	normalize_really_simple_vector( nw, dim);
	
	cosine = nw[0]*lsi->normal[0];
	cosine += nw[1]*lsi->normal[1];
	if( dim == 3 ) cosine += nw[2]*lsi->normal[2];
...
static struct LS_Surf *
closest_other_surf(  struct LS_Surf_List *Surf_list,
					 double *x,
					 Exo_DB *exo,
			   ...
			   int  *I_adc  )
{
  ...
  if ( ns >= exo->num_node_sets )
	...
	if ( ie_adc == -1 ) EH(-1," Error in find_adc_node:  Can't locate level set unknown\n");
					...
					double val0 )
{
	...
	for( I=0; I<exo->num_nodes ; I++)
	{
		...
		if( ie != -1 )
		{
			...
			if( pd->Num_Dim == 3 ) ll +=  ( p[2] - p0[2] )*( p[2] - p0[2] );
