...
int 
gravity_vibrational_source (dbl f[DIM], /* Body force. */
			    MOMENTUM_SOURCE_DEPENDENCE_STRUCT *df,
			    dbl time)
{
  int eqn;
 ...
 */
int 
suspend_momentum_source(dbl f[DIM], /* Body force. */
			MOMENTUM_SOURCE_DEPENDENCE_STRUCT *df )
{
  int eqn, var;
  int a;
 ...
 */
int 
fill_momentum_source( double f[DIM] )
{
  /* Local Variables */
...
int
epoxy_dea_species_source( int species_no,    /* Current species number */
...
int
epoxy_species_source(int species_no,   /* Current species number */
                     double *param )   /* pointer to user-defined parameter list */
 ...
 */
int
foam_epoxy_species_source(int species_no,   /* Current species number */
			  double *param, 
			  double tt, double dt)   
...
/* tt, dt - time derivative parameters */  
{
  ...
  if (mp->DensityModel == DENSITY_FOAM_CONC)
    ...
    {
      EH( -1, "Foam_epoxy_species_source only works with foam_conc density model");
    }
  ...
  rho = rho_epoxy
    + fv->c[species_v]*(1.-rho_epoxy*rho_v_inv) 
  ...
  if(Press > p_vap)
  ...
  else if(p_vap > Press)
    {
      ...
      dRe_dT   = (ff_e*d_rho_v_dT*fv->c[species_l]/rho 
		  -ff_e*rho_v*fv->c[species_l]/rho2*drho_T)*Re_2
	+ Rc_1/(Rc_2 *3.)*rho_fluor*bT;
		  ...
		  double dt)	/* current time step size */
{
  ...
  for (w = 0; w < pd->Num_Species_Eqn; w++) {
    if (mp->SpeciesSourceModel[w] == EPOXY ||
	mp->SpeciesSourceModel[w] == EPOXY_DEA ) {
      /* extent of reaction, alpha */
      species_no = w;
    }
  }
...
double
butler_volmer_source(dbl *a, int key, dbl *dh)
{
  /* Local Variables */
  ...
  if (key == 2) n = a[9];  /* for species source/sink, get the number
...
double
vary_rho_heat_source(HEAT_SOURCE_DEPENDENCE_STRUCT *d_h,
		  double tt,	/* parameter to vary time integration from 
				 * explicit (tt = 1) to implicit (tt = 0) */
		  double dt)	/* current time step size */
{
...
double
foam_heat_source(HEAT_SOURCE_DEPENDENCE_STRUCT *d_h,
   ...
   
{
  ...
  if ( mp->len_u_heat_source == 0 ) {
    ...
    sprintf(err_msg, 
	    ...
	    efv->name[a+0],efv->name[a+1],efv->name[a+2]);
    WH(-1,err_msg);
    for ( a = 0; a < dim; a++) {
...
double
visc_diss_heat_source(HEAT_SOURCE_DEPENDENCE_STRUCT *d_h,
...
if(af->Assemble_Jacobian)
  ...
  if ( d_h != NULL && pd->v[0][VELOCITY1] )
       ...
       for ( b=0; b<VIM; b++)
  	       ...
  	       if ( pd->v[0][POLYMER_STRESS11] )
		{
       		for ( p=0; p<VIM; p++)
         	  {
       		    for ( q=0; q<VIM; q++)
         	      {
			d_h->v[b][j] += param[0] * s[p][q]*
					bf[VELOCITY1]->grad_phi_e[j][b][q][p];
		      }
		  }
		}
              }
   ...
   if ( d_h != NULL && pd->v[0][POLYMER_STRESS11] )
     {
      for ( mode=0; mode<vn->modes; mode++)
         {
           for ( a=0; a<VIM; a++)
...
enthalpy_heat_capacity_model( HEAT_CAPACITY_DEPENDENCE_STRUCT *d_Cp )
{
  ...
  for (a = 0; a < ei->ielem_dim; a++)
    {
	  ...
	  if(*esp->T[i] < (T_sol))
	    ...
	    {
              grad_H[a] += bf[TEMPERATURE]->grad_phi[i][a]*(*esp->T[i]*Cp_base + 
							    latent_heat*(*esp->T[i])/(T_liq-T_sol));
	    }
  ...
  if(af->Assemble_Jacobian)  
    { 
      if(fabs(norm_grad_T) > 1.e-5) 
 	{ 
 	  if ( d_Cp != NULL && pd->v[0][TEMPERATURE] )
 	    { 
 	      var = TEMPERATURE; 
 	      if (pd->v[0][var] ) 
 		{ 
 		  ...
 		  for (j=0; j<ei->dof[var]; j++) 
 			...
 			{ 
 			  d_Cp->T[j] += (sqrt(norm_grad_T)*
					 ...
					 - sqrt(norm_grad_H)* 
					 0.5*(2.*fv->grad_T[a]*bf[TEMPERATURE]->grad_phi[j][a])/sqrt(norm_grad_T))/ 
			    norm_grad_T; 
 			} 
 		    } 
 		} 
...
origin[0] = X_0[0];  origin[1] = X_0[1]; origin[2] = X_0[2];
dir_angle[0] = param[4];  dir_angle[1] = param[5]; dir_angle[2] = param[6];
...
t = (dir_angle[0]*(X[0]-origin[0]) + dir_angle[1]*(X[1]-origin[1])
	+ dir_angle[2]*(X[2]-origin[2]))/(SQUARE(dir_angle[0]) +
...
R = sqrt( SQUARE(X[0]-axis_pt[0]) + SQUARE(X[1]-axis_pt[1]) + 
		SQUARE(X[2]-axis_pt[2]) );
      ...
      ********************************************************************/
{
  ...
  for (w = 0; w < pd->Num_Species; w++) {
    ...
    case LEVEL_SET:
      err = ls_transport_property(mp->u_diffusivity[w][0], 
				  mp->u_diffusivity[w][1], 
	   ...
	   const double hsquared[DIM])
{
  ...
  if(gn->ConstitutiveEquation == SUSPENSION
     ...
     || gn->ConstitutiveEquation == POWERLAW_SUSPENSION)
    {
      ...
      if( rel_mu_denom <= 0 )  rel_mu_denom = 0.01;
  ...
  if(mp->FickDiffType[w] == ANISOTROPIC )
      ...
      for ( a=0; a<dim; a++)
	{
      ...
      for ( a=0; a<dim; a++)
	{
	  Dd[a]  = mp->u_fdiffusivity[w][0] * h_elem *
	    sign_flux*(y_dot+adv)/(sign_adv*adv+h_elem);
	  
	  if (pd->TimeIntegration != STEADY) 
	    dDd_dy[a] = mp->u_fdiffusivity[w][0] * h_elem *
	      sign_flux*(1 + 2. * tt) / dt/(sign_adv*adv+h_elem);
	  else
	    dDd_dy[a] = 0.;
	  
	  dDd_dgrady[a] = mp->u_fdiffusivity[w][0] * h_elem *
	    sign_flux*(fv->v[a])/(sign_adv*adv+h_elem)*(1. -
							(y_dot+adv)/(sign_adv*adv+h_elem));
	  dDd_dv[a] = mp->u_fdiffusivity[w][0] * h_elem *
	    sign_flux*(grad_Y[w][a])/(sign_adv*adv+h_elem)*(1. -
							    (y_dot+adv)/(sign_adv*adv+h_elem));
	  ...
	  for ( a=0; a<dim && pd->v[0][var]; a++)
	    {
	      ...
	      c_term *= Dc;
	      c_term -= dDc_dy*bf[var]->phi[j] * 
		Y[w]*(Y[w]*grad_gd[a] + gammadot*grad_Y[w][a]) ;
	      
	      mu_term = -2.0*Y[w]*gammadot*(Dmu*grad_mu[a]+ Dmu1*grad_mu1[a]) *bf[var]->phi[j]/mu;
	      mu_term += Y[w]*Y[w]*gammadot*d_mu->C[w][j]*(Dmu*grad_mu[a]+ Dmu1*grad_mu1[a])/mu/mu;
	      mu_term += -Dmu*Y[w]*Y[w]*gammadot*(dmu_dY[w]*bf[var]->grad_phi[j][a] + 
					      d2mu_dY2[w]*grad_Y[w][a]*bf[var]->phi[j])/mu;

/* 	      mu_term += -Dmu*Y[w]*Y[w]*gammadot*(d2mu_dgd_dY*bf[var]->phi[j]*grad_Y[w][a])/mu; */
/* 	      mu_term += -Dmu1*Y[w]*Y[w]*gammadot*(d2mu_dgd_dY*bf[var]->phi[j]*fv->grad_SH[a])/mu; */

	      mu_term -= dDmu_dy*bf[var]->phi[j]*Y[w]*Y[w]
		*gammadot*grad_mu[a]/mu ; 
	      ...
	      if (gn->ConstitutiveEquation == FILLED_EPOXY)
		{
		   st->d_diff_flux_dc[w][a] [gn->cure_species_no][j] = 
		      Y[w]*Y[w]*gammadot*d_mu->C[gn->cure_species_no][j]*(Dmu*grad_mu[a]+ Dmu1*grad_mu1[a])/mu/mu;

		   
		   st->d_diff_flux_dc[w][a][gn->cure_species_no][j] 
		     += (-(Dg*f*Y[w]*mp->momentum_source[a])/mu0)*d_mu->C[gn->cure_species_no][j]/mu_ppt/mu0;
		}
	    }
	}
	  ...
	  for( j=0; j<ei->dof[var];j++)
	    {
	      ...
	      mu_term -= Dmu1 * ( Y[w]*Y[w]*gammadot*
				 (d_mu->gd*bf[var]->grad_phi[j][a] +
				  mp->d2_viscosity[SHEAR_RATE]*fv->grad_SH[a]*bf[var]->phi[j])/mu);
	      ...
	      for( p=0; p<dim; p++)
		{
		  for( j=0;  j<ei->dof[var]; j++)
		    
		    {
		      ...
		      st->d_diff_flux_dmesh[w][a][p][j] = c_term + mu_term 
			+ d_term + r_term;
		    }
		}
	    }
...
int
particle_stress(dbl tau_p[DIM][DIM],                     /* particle stress */
		...
		int w)                                   /* species number */
{
  ...
  for ( a=0; a<VIM; a++)
    {
      tau_p[a][a] += cp*Y[w]*fv->P; /* suspension pressure adds an isotropic term on the diagonal */
      for ( b=0; b<VIM; b++)
	{
...
int
divergence_particle_stress(dbl div_tau_p[DIM],               /* divergence of the stress*/
 ...
 */
int 
suspension_pm_fluid_momentum_source(
 ...
 */
int 
suspension_pm_particle_momentum_source(
  dbl f[DIM],				/* Body force. */
...
int
molten_glass_viscosity(dbl *vis, /* Base FLOWING LIQUID VISCOITY  */
...
dbl
solidification_permeability(dbl h_elem_avg, /* average element size */
...
foam_species_source(double *param)
{
  ...
  for (j = 0;j < pd->Num_Species; j++) 
    {
      if (fv->c[j] <= 1.e-10) fv->c[j] = 1.e-10;
    }
  ...
  for(j=0;j<2;j++)
    {
      ...
      if (ext[j] < 0.5) drpar[j] = -drpar[j]; 
      if (ext[j] == 0.) drpar[j] = 0.;
    }
  drpar[0] = -drpar[0];
 ...
 */
int
electrode_species_source(int species_no,   /* Current species number */
        ...
        {                                         /* FeS2 cathode; see SAND2000-0207 by Chen et al. 2000, p.19   */ 
          ...
          if(util < util1)  
          ...
          else if(util >= util2 && util <= util3)
            {
              ...
              U0 = 1.208771 + 0.00065142*T + 
                   (0.130129-0.00063812*T)/(1.-(2.2*(4.0*x0p-2.0)/(2.0*x0p-0.8)-3.0))*
                   (util-0.434)/(0.5-0.434)*0.53;
            }       
          else if(util > util3)
            {
...
int
ion_reaction_source ( int species_no )   /* current species number */
...
int
electrolyte_temperature (double t,         /* present value of time */
                         double delta_t,   /* present time step     */
...
int 
assemble_bond_evolution(double time,	/* present time value */
			double tt,	/* parameter to vary time integration from 
					   explicit (tt = 1) to implicit (tt = 0) */
			double dt	/* current time step size */)
{
	      ...
	      if ( pd->v[0][var] )
		{
		  ...
		  for ( j=0; j<ei->dof[var]; j++)
		      ...
		      if ( pd->e[0][eqn] & T_ADVECTION )
			{
			  ...
			  diffusion *= diff_coef*pd->etm[0][eqn][(LOG2_DIFFUSION)];
			}
		      ...
		      if ( pd->e[0][eqn] & T_SOURCE )
			{
			  if(nn <=0.)
				...
				+(d_source2)*d_gd_dmesh[b][j]*h3*det_J;
			      ...
			      source *= phi_i * wt * pd->etm[0][eqn][(LOG2_SOURCE)];
			    }
			  else
			    {
...
/*****************************************************************************/
static void
cal_current_density (double x[],           /* global nodal solution vector  */
                     double time,          /* present time value            */
                     double delta_t,       /* present time step             */
