	 ...
	 GD_count to indicate that we've got the geometry here */
      ...
      if (    BC_Types[bc_input_id].BC_Name == PLANE_BC ||
	      ...
	      (BC_Types[bc_input_id].BC_Name == GD_CONST_BC ||
	       ...
	       BC_Types[bc_input_id].BC_Name == MESH_CONSTRAINT_BC)) GD_count++;
      ...
      if ( ( (BC_Types[bc_input_id].BC_Name == KINEMATIC_BC ||
	      ...
	      (BC_Types[bc_input_id].BC_Name == GD_CONST_BC ||
	       ...
	       BC_Types[bc_input_id].BC_Name == GD_POLYN_BC ||
	       BC_Types[bc_input_id].BC_Name == GD_TABLE_BC ||
	       BC_Types[bc_input_id].BC_Name == MESH_CONSTRAINT_BC) ) && (bc_application == SPECIAL)) ||
		...
		(BC_Types[bc_input_id].BC_Name == KINEMATIC_BC ||
		 ...
		 ( (BC_Types[bc_input_id].BC_Name == GD_CONST_BC ||
		   ...
		   (BC_Types[bc_input_id].desc->equation == R_MESH_NORMAL || 
		    BC_Types[bc_input_id].desc->equation == R_MESH2))) /* This is for the infamous GD_LINEAR trick */
		/* && iapply   */
 		) {
...
/*			BC_Types[bc_input_id].BC_Name == GD_PARAB_BC ||*/
			for(p=0;p<ielem_dim;p++)
		      ...
		      for(p=0 ; p<MAX_CA ; p++)	
			{
			  if( (CA_fselem[p] == ielem || CA_sselem[p] == ielem)
			      && CA_fselem[p] != -1 && CA_sselem[p] != -1 
			      && CA_id[p] !=-2)  jflag = p;
			}
 		      if(jflag != -1) 
			{
			   ...
			   * normal and solid surface normal */
                          ...
                          if (BC_Types[j_bc_id].BC_Name == CA_OR_FIX_BC)
			    {
 				...
 				(&fsnormal[jflag][0], 
 				 &ssnormal[jflag][0], 
				 &(BC_Types[j].BC_Data_Int[1]),
				 BC_Types[j].BC_Data_Float[0],
				 BC_Types[j].BC_Data_Float[4],
				 BC_Types[j].BC_Data_Float[5],
				 BC_Types[j].BC_Data_Float[6],
				 BC_Types[j].BC_Data_Float[7],
				 BC_Types[j].BC_Data_Float[8],
			      ...
			      if (Gibbs)
				{
				  ...
				  fapply_CA(func, d_func, d_func_ss,
					    fsnrml, dfsnrml_dx, ssnrml, dssnrml_dx,
					    BC_Types[j].BC_Data_Float[0]);
				}
			      else
				{
				  ...
				  for (p = 0; p < ielem_dim; p++) {
					...
					ldof_eqn = ei->ln_to_first_dof[MESH_DISPLACEMENT1+p][id];
                                        eqn = upd->ep[0][MESH_DISPLACEMENT1 + p];
			      ...
			      for (i1=0;i1<MAX_PDIM;i1++)
				{
			      ...
			      fapply_moving_CA(func, d_func, d_func_ss,
					       fsnrml, dfsnrml_dx, ssnrml, dssnrml_dx,
					       BC_Types[j].BC_Data_Float[0],
					       BC_Types[j].BC_Data_Float[4],
					       BC_Types[j].BC_Data_Float[5],
		...
		if (BC_Types[j].BC_Data_Int[0] == I &&
		    (BC_Types[j].BC_Name == SURFTANG_BC || 
		     BC_Types[j].BC_Name == CAP_ENDFORCE_BC)) {
		  j_bc_id = j;
		}
	      }
		...
		{
		  fapply_ST(func, d_func,
			    BC_Types[j_bc_id].BC_Data_Float[0],
			    BC_Types[j_bc_id].BC_Data_Float[1],
			    BC_Types[j_bc_id].BC_Data_Float[2],
			    BC_Types[j_bc_id].BC_Data_Float[3],
			    rcoord, (int) elem_side_bc->id_side);
		}	else	{
		fapply_ST(func, d_func,
			  BC_Types[j_bc_id].BC_Data_Float[0],
			  BC_Types[j_bc_id].BC_Data_Float[1],
		...
		{
		  if ( (BC_Types[j].BC_Data_Int[0] == I ) &&
		       (BC_Types[j].BC_Name == SHEET_ENDSLOPE_BC) ) 
		    {
		      j_bc_id = j;
		    }
	      ...
	      apply_SES(func, d_func, elem_side_bc,
			...
			BC_Types[j_bc_id].BC_Data_Float[0],
			BC_Types[j_bc_id].BC_Data_Float[1],
			id, iconnect_ptr, (int) elem_side_bc->id_side ) ;
		...
		for (p = 0; p < BC_Types[j_bc_id].desc->vector; p++)
		  {
		     ...
		     */
                    if (jflag != -1 &&
			(BC_Types[j_bc_id].BC_Name == CA_BC ||
			 BC_Types[j_bc_id].BC_Name == CA_MOMENTUM_BC ||
			...
			if (BC_Types[j_bc_id].BC_Name == CA_BC ||
			    ...
			    BC_Types[j_bc_id].BC_Name == VELO_THETA_SHIK_BC ||
			    BC_Types[j_bc_id].BC_Name == MOVING_CA_BC ||
			    BC_Types[j_bc_id].BC_Name == SHEET_ENDSLOPE_BC) weight = BIG_PENALTY;
			...
			if (af->Assemble_Jacobian) {
			  ...
			  for (var=0; var < MAX_VARIABLE_TYPES; var++)
				...
				/*			    if (pd->v[0][var] && (BC_Types[j_bc_id].desc->sens[var] || 1 ) && jflag != -1) TAB 01-08-07 */
				{
  ...
  for (ef = 0; ef < nf; ef++)
    {
      ...
      if ( (ielem_dim - nbr_dim) == 1)
		...
		{ coord[i] = fv->x[i]; }
              ...
              for (ibc=0; ibc<nbc; ibc++)
                {
                  ...
                  bc = BC_Types + BCid;
                  bc_desc = bc->desc;
                  if ((ss_index = in_list(bc->BC_ID, 0, exo->num_side_sets,
					  &(ss_to_blks[0][0]))) == -1)
                    {
                   ...
                   */
                  if ( (bc_desc->method == bc_application) && BCid != -1 &&
                       (bc->BC_Data_Int[0] == ebid1) &&
			...
			eind = R_SURF_CHARGE;
			evec = 1;
			surface_electric_field_bc(local_r, local_j,
						  m1, n_dof, wt);
			break;
			...
			evec = 1;
			surface_acoustic_velocity_bc(local_r, local_j,
						     m1, n_dof, wt, time_value);
			break;
			...
			eind = R_SHELL_USER;
			evec = 1;
			surface_user_shell_bc(local_r, local_j,
  ...
  if ( !ls->Ignore_F_deps )
    {
      ...
      for( ip=0; ip<ip_total;ip++)
        {
          xi[0] = s[ip][0];  xi[1] = s[ip][1]; xi[2] = s[ip][2];
          weight = wt[ip] / (fplus - fminus);
          
          assemble_sharp_integrated_bc( x, resid_vector, time, delta_t, theta,
                                        ielem, ielem_type, num_local_nodes, ielem_dim,
                                        iconnect_ptr, elem_side_bc, bc_application,
                                        exo, xi, weight, TRUE );
        }
...
void	  
assemble_sharp_integrated_bc( double x[],           /* Solution vector for the current processor */
			      double resid_vector[],/* Residual vector for the current processor */
			      const double time,    /* current time  */
			      const double delta_t, /* current time step size */
			      const double theta,   /* parameter (0 to 1) to vary time integration
			      ...
			      )
{
  ...
  while(  (bc_input_id = (int) elem_side_bc->BC_input_id[ibc]) != -1 )
	...
	{
	case SHARP_CA_2D_BC:
	  apply_sharp_ca(func, d_func,
			 BC_Types[bc_input_id].BC_Data_Float[0]);             
	  break;
	case SHARP_WETLIN_VELOCITY_BC:
	  apply_sharp_wetting_velocity( func, d_func, bc->BC_Name,
