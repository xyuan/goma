...

{
    ...
    if (af->Assemble_Residual) {
	  ...
	  lec->R[upd->ep[0][ieqn_filmp]][id_doffilmp] =
	       -lec->R[upd->ep[0][ieqn_lubp]][id_doflubp];
    }
    ...
    if (af->Assemble_Jacobian)
      {
	ieqn_lubp = R_LUBP;
	peqn_lubp = upd->ep[0][ieqn_lubp];
	ieqn_filmp = R_SHELL_FILMP;
	peqn_filmp = upd->ep[0][ieqn_filmp];
	id_doflubp = ei->ln_to_dof[ieqn_lubp][id];
	id_doffilmp = ei->ln_to_dof[ieqn_filmp][id];
