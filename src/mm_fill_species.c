	    ...
	    if (ei->active_interp_ledof[ledof]) {
		  ...
		  if ( pd->e[0][eqn] & T_ADVECTION )
		      ...
		      if(taylor_galerkin[w])
			{
		      ...
		      advection *= pd->etm[0][eqn][(LOG2_ADVECTION)] * det_J * h3 * wt 
...
*/
{
  ...
  if (af->Assemble_Jacobian) {
    ...
    for (b = 0; b < dim; b++) {
      var = MESH_DISPLACEMENT1 + b;
      if (pd->v[0][var] && fluxbc->BC_Name != LS_LATENT_HEAT_BC ) {
	for (j = 0; j < ei->dof[var]; j++) {
...
/***************************** EXECUTION BEGINS *******************************/
  ...
  if ( af->Assemble_Jacobian )
     {
      if (pd->v[0][MESH_DISPLACEMENT1]) memset(dnxdot_dx, 0, DIM*DIM*MDE*sizeof(dbl));
     }
      ...
      **************************************************************************/
{
  ...
  if (mp->SpeciesSourceModel[0] == SSM_CHEMKIN_GAS) {
	 ...
	 */
	err = ck_VD_dsdy(ei->mn, &pressureCGS, &(fv->T), fv->c, NULL,
    ...
    err = ck_VD_wyp(ei->mn, &pressureCGS, &(fv->T), fv->c, NULL,
		    st->MassSource);
    for (w = 0; w < pd->Num_Species_Eqn; w++) {
				 ...
				 double tt)
{
  ...
  for (p=0; p<VIM; p++)
	...
	*/
       ...
       /* calculate (sum diffusion flux)/solid volume frac. and it's derivatives 
	*  v_solvent_flux  */
/*       if (pd->v[0][MASS_FRACTION] && mp->PorousMediaType == CONTINUOUS)   */
       if (pd->v[0][MASS_FRACTION] )  
	 /* if no solvents - then no convective velocity */
	 {
	   /* calculate volume fraction of solids and total diffusion flux */
			  ...
			  struct Species_Conservation_Terms *st)
{
 ...
 if(  mp->mp2nd == NULL ||
      ...
      !(pd->e[0][R_MASS] & T_SOURCE) ) return(0);
