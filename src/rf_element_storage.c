      ...
      *****************************************************************/
{
  ...
  for (eb_index = 0; eb_index < EXO_ptr->num_elem_blocks; eb_index++) {
    ...
    if(pd->e[0][R_MESH1] && pd->MeshMotion == LAGRANGIAN) {
      if (pd->TimeIntegration == TRANSIENT) {
	do_malloc = TRUE;
      }
    }
      ...
      *****************************************************************/
{
  ...
  if (!eb_ptr->ElemStorage) {
     ...
     */
     ...
     else if (pd->e[0][R_SHELL_SAT_OPEN] || pd->e[0][R_SHELL_SAT_OPEN_2] ) {
       base_ptr = alloc_dbl_1(4 * numStorage * eb_ptr->Num_Elems_In_Block, 
			      DBL_NOINIT); 
     }
     else if(pd->e[0][R_MESH1] && pd->MeshMotion == LAGRANGIAN) {
    ...
    for (i = 0; i < eb_ptr->Num_Elems_In_Block; i++, s_ptr++) {
      if (pd->e[0][R_POR_LIQ_PRES] || pd->e[0][R_SHELL_SAT_OPEN] || pd->e[0][R_SHELL_SAT_OPEN_2] ) {
