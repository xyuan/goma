...
init_shell_element_blocks(const Exo_DB *exo)
{
  ...
  for (si = 0; si < num_shell_blocks; ++si)
	 ...
	 *   case of mesh displacements and see where we are.
         *
         *   We should also note that pd->e[0][MESH_DISPLACEMENT1] is not
	...
	if (pdv[MESH_DISPLACEMENT1] == 0) {
	  if (pdvParent[MESH_DISPLACEMENT1] && V_SOLNVECTOR) {
	    ...
	    pd->i[0][MESH_DISPLACEMENT1] = PDParent->i[0][MESH_DISPLACEMENT1];
            /*
	...
	if (pdv[MESH_DISPLACEMENT2] == 0) {
	  if (pdvParent[MESH_DISPLACEMENT2] && V_SOLNVECTOR) {
	    ...
	    pd->i[0][MESH_DISPLACEMENT2] = PDParent->i[0][MESH_DISPLACEMENT2];
	  }
	}
	if (pdv[MESH_DISPLACEMENT3] == 0) {
	  if (pdvParent[MESH_DISPLACEMENT3] && V_SOLNVECTOR) {
	    pdv[MESH_DISPLACEMENT3] = V_SPECIFIED;
	    pd->i[0][MESH_DISPLACEMENT3] = PDParent->i[0][MESH_DISPLACEMENT3];
	  }
	}
                  ...
                  double xi[DIM], const Exo_DB *exo)
{
  ...
  for (iter = 0; iter < max_iter; iter++)
      ...
      for (j = 0; j < pd->Num_Dim ; j++)
	{
	  ...
	  for(i = 0; i < dofs; i++)
	    {
	      node  = ei->dof_list[dof_list_index][i];
		       ...
		       const Exo_DB *exo)
{
  ...
  for (i = 0; i < MAX_VARIABLE_TYPES; i++)
    {
      ...
      for (j = 0; j < ndofs[i] ; j++)
	{
	  ndofptr[i][j] = ei->ieqn_ledof[ei->lvdof_to_ledof[i][j]];
	}
    }
  ...
  if (ddim == -1)
    {
      ...
      get_side_info(ei->ielem_type, id_side,
                    &nodes_per_side, local_elem_node_id);
      surface_determinant_and_normal(el2, ei->iconnect_ptr,
                                     ei->num_local_nodes, ei->ielem_dim-1,
                                     id_side, nodes_per_side,
                                     local_elem_node_id);
      if (ei->ielem_dim != 3) 
	{
	  calc_surf_tangent(el2, ei->iconnect_ptr, ei->num_local_nodes, ei->ielem_dim-1,
			    nodes_per_side, local_elem_node_id );
	}
      ...
      if (dof_map != NULL)
        {
          ...
          for (i = 0; i < ei->dof[svar]; i++)
            {
              bulk_gnn[i] = ei->gnn_list[svar][i];
            }
        }
    }
  if (ddim == 1)
      ...
      if (dof_map != NULL)
        {
          ...
          for (i = 0; i < ei->dof[svar]; i++)
            {
              shell_gnn[i] = ei->gnn_list[svar][i];
            }
        }
    }
  ...
  if (ddim == -1)
    {
      ...
      (void) copy_int_1(ShellElementParentElementCoverageForVariable, pd->v[0], MAX_VARIABLE_TYPES);
    }
  ...
  if (ddim == -1 && dof_map != NULL)
    {
      for (i = 0; i < ei->dof[svar1]; i++)
        {
          node = ei->gnn_list[svar1][i];
     ...
     ***********************************************************************/
{
  ...
  if (!pd->e[0][eqn]) EH (-1, "No normal equations on shell!");
  dofs = ei->dof[eqn];
  pdim = pd->Num_Dim;
     ...
     ***********************************************************************/
{
  ...
  DeformingMesh = (upd->ep[0][R_MESH1] != -1);
  ...
  memset(fv->dsnormal_dx,0,siz);
   ...
   /* It is set as ei->ielem_dim (not pd->Num_Dim which is usual 1 dimension more) */
 ...
 ******************************************************************************/
{
  ...
  if (pd->e[0][EQN])   
	...
	D_H_DP[i] = dH_U_dp;
      ...
      if (pd->v[0][SHELL_DELTAH] && (mp->HeightUFunctionModel == CONSTANT_SPEED_DEFORM || mp->HeightUFunctionModel == CONSTANT_SPEED_MELT || mp->HeightUFunctionModel == ROLL_ON_MELT || mp->HeightUFunctionModel == FLAT_GRAD_FLAT_MELT || mp->HeightUFunctionModel == CIRCLE_MELT )) {
	...
	CURV += fv->sh_l_curv;
      ...
      if ( pd->e[0][SHELL_LUB_CURV_2] ) {
	CURV += fv->sh_l_curv_2;
      }
      
      /* Sensitivity to height */
      if ( pd->v[0][VAR] )  D_CURV_DH = -(cos(M_PIE-dcaU-atan(slopeU)) + cos(M_PIE-dcaL-atan(-slopeL)))/(H*H); 
	...
	if (pd->v[0][VAR]) q[i] -= pow(H,3)/(k_turb * mu) * GRADH[i] * CURV * mp->surface_tension;
      }
      memset(v_avg, 0.0, sizeof(double)*DIM);
      for (i = 0; i< DIM; i++) {
	v_avg[i] -= pow(H,2)/(k_turb * mu) * ( GRADP[i] - GRAV[i] );
	v_avg[i] += 0.5 * (veloL[i] + veloU[i]);
	if (pd->v[0][VAR]) v_avg[i] -= pow(H,2)/(k_turb * mu) * GRADH[i] * CURV * mp->surface_tension;
      }
     ...
     for (i = 0; i < DIM; i++)
        {
         ...
         for ( j = 0; j < ei->dof[SHELL_SHEAR_TOP]; j++) 
            {
             LubAux->dq_dshear_top[i][j] = dq_dshear_top[i];
            }
         for ( j = 0; j < ei->dof[SHELL_SHEAR_BOT]; j++) 
            {
             LubAux->dq_dshear_bot[i][j] = dq_dshear_bot[i];
            }
         for ( j = 0; j < ei->dof[SHELL_CROSS_SHEAR]; j++) 
            {
