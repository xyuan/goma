 ...
 ******************************************************************************/
 ...
 if ( pd->e[0][R_LUBP_2] ) {
  ...
  v_dot_DF = 0.0;
  if ( pd->TimeIntegration != STEADY && pd->v[0][MESH_DISPLACEMENT1] )
    {
  ...
  if ( Fill_Wt_Fcn == FILL_WEIGHT_SUPG )
    {
      ...
      memset(vc_dot_Dphi, 0, sizeof(double)*MDE);
      supg_term = 0.;
      get_supg_stuff(&supg_term, vcent, d_vcent_du, d_supg_term_du, d_supg_term_dx, pd->e[0][R_MESH1]);
    }
...
/*  tau_gls = 1./sqrt((2./dt)*(2./dt) + (2.*vmag_old/h_elem)*(2.*vmag_old/h_elem));  */
  ...
  for ( a=0; a<pfd->num_phase_funcs; a++)
   ...
   if ( pd->e[0][R_LUBP_2] )  safe_free((void *) n_dof);
  return (0);
}
