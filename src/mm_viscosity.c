          ...
          VISCOSITY_DEPENDENCE_STRUCT *d_mu)
{
  ...
  if (gn_local->ConstitutiveEquation == NEWTONIAN) 
      ...
      if (mp->ViscosityModel == USER )
	{
	  ...
	  if ( d_mu != NULL && pd->v[0][MESH_DISPLACEMENT1] )
	    {
	      for ( a=0; a<dim; a++)
		{
	  ...
	  if ( d_mu != NULL && pd->v[0][MASS_FRACTION] )
	    {
  ...
  /* this section is for all non-Newtonian models */
      ...
      if(gn_local->atexpModel == LEVEL_SET)
	{
      ...
      var = TEMPERATURE;
      if ( d_mu != NULL && pd->v[0][TEMPERATURE] )
                    ...
                    VISCOSITY_DEPENDENCE_STRUCT *d_mu )
{
   ...
   * d( mu )/dv
   */
  if ( d_mu != NULL && pd->e[0][R_MOMENTUM1] )
    {
      for ( a=0; a<VIM; a++)
        {
          for ( i=0; i<vdofs; i++)
		  ...
		  VISCOSITY_DEPENDENCE_STRUCT *d_mu)
{
  ...
  var = TEMPERATURE;
  if ( pd->e[0][var] && (temp != 0.) &&  (mp->reference[TEMPERATURE] != 0.))
    {
#if MELTING_BINGHAM
      /* melting version */
      if(temp <= tmelt)
		     ...
		     dbl alpha_g, dbl Aexp)
{
  ...
  if (Num_Var_In_Type[MASS_FRACTION])
	  ...
	  mp->volumeFractionGas <= 0.99 ) 
	{
	  ...
	  for (j = 0; j < Num_Var_In_Type[MASS_FRACTION]; j++) 
	    {
